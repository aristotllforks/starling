...
public class FunctionCompilationContext extends AbstractFunctionContext {
  ...
  /**
   * Gets the computation target resolver configured for the correct version/correction. Functions shouldn't need to access this directly as their
   * target will always be resolved when they are invoked.
   *
  ...
  /**
   * Gets the raw computation target resolver. Functions should not need to access this directly - the resolver returned by
   * {@link #getComputationTargetResolver} is correctly configured for the version/correction time the owning view process needs.
   *
   * @return the computation target resolver, null if not in the context
   */
  ...
  /**
   * Gets the view calculation configuration information. This may only be valid during dependency graph construction and not during function
   ...
   * <p>
   * There is no explicit setter for this context member as the current implementation of {@link CompiledFunctionService} uses the initialization
   * identifier in a particular fashion. Functions should not rely on this relationship as it may be removed/changed in future releases.
   *
  ...
  /**
   * Returns the function blacklist to use when executing a graph. This is part of the compilation context because the blacklist applies
   * immediately before the graph is submitted for execution, before an execution context is valid.
   *
  ...
  /**
   * Sets the function blacklist to use when executing a graph. This is part of the compilation context because the blacklist applies
   * immediately before the graph is submitted for execution, before an execution context is valid.
   *
