...
public interface MarketDataProvider {
  ...
  //-------------------------------------------------------------------------
  /**
   * Attempts to subscribe a user to a piece of market data. All listeners will be notified of the result asynchronously.
   * The existence of a subscription might notify the provider that the value should be included in snapshots.
   *
  ...
  /**
   * Attempts to subscribe a user to a set of market data. All listeners will be notified of the result asynchronously.
   * The existence of a subscription might notify the provider that the value should be included in snapshots.
   *
   ...
   *
   * @param valueSpecification the market data specification as passed to an earlier call to {@link #subscribe(ValueSpecification)}
   * or {@link #subscribe(Set)}, not null
   */
   ...
   *
   * @param valueSpecifications the market data specifications as passed to earlier calls to {@link #subscribe(ValueSpecification)} or
   * {@link #subscribe(Set)}, not null
   */
  void unsubscribe(Set<ValueSpecification> valueSpecifications);

  //-------------------------------------------------------------------------
  /**
   * Gets the availability provider for this market data provider. It is expected that obtaining an accurate availability provider
   * could be a heavy operation. This method is called every time a view definition is compiled, in order to build the dependency
   * graphs, and the result on each occasion is cached and reused throughout that compilation.
   *
  ...
  //-------------------------------------------------------------------------
  /**
   * Gets whether a market data specification is compatible with this market data provider. It does not necessarily indicate
   * that the specification can be satisfied, only whether the market data provider knows how to make the best attempt to satisfy it.
   *
  ...
  /**
   * Gets the real-time duration between two instants on the market data time-line. If the market
   * data provider is replaying data at a different rate from normal then this will not correspond to the
