...
public interface ViewProcessWorkerContext {
  ...
  /**
   * Returns the context for the owning process. The process context may contain both shared, immutable, state and configurable items. In the case
   * where the job is spawned remotely, the remote host should construct its own equivalent process context with direct reference to the shared items.
   * Only the configurable items should read (or write) through to the origin process.
   *
