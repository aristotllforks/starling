 ...
 *
 */
public final class SwapDetailsCalculator extends InstrumentDerivativeVisitorAdapter<SwapDetailsProvider, SwapLegCashFlows> {

  @Override
  public SwapLegCashFlows visitFixedCouponSwap(final SwapFixedCoupon<?> derivative, final SwapDetailsProvider provider) {
    return getCashFlows(derivative, provider);
  }

  @Override
  public SwapLegCashFlows visitSwap(final Swap<?, ?> derivative, final SwapDetailsProvider provider) {
    return getCashFlows(derivative, provider);
  }

  private static SwapLegCashFlows getCashFlows(final Swap<?, ?> swapDerivative, final SwapDetailsProvider provider) {
    ...
    final ZonedDateTime valuationTime = provider.getValuationTime();
    final MulticurveProviderInterface bundle = provider.getCurves();
