...
/**
 * Convert a FRA node into an Instrument definition. The dates of the FRA are computed in the following way: - The spot date is computed from the trade date
 * time adding the "Settlement Days" (i.e. the number of business days) of the convention. - The accrual start date is computed from the spot date adding the
 * "FixingStart" of the node and using the business-day-convention, calendar and EOM of the convention. - The accrual end date is computed from the spot date
 * adding the "FixingEnd" of the node and using the business-day-convention, calendar and EOM of the convention. The FRA notional is 1.
 */
public class SecurityFromNodeConverter extends CurveNodeVisitorAdapter<FinancialSecurity> {
   ...
   * @param identifier
   *          The floating rate identifier
   * @param tradeDateTime
   *          The trade date time, not null
   */
  public SecurityFromNodeConverter(final ConventionSource conventionSource, final HolidaySource holidaySource, final RegionSource regionSource,
      final ZonedDateTime tradeDateTime, final Double rate, final Double amount, final ExternalId identifier) {
  ...
  public FRASecurity visitFRANode(final FRANode fraNode) {
    ...
    final ZonedDateTime fixingDate = ScheduleCalculator.getAdjustedDate(accrualStartDate, -iborIndex.getSpotLag(), fixingCalendar);
    final FRASecurity security = new FRASecurity(currency, indexConvention.getRegionCalendar(), accrualStartDate, accrualEndDate, _rate, 1, _identifier,
        fixingDate);
  ...
  public FinancialSecurity visitSwapNode(final SwapNode swapNode) {
    ...
    final Triple<? extends SwapLeg, ZonedDateTime, ZonedDateTime> payLeg = NodeConverterUtils.createSwapLeg(payLegConvention,
        swapNode.getStartTenor().getPeriod(), swapNode.getMaturityTenor().getPeriod(), _regionSource, _holidaySource, _conventionSource, snapshotDataBundle,
        _identifier, _tradeDateTime, true, isFloatFloat);

    final Triple<? extends SwapLeg, ZonedDateTime, ZonedDateTime> receiveLeg = NodeConverterUtils.createSwapLeg(receiveLegConvention,
        swapNode.getStartTenor().getPeriod(), swapNode.getMaturityTenor().getPeriod(), _regionSource, _holidaySource, _conventionSource, snapshotDataBundle,
        _identifier, _tradeDateTime, false, isFloatFloat);

    final SwapSecurity security = new SwapSecurity(_tradeDateTime, payLeg.getSecond(), payLeg.getThird(), "counterparty", payLeg.getFirst(),
        receiveLeg.getFirst());
  ...
  public CashSecurity visitCashNode(final CashNode cashNode) {
    ...
    if (convention instanceof DepositConvention) {
      ...
      return new CashSecurity(currency, depositConvention.getRegionCalendar(), startDate, endDate, dayCount, _rate, _amount);
      ...
      final ZonedDateTime startDate = ScheduleCalculator.getAdjustedDate(spotDate, startPeriod, businessDayConvention, calendar, isEOM);
      final ZonedDateTime endDate = ScheduleCalculator.getAdjustedDate(startDate, maturityPeriod, businessDayConvention, calendar, isEOM);
      final CashSecurity security = new CashSecurity(currency, iborConvention.getRegionCalendar(), startDate, endDate, dayCount, _rate, _amount);
  ...
  private InterestRateFutureSecurity getInterestRateFuture(final RateFutureNode rateFuture, final InterestRateFutureConvention futureConvention,
    ...
    final ExchangeTradedInstrumentExpiryCalculator expiryCalculator = ExchangeTradedInstrumentExpiryCalculatorFactory.of(expiryCalculatorName);
    ...
    final ZonedDateTime expiryDate = ZonedDateTime.of(expiryCalculator.getExpiryDate(rateFuture.getFutureNumber(), startDate.toLocalDate(), regionCalendar),
        time, timeZone);
    final InterestRateFutureSecurityDefinition securityDefinition = new InterestRateFutureSecurityDefinition(expiryDate, iborIndex, 1, paymentAccrualFactor, "",
        fixingCalendar);
    final InterestRateFutureTransactionDefinition transactionDefinition = new InterestRateFutureTransactionDefinition(securityDefinition, 1, _tradeDateTime,
        price);
  ...
  private FederalFundsFutureSecurity getFederalFundsFuture(final RateFutureNode rateFuture, final FederalFundsFutureConvention futureConvention,
    ...
    final LocalTime time = startDate.toLocalTime();
    final ZoneId timeZone = startDate.getZone();
    final ZonedDateTime expiryDate = ZonedDateTime.of(expiryCalculator.getExpiryDate(rateFuture.getFutureNumber(), startDate.toLocalDate(), calendar), time,
        timeZone);
    final FederalFundsFutureSecurityDefinition securityDefinition = FederalFundsFutureSecurityDefinition.from(expiryDate, index, 1, paymentAccrualFactor, "",
        calendar);
    final FederalFundsFutureTransactionDefinition transactionDefinition = new FederalFundsFutureTransactionDefinition(securityDefinition, 1, _tradeDateTime,
        price);
    // return transactionDefinition;
