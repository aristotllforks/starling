...
public class ConfigDBCurveSpecificationBuilder implements CurveSpecificationBuilder {
  ...
  public ConfigDBCurveSpecificationBuilder(final ConfigSource configSource, final VersionCorrection versionCorrection) {
    this(new ConfigSourceQuery<>(configSource, CurveNodeIdMapper.class, versionCorrection));
  }

  private ConfigDBCurveSpecificationBuilder(final ConfigSourceQuery<CurveNodeIdMapper> queryCurveNodeIdMapper) {
  ...
  public static ConfigDBCurveSpecificationBuilder init(final FunctionCompilationContext context, final FunctionDefinition function) {
    return new ConfigDBCurveSpecificationBuilder(ConfigSourceQuery.init(context, function, CurveNodeIdMapper.class));
  }
  ...
  public CurveSpecification buildCurve(final Instant valuationTime, final LocalDate curveDate, final CurveDefinition curveDefinition) {
    ...
    if (curveDefinition instanceof InterpolatedCurveDefinition) {
      return getInterpolatedCurveSpecification(curveDate, (InterpolatedCurveDefinition) curveDefinition);
    }
    return getCurveSpecification(curveDate, curveDefinition);
  }
  ...
  public AbstractCurveSpecification buildSpecification(final Instant valuationTime, final LocalDate curveDate, final AbstractCurveDefinition curveDefinition) {
    ...
    if (curveDefinition instanceof InterpolatedCurveDefinition) {
      ...
      return getCurveSpecification(curveDate, (CurveDefinition) curveDefinition);
    } else if (curveDefinition instanceof ConstantCurveDefinition) {
      return getConstantCurveSpecification(curveDate, (ConstantCurveDefinition) curveDefinition);
   ...
   */
  private CurveSpecification getCurveSpecification(final LocalDate curveDate, final CurveDefinition curveDefinition) {
    ...
    for (final CurveNode node : curveDefinition.getNodes()) {
      ...
      final CurveNodeIdMapper builderConfig = getCurveNodeIdMapper(cache, curveSpecificationName);
   ...
   */
  private InterpolatedCurveSpecification getInterpolatedCurveSpecification(final LocalDate curveDate, final InterpolatedCurveDefinition curveDefinition) {
    final Map<String, CurveNodeIdMapper> cache = new HashMap<>();
    final Collection<CurveNodeWithIdentifier> identifiers = new ArrayList<>();
    final String curveName = curveDefinition.getName();
    for (final CurveNode node : curveDefinition.getNodes()) {
   ...
   * @return The curve specification
   */
  private static AbstractCurveSpecification getConstantCurveSpecification(final LocalDate curveDate, final ConstantCurveDefinition curveDefinition) {
    final String curveName = curveDefinition.getName();
  ...
  //   * @return The curve specification
  //   */
  // private AbstractCurveSpecification getSpreadCurveSpecification(final Instant valuationTime, final LocalDate curveDate, final SpreadCurveDefinition
  // curveDefinition) {
   ...
   */
  private CurveNodeIdMapper getCurveNodeIdMapper(final Map<String, CurveNodeIdMapper> cache, final String curveSpecificationName) {
