...
public class BloombergCommodityFutureOptionVolatilitySurfaceInstrumentProvider extends BloombergFutureOptionVolatilitySurfaceInstrumentProvider {
   ...
   *          the prefix to the resulting code (e.g. "S " for Soybeans), not null
   * @param postfix
   *          the postfix to the resulting code (e.g. Comdty), not null
   * @param dataFieldName
   ...
   */
  public BloombergCommodityFutureOptionVolatilitySurfaceInstrumentProvider(final String futureOptionPrefix, final String postfix, final String dataFieldName,
   ...
   * <p>
   * The format is prefix + expiry code + callPutFlag + strike + postfix
   * <p>
   * e.g. S U3P 45 Comdty
   * <p>
   * 
  ...
  public ExchangeTradedInstrumentExpiryCalculator getExpiryRuleCalculator() {
    ...
    if (expiryRule == null) {
      throw new OpenGammaRuntimeException(
          "No expiry rule has been setup for " + getFutureOptionPrefix() + ". Determine week and day pattern and add to EXPIRY_RULES.");
    }
    return expiryRule;
  }
