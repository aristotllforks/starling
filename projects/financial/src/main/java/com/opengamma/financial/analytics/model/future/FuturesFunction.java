...
public abstract class FuturesFunction<T> extends AbstractFunction.NonCompiledInvoker {
   ...
   *          String describes the value requested
   * @param calculator
   *          The calculator
   ...
   */
  protected abstract ValueProperties.Builder createValueProperties(ComputationTarget target);
   ...
   * @param timeSeriesBundle
   *          A bundle containing time series
   * @param desiredValue
   *          The desired value
   * @return The data bundle used for pricing futures
   */
  protected abstract SimpleFutureDataBundle getFutureDataBundle(FutureSecurity security, FunctionInputs inputs,
      HistoricalTimeSeriesBundle timeSeriesBundle, ValueRequirement desiredValue);
  ...
  protected ValueRequirement getReferencePriceRequirement(final FunctionCompilationContext context, final FutureSecurity security) {
    ...
    final HistoricalTimeSeriesResolutionResult timeSeries = resolver.resolve(security.getExternalIdBundle(), null, null, null,
        MarketDataRequirementNames.MARKET_VALUE, getResolutionKey());
    if (timeSeries == null) {
