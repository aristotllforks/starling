...
/**
 * Convert rate futures nodes into an {@link InstrumentDefinition}. The dates of the futures are computed in the following way: - The start date is the
 * valuation date plus the "startTenor" without convention. - The last trade date is computed from the expiry calculator from the start date, plus the number of
 * futures. - The delivery date is computed from the last trade date adding the "Settlement Days" (i.e. the number of business days) of the swap convention. The
 * futures notional is 1.
 */
public class RateFutureNodeConverter extends CurveNodeVisitorAdapter<InstrumentDefinition<?>> {
  ...
  private InstrumentDefinition<?> getFederalFundsFuture(final RateFutureNode rateFuture, final FederalFundsFutureConvention futureConvention,
    ...
    final ZonedDateTime expiryDate = ZonedDateTime.of(expiryCalculator.getExpiryDate(rateFuture.getFutureNumber(), startDate.toLocalDate(), calendar), time,
        timeZone);
    final FederalFundsFutureSecurityDefinition securityDefinition = FederalFundsFutureSecurityDefinition.from(expiryDate, indexON, 1, paymentAccrualFactor, "",
        calendar);
