...
/**
 * Utility Class for computing Expiries of Future Options from ordinals (i.e. nth future after valuationDate) For IR Options use:
 * TemporalAdjusters.dayOfWeekInMonth(3, DayOfWeek.WEDNESDAY), new NextExpiryAdjuster() For Equity Options use: new SaturdayAfterThirdFridayAdjuster(), new
 * NextEquityExpiryAdjuster()
 */
...
public final class FutureOptionExpiries implements ExchangeTradedInstrumentExpiryCalculator {
   ...
   *
   * @param nextExpiryAdjuster
   *          Examples: NextExpiryAdjuster, NextExpiryAdjuster
   */
  ...
  /**
   * Create instance of {@code FutureOptionExpiries}, a utility Class for computing Expiries of Future Options from ordinals (i.e. nth option after
   * valuationDate)
   *
   * @param nextExpiryAdjuster
   *          Examples: NextExpiryAdjuster, NextExpiryAdjuster
   ...
   * valuationDate).
   * <p>
   * Specify the day and week in the month, plus an offset in number of days.
   * <p>
   * e.g. (3,DayOfWeek.FRIDAY,1) is the Saturday after the 3rd Friday in the month. This is different from the 3rd Saturday.
   *
   * @param week
   *          Ordinal of week in month, beginning from 1.
   * @param day
   *          DayOfWeek
   * @param offset
   *          Integer offset, positive or negative from the result of week,day.
  ...
  /**
   * @deprecated Compute time between now and future or future option's settlement date, typically two business days before the third wednesday of the expiry
   *             month.
   * @param n
   ...
   *          Valuation Date
   * @param tenor
   *          The tenor of future option resets
   * @return OG-Analytic Time in years between now and the future's settlement date
   */
  ...
  public Double getFutureTtm(final int n, final LocalDate today) {
    ...
    final LocalDate previousMonday = expiry.minusDays(2); // TODO this should take a calendar and do two business days, and should use a convention for the
                                                          // number of days
    return TimeCalculator.getTimeBetween(today, previousMonday);
  }

  /**
   * @deprecated Hard-codes in assumptions about which expiries to look for Gets monthly expiries for the first six months, then switch to quarterly
   * @param nthFuture
   *          nth future
   * @param valDate
  ...
  /**
   * @deprecated Hard-codes in assumptions about which expiries to look for Get monthly expiries for the first 6 expires then look for January yearly ones CME
   *             options seem to follow no clear pattern some switch to yearly after 4 monthly options and some have 8 or more pick a value in the middle so
   *             hopefully we get a reasonable number of valid expires for all options
   * @param nthFuture
   *          nth future in the future
   * @param valDate
   *          date to start from
  ...
  /**
   * @deprecated Hard-codes in assumptions about which expiries to look for. Get n'th future expiry. Only supports One Chicago equity futures. 2 serial months
   *             and 2 quarterly http://www.onechicago.com/?page_id=22
   * @param nthFuture
   *          nth future in the future
   * @param valDate
   *          date to start from
   ...
   *           If the tenor is not monthly or quarterly
   */
  ...
  /**
   * Produced n'th expiry date after today of monthly or quarterly tenor.
   * <p>
   ...
   *          valuation date
   * @param tenor
   *          accepts ONE_MONTH or THREE_MONTHS
