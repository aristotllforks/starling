 ...
 * <p>
 * For example, IMM future options use the HMUZ schedule and expire two business days before the third Wednesday of the month, so on 1/1/2012, the expiry date
 * of the third option is the 17th of September.
 */
public interface ExchangeTradedInstrumentExpiryCalculator extends NamedInstance {
   ...
   * @deprecated use {@link #getExpiryDate(int, LocalDate, WorkingDayCalendar)}
   */
  @Deprecated
   ...
   * @return the expiry date, not null
   */
  LocalDate getExpiryDate(int n, LocalDate today, WorkingDayCalendar holidayCalendar);

  /**
   * Gets the expiry month of a future.
   * <p>
   * Given a LocalDate representing the valuation date and an integer representing the n'th expiry after that date, returns a date in the expiry month.
   *
   * @param n
   *          the n'th expiry date after today, greater than zero
   * @param today
