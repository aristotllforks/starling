...
public final class NodeConverterUtils {
   ...
   */
  private NodeConverterUtils() {
  }

  /**
   * Creates a {@link SwapDefinition} from the pay and receive conventions of a swap. If both legs are floating, the spread will be on the receiver leg.
   *
  ...
  public static SwapDefinition getSwapDefinition(
    ...
    final AnnuityDefinition<? extends PaymentDefinition> payLeg = getSwapLeg(payLegConvention, startTenor, maturityTenor, securitySource, regionSource,
        holidaySource,
        conventionSource, marketData, dataId, valuationTime, true, isFloatFloat, isXCcy, 1.0);
    final AnnuityDefinition<? extends PaymentDefinition> receiveLeg = getSwapLeg(receiveLegConvention, startTenor, maturityTenor, securitySource, regionSource,
        holidaySource,
   ...
   * @param receiveLegConvention
   *          The receive leg convention, not null
   * @param unadjustedStartDate
   *          Unadjusted start date. The roll date adjuster will start at that date.
   * @param startNumberRollDate
   *          The roll date adjuster start number.
   * @param endNumberRollDate
   *          The roll date adjuster end number.
   * @param adjuster
   *          The roll date adjuster. Not null.
   * @param securitySource
   *          The security source, not null
   * @param regionSource
   *          The region source, not null
  ...
  public static SwapDefinition getSwapRollDateDefinition(
    ...
    final AnnuityDefinition<? extends PaymentDefinition> payLeg = getRollDateSwapLeg(payLegConvention, unadjustedStartDate, startNumberRollDate,
        endNumberRollDate, adjuster,
        securitySource, regionSource, holidaySource, conventionSource, marketData, dataId, valuationTime, true, isFloatFloat);
    final AnnuityDefinition<? extends PaymentDefinition> receiveLeg = getRollDateSwapLeg(receiveLegConvention, unadjustedStartDate, startNumberRollDate,
        endNumberRollDate, adjuster,
   ...
   *          The number of the start date in the calendar non-good business days.
   * @param endDateNumber
   *          The number of the end date in the calendar non-good business days.
   * @param calendarStartEndDate
   *          The calendar with the start and end dates of the swap.
   * @param securitySource
   *          The security source, not null
   * @param regionSource
   *          The region source, not null
  ...
  public static SwapDefinition getSwapCalendarDefinition(
    ...
    ArgumentChecker.notNull(valuationTime, "valuation time");
    final boolean isFloatFloat = isFloatFloatRoll(payLegConvention, receiveLegConvention);
    final AnnuityDefinition<? extends PaymentDefinition> payLeg = getCalendarSwapLeg(payLegConvention, unadjustedStartDate, startDateNumber, endDateNumber,
        calendarStartEndDate,
        securitySource, regionSource, holidaySource, conventionSource, marketData, dataId, true, isFloatFloat);
    final AnnuityDefinition<? extends PaymentDefinition> receiveLeg = getCalendarSwapLeg(receiveLegConvention, unadjustedStartDate, startDateNumber,
        endDateNumber, calendarStartEndDate,
   ...
   *          The valuation time
   * @param isPayer
   *          True if the leg is to be paid
   * @param isMarketDataSpread
   *          True if the market data is a spread paid on a floating leg
   * @param isXCcy
   *          Flag indicating if the swap is cross-currency.
   * @param notional
   *          Notional of the leg. Used in particular for cross-currency swaps.
   * @return An annuity definition
   * @throws UnsupportedOperationException
   *           If the convention type has not been implemented
   */
  public static AnnuityDefinition<? extends PaymentDefinition> getSwapLeg(
    ...
    final FinancialConventionVisitor<AnnuityDefinition<? extends PaymentDefinition>> visitor =
      ...
      public AnnuityCouponFixedDefinition visitSwapFixedLegConvention(final SwapFixedLegConvention convention) {
        ...
        final Period paymentPeriod = convention.getPaymentTenor().getPeriod();
        if (isXCcy) {
          return AnnuityDefinitionBuilder.couponFixedWithNotional(currency, startDate, maturityDate, paymentPeriod, calendar, dayCount, businessDayConvention,
              eomLeg,
      ...
      public AnnuityDefinition<? extends PaymentDefinition> visitVanillaIborLegConvention(final VanillaIborLegConvention convention) {
        ...
        if (sec == null) {
          ...
          if (indexConvention == null) {
            throw new OpenGammaRuntimeException("Could not get ibor index convention with id " + convention.getIborIndexConvention());
          }
          index = ConverterUtils.indexIbor(indexConvention.getName(), indexConvention, Tenor.of(indexTenor));
          calendar = CalendarUtils.getCalendar(regionSource, holidaySource, indexConvention.getRegionCalendar());
        } else {
          final com.opengamma.financial.security.index.IborIndex indexSecurity = (com.opengamma.financial.security.index.IborIndex) sec;
          final IborIndexConvention indexConvention = conventionSource.getSingle(indexSecurity.getConventionId(), IborIndexConvention.class);
          if (indexConvention == null) {
            throw new OpenGammaRuntimeException("Could not get ibor index convention with id " + indexSecurity.getConventionId());
          }
        ...
        if (isPayer && isMarketDataSpread) { // Implementation note: Market data is used as spread on pay leg
          ...
          if (isXCcy) {
            return AnnuityDefinitionBuilder.couponIborSpreadWithNotional(startDate, maturityDate, notional, spread, index, isPayer, calendar, stub, paymentLag,
                true, true);
          }
   ...
   *          The leg convention
   * @param unadjustedStartDate
   *          The swap unadjusted start date.
   * @param rollDateStartNumber
   *          The start number for the roll date adjustment.
   * @param rollDateEndNumber
   *          The end number for the roll date adjustment.
   * @param adjuster
  ...
  private static AnnuityDefinition<? extends PaymentDefinition> getRollDateSwapLeg(
      final FinancialConvention legConvention, final ZonedDateTime unadjustedStartDate, final int rollDateStartNumber, final int rollDateEndNumber,
    ...
    final FinancialConventionVisitor<AnnuityDefinition<? extends PaymentDefinition>> visitor =
      ...
      public AnnuityDefinition<? extends PaymentDefinition> visitFixedLegRollDateConvention(final FixedLegRollDateConvention convention) {
        ...
        final Period paymentPeriod = convention.getPaymentTenor().getPeriod();
        final int paymentLag = convention.getPaymentLag();
        return AnnuityDefinitionBuilder.couponFixedRollDate(currency, adjustedStartDate, rollDateStartNumber, rollDateEndNumber, adjuster, paymentPeriod, 1,
            rate,
      ...
      public AnnuityDefinition<? extends PaymentDefinition> visitVanillaIborLegRollDateConvention(final VanillaIborLegRollDateConvention convention) {
        ...
        if (isPayer && isMarketDataSpread) { // Implementation note: Market data is used as spread on pay leg
          ...
          return AnnuityDefinitionBuilder.couponIborSpreadRollDateIndexAdjusted(adjustedStartDate, rollDateStartNumber, rollDateEndNumber, adjuster, indexIbor,
              1, spread,
    ...
    final FinancialConventionVisitor<AnnuityDefinition<? extends PaymentDefinition>> visitor =
      ...
      public AnnuityDefinition<? extends PaymentDefinition> visitOISLegConvention(final OISLegConvention convention) {
        ...
        if (isPayer && isMarketDataSpread) { // Implementation note: Market data is used as spread on pay leg
          ...
          return AnnuityDefinitionBuilder.couponONSimpleCompoundedSpreadSimplified(effectiveDateTime, maturityDateTime, paymentPeriod, 1.0d,
              spread, indexON, isPayer, businessDayConvention, eomLeg, calendar, stub, paymentLag);
        }
        return AnnuityDefinitionBuilder.couponONSimpleCompoundedSpreadSimplified(effectiveDateTime, maturityDateTime, paymentPeriod, 1.0d, 0.0d,
            indexON, isPayer, businessDayConvention, eomLeg, calendar, stub, paymentLag);
      }
      ...
      public AnnuityCouponFixedDefinition visitSwapFixedLegConvention(final SwapFixedLegConvention convention) {
        ...
        final StubType stub = convention.getStubType();
        final Period paymentPeriod = convention.getPaymentTenor().getPeriod();
        return AnnuityDefinitionBuilder.couponFixed(currency, effectiveDateTime, maturityDateTime, paymentPeriod, calendar, dayCount,
  ...
  /**
   * Constructs an {@link AnnuityDefinition} from the pay and receive conventions of a swap. If the convention type has not been implemented, throws an
   * exception
   *
   * @param legConvention
   *          The leg convention
  ...
  public static Triple<? extends SwapLeg, ZonedDateTime, ZonedDateTime> createSwapLeg(
    ...
    final FinancialConventionVisitor<Triple<? extends SwapLeg, ZonedDateTime, ZonedDateTime>> visitor =
      ...
      public Triple<? extends SwapLeg, ZonedDateTime, ZonedDateTime> visitCompoundingIborLegConvention(final CompoundingIborLegConvention convention) {
        ...
        // final IborIndex iborIndex = new IborIndex(currency, indexTenor, spotLag, dayCount, businessDayConvention, eomIndex,
