...
public class DbHistoricalTimeSeriesDataPointsWorker extends AbstractDbMaster {
  ...
  public UniqueId updateTimeSeriesDataPoints(final ObjectIdentifiable objectId, final LocalDateDoubleTimeSeries series) {
    ...
    final Pair<UniqueId, Instant> result = getTransactionTemplateRetrying(getMaxRetries()).execute(new TransactionCallback<Pair<UniqueId, Instant>>() {
      @Override
  ...
  protected void insertDataPointsCheckMaxDate(final UniqueId uniqueId, final LocalDateDoubleTimeSeries series) {
    ...
    if (result != null) {
      final LocalDate maxDate = DbDateUtils.fromSqlDateAllowNull(result);
      if (!series.getEarliestTime().isAfter(maxDate)) {
  ...
  protected UniqueId insertDataPoints(final UniqueId uniqueId, final LocalDateDoubleTimeSeries series, final Instant now) {
    ...
    for (final Entry<LocalDate, Double> entry : series) {
  ...
  protected UniqueId createTimeSeriesUniqueId(final long oid, final Instant verInstant, final Instant corrInstant) {
    final String oidStr = DATA_POINT_PREFIX + oid;
    final Duration dur = Duration.between(verInstant, corrInstant);
    final String verStr = verInstant.toString() + dur.toString();
    return UniqueId.of(getUniqueIdScheme(), oidStr, verStr);
  }

  @Override
  protected long extractRowId(final UniqueId uniqueId) {
  ...
  protected final class DataPointsExtractor implements ResultSetExtractor<LocalDateDoubleTimeSeries> {
    ...
    public LocalDateDoubleTimeSeries extractData(final ResultSet rs) throws SQLException, DataAccessException {
      final List<LocalDate> dates = new ArrayList<>(256);
      final List<Double> values = new ArrayList<>(256);
      LocalDate last = null;
      while (rs.next()) {
        final LocalDate date = DbDateUtils.fromSqlDateAllowNull(rs.getDate("POINT_DATE"));
        if (!date.equals(last)) {
          last = date;
          final Double value = (Double) rs.getObject("POINT_VALUE");
          if (value != null) {
            dates.add(date);
            values.add(value);
          }
  ...
  protected final class UniqueIdExtractor implements ResultSetExtractor<UniqueId> {
    ...
    public UniqueId extractData(final ResultSet rs) throws SQLException, DataAccessException {
      while (rs.next()) {
        ...
        final Instant verInstant = DbDateUtils.fromSqlTimestamp(ver);
        final Instant corrInstant = corr != null ? DbDateUtils.fromSqlTimestamp(corr) : verInstant;
  ...
  protected final class ManageableHTSExtractor implements ResultSetExtractor<ManageableHistoricalTimeSeries> {
    ...
    public ManageableHistoricalTimeSeries extractData(final ResultSet rs) throws SQLException, DataAccessException {
      while (rs.next()) {
        final Timestamp ver = rs.getTimestamp("max_ver_instant");
        final Timestamp corr = rs.getTimestamp("max_corr_instant");
        final Instant verInstant = ver != null ? DbDateUtils.fromSqlTimestamp(ver) : null;
        final Instant corrInstant = corr != null ? DbDateUtils.fromSqlTimestamp(corr) : verInstant;
