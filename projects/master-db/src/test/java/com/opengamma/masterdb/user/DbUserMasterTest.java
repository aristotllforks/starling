...
public class DbUserMasterTest extends AbstractDbTest {
  ...
  public void test_example() throws Exception {
    ...
    final ManageableUser loaded = _userMaster.getById(uid.getObjectId());
  ...
  public void test_update_main() throws Exception {
    ...
    final UniqueId uid2 = _userMaster.update(user);
    user.setUniqueId(uid2);
    final ManageableUser loaded = _userMaster.getById(uid2.getObjectId());
    assertEquals(user, loaded);
  }
  ...
  public void test_update_linked() throws Exception {
    ...
    final UserEventHistoryResult events = _userMaster.eventHistory(new UserEventHistoryRequest(uid2.getObjectId()));
    assertEquals(2, events.getEvents().size());
    assertEquals(HistoryEventType.ADDED, events.getEvents().get(0).getType());
    assertEquals(0, events.getEvents().get(0).getChanges().size());
  ...
  public void test_update_rename_fail() throws Exception {
    ...
    final UniqueId uid2 = _userMaster.add(user2);
  ...
  public void test_search() throws Exception {
    ...
    final UserSearchResult result = _userMaster.search(new UserSearchRequest());
  ...
  private ManageableUser createUser2() {
    final ManageableUser user = new ManageableUser(TEST_USER2);
