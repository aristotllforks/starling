...
/**
 * Tests for {@link BulkTimeSeriesOperations}.
 */
@Test(groups = "unit")
public class BulkTimeSeriesOperationsTest {
  ...
  /**
   * Tests the intersection of multiple time series.
   */
  @Test
  public void testBulkIntersection() {
    final LocalDateDoubleTimeSeries one = ImmutableLocalDateDoubleTimeSeries.of(DATES1, VALUES1);
    final LocalDateDoubleTimeSeries two = ImmutableLocalDateDoubleTimeSeries.of(DATES2, VALUES2);
    final LocalDateDoubleTimeSeries three = ImmutableLocalDateDoubleTimeSeries.of(DATES3, VALUES3);
    final LocalDateDoubleTimeSeries four = ImmutableLocalDateDoubleTimeSeries.of(DATES4, VALUES4);

    final LocalDateDoubleTimeSeries[] inputs = new LocalDateDoubleTimeSeries[] {one, two, three, four };
    final DoubleTimeSeries<LocalDate>[] intersection = BulkTimeSeriesOperations.intersection(inputs);
    for (int i = 0; i < intersection.length; i++) {
      final LocalDate[] timesArray = intersection[i].timesArray();
      assertEquals(Arrays.asList(RESULT), Arrays.asList(timesArray));
      final Double[] valuesArray = intersection[i].valuesArray();
      for (int j = 0; j < timesArray.length; j++) {
        assertEquals((i + 1) * timesArray[j].getDayOfMonth(), valuesArray[j], 1E-20);
      }
    }
