...
public class TimeSeriesLoaderTest {
  ...
  public void testTimeSeriesWritersExistingHts() {
    ...
    final TimeSeriesWriter writer = new MasterTimeSeriesWriter(htsMaster);
    ...
    final HistoricalTimeSeriesInfoSearchResult result = htsMaster.search(request);
  ...
  public void testTimeSeriesLoaderTool() throws FileNotFoundException {
    ...
    final DateTimeFormatter dateFormat = builder.toFormatter();
    ...
    final LocalDateDoubleTimeSeries compareDataPoints = ImmutableLocalDateDoubleTimeSeries.of(dates, values);

    // Set up the reader to read from file and the writer to write to the in-memory master, and do the import
    final InputStream fileStream = new BufferedInputStream(new FileInputStream(FILENAME));
    final TimeSeriesReader reader = new SingleSheetMultiTimeSeriesReader(
  ...
  private SheetReader buildMockSheetReader(final LocalDateDoubleTimeSeries lddts) {
    ...
    final SheetReader mock = mock(SheetReader.class);
    OngoingStubbing<Map<String, String>> stub = when(mock.loadNextRow());
    for (final Map.Entry<LocalDate, Double> entry : lddts) {
  ...
  private HistoricalTimeSeriesMaster buildHistoricalTimeSeriesMaster(final LocalDateDoubleTimeSeries lddts) {
    // Create HTS info and infoDoc
    final ManageableHistoricalTimeSeriesInfo mHtsInfo = new ManageableHistoricalTimeSeriesInfo();
