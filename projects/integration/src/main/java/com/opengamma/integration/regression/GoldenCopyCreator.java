...
public class GoldenCopyCreator {
  ...
  public GoldenCopyCreator(final ToolContext toolContext) {
   ...
   */
  public GoldenCopy run(final String viewName, final String snapshotName, final String version) {

    final Instant valuationTime = getValuationTime(snapshotName);

    final ViewRunner viewRunner = new ViewRunner(getToolContext().getConfigMaster(),
  ...
  private Instant getValuationTime(final String snapshotName) {
    final MarketDataSnapshotMaster snapshotMaster = getToolContext().getMarketDataSnapshotMaster();
