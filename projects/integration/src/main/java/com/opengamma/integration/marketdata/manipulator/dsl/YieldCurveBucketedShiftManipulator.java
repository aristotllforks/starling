...
public final class YieldCurveBucketedShiftManipulator implements ImmutableBean, StructureManipulator<YieldCurve> {
  ...
  public YieldCurveBucketedShiftManipulator(final ScenarioShiftType shiftType, final List<YieldCurveBucketedShift> shifts) {
    _shiftType = shiftType;
    _shifts = ImmutableList.copyOf(ArgumentChecker.notEmpty(shifts, "shifts"));
  }

  @Override
  public YieldCurve execute(final YieldCurve curve, final ValueSpecification valueSpec, final FunctionExecutionContext executionContext) {
    final List<DoublesPair> buckets = new ArrayList<>();
    final List<Double> shifts = new ArrayList<>();
    final ZonedDateTime valuationTime = ZonedDateTime.now(executionContext.getValuationClock());
    final ShiftType shiftType = _shiftType.toAnalyticsType();

    for (final YieldCurveBucketedShift shift : _shifts) {
      final double start = TimeCalculator.getTimeBetween(valuationTime, valuationTime.plus(shift.getStart()));
      final double end = TimeCalculator.getTimeBetween(valuationTime, valuationTime.plus(shift.getEnd()));
