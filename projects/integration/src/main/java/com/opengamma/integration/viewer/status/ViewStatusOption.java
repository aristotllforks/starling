...
public final class ViewStatusOption {
  ...
  private void validateFormat(final String formatOption) {
    final String option = formatOption.toLowerCase();
    if (!SUPPORTED_FORMAT.contains(option)) {
      throw new OpenGammaRuntimeException("Unsupported format type: " + option);
    }
  }
  ...
  public static Options createOptions() {
    ...
    final Option portfolioNameOption = new Option(PORTFOLIO_NAME_OPT, "name", true, "the name of the source OpenGamma portfolio");
    portfolioNameOption.setArgName("portfolioName");
    portfolioNameOption.setRequired(true);

    final Option userOption = new Option(USER_OPT, "user", true, "the username/ipaddress for computing views");
    userOption.setArgName("username/ipaddress");

    final Option formatTypeOption = new Option(FORMAT_TYPE_OPT, "format", true, "the format of status result, default is html");
    formatTypeOption.setArgName("csv, xml, html");

    final Option aggregationTypeOption = new Option(AGGREGATION_TYPE_OPT, "aggregate", true, "the aggregation type of result, default is no-aggregation");
    aggregationTypeOption.setArgName("TSVC, CSVT");

    final Option outputOption = new Option(OUTPUT_OPT, "output", true, "the output filename");
    outputOption.setArgName("filePath");

    final Option liveMarketDataOption = new Option(LIVE_MARKET_DATA_OPT, "live", true, "the live marketdata datasource");
    liveMarketDataOption.setArgName("datasource");

    final Option userMarketDataOption = new Option(USER_MARKET_DATA_OPT, "snapshot", true, "the user marketdata snapshot name");
    userMarketDataOption.setArgName("snapshot name");

    final Option historicalMarketDataOption = new Option(HISTORICAL_MARKET_DATA_OPT, "historical", true, "the historical marketdata specification");
  ...
  public static ViewStatusOption getViewStatusReporterOption(final CommandLine commandLine, final ToolContext toolContext) {
    ...
    final String portfolioName = trimToNull(commandLine.getOptionValue(PORTFOLIO_NAME_OPT));
    final String userOption = trimToNull(commandLine.getOptionValue(USER_OPT));
    UserPrincipal user = null;
    if (userOption == null) {
      user = DEFAULT_USER;
    } else {
      final Matcher matcher = USER_OR_HISTORICAL_PATTERN.matcher(userOption);
      if (matcher.matches()) {
        final String username = matcher.group(1);
        final String ipaddress = matcher.group(2);
    ...
    final String format = defaultString(trimToNull(commandLine.getOptionValue(FORMAT_TYPE_OPT)), DEFAULT_FORMAT);
    final String aggregationOption = trimToNull(commandLine.getOptionValue(AGGREGATION_TYPE_OPT));
    ...
    final String outputOption = trimToNull(commandLine.getOptionValue(OUTPUT_OPT));
    ...
    final MarketDataSpecification marketDataSpec = getMarketDataSpecification(commandLine, toolContext);
  ...
  private static MarketDataSpecification getMarketDataSpecification(final CommandLine commandLine, final ToolContext toolContext) {
    final String marketDataOption = trimToNull(commandLine.getOptionValue(LIVE_MARKET_DATA_OPT));
    if (marketDataOption != null) {
      return LiveMarketDataSpecification.of(marketDataOption);
    }
    final String snapshotOption = trimToNull(commandLine.getOptionValue(USER_MARKET_DATA_OPT));
    if (snapshotOption != null) {
      final MarketDataSnapshotMaster snapshotMaster = toolContext.getMarketDataSnapshotMaster();
      ...
      final MarketDataSnapshotSearchResult searchResult = snapshotMaster.search(request);
    ...
    final String historicalOption = trimToNull(commandLine.getOptionValue(HISTORICAL_MARKET_DATA_OPT));
    if (historicalOption != null) {
      final Matcher matcher = USER_OR_HISTORICAL_PATTERN.matcher(historicalOption);
      if (matcher.matches()) {
        final String localDateStr = matcher.group(1);
        final String htsKey = matcher.group(2);
