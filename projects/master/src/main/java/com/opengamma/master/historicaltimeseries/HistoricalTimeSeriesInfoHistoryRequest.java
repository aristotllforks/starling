 ...
 * <p>
 * A full time-series master implements historical storage of data. History can
 ...
 * version is stored in such a manner that previous versions can be accessed.
 * <p>
 * The second historic dimension is corrections. A correction occurs when it is
 * realized that the original data stored was incorrect. A simple historical
 * time series master might simply replace the original version with the
 * corrected value. A full implementation will store the correction in such a
 * manner that it is still possible to obtain the value before the correction
 * was made.
 * <p>
 * For example, time-series information added on Monday and updated on Thursday
 * has two versions. If it is realized on Friday that the version stored on
 * Monday was incorrect, then a correction may be applied. There are now two
 * versions, the first of which has one correction. This may continue, with
 * multiple corrections allowed for each version.
 * <p>
...
public class HistoricalTimeSeriesInfoHistoryRequest extends AbstractHistoryRequest {
   ...
   */
  public HistoricalTimeSeriesInfoHistoryRequest(final ObjectIdentifiable objectId, final Instant versionInstant, final Instant correctedToInstant) {
