...
public class BloombergDataUtilsTest {
  ...
  public void optionChain() throws Exception {
    ...
    final Set<ExternalId> optionChain = BloombergDataUtils.getOptionChain(rdp, "FIRST US Equity");
  ...
  public void addTwoDigitYearCode() throws Exception {
    ...
    for (final ExternalIdWithDates identifierWithDates : identifiers) {
  ...
  public void parseIdentifiers() throws Exception {
    ...
    final ExternalIdBundleWithDates expected = new ExternalIdBundleWithDates(identifiers);

    final MutableFudgeMsg message = new FudgeContext().newMessage();
    ...
    final ExternalIdBundleWithDates actual = BloombergDataUtils.parseIdentifiers(message, FIELD_FUT_FIRST_TRADE_DT, FIELD_FUT_LAST_TRADE_DT);
  ...
  public void getBUID() throws Exception {
    ...
    final Map<String, String> buids = BloombergDataUtils.getBUID(rdp, Sets.newHashSet("FIRST US Equity", "SECOND US Equity"));
    assertNotNull(buids);
  ...
  private static String multiStringLine(final String... lines) {
