...
public class ResolvingPortfolioLoaderTool extends AbstractTool<IntegrationToolContext> {
  ...
  protected void doRun() {
    final IntegrationToolContext context = getToolContext();
  ...
  private static PositionWriter constructPortfolioWriter(final String portfolioName, final PortfolioMaster portfolioMaster,
      final PositionMaster positionMaster, final SecurityMaster securityMaster, final boolean write, final boolean overwrite) {
    if (write) {
      if (overwrite) {
  ...
  protected Options createOptions(final boolean contextProvided) {
    ...
    final Option overwriteOption = new Option(
    ...
    final Option dateFormatter = new Option(
