...
public class RandomTimeSeriesLoader extends AbstractHistoricalTimeSeriesLoader {
  ...
  protected HistoricalTimeSeriesLoaderResult doBulkLoad(final HistoricalTimeSeriesLoaderRequest request) {
    ...
    final Set<ExternalId> missingIdentifiers = Sets.newHashSet();
    for (final ExternalId identifier : externalIds) {
      if (!haveTimeSeries(identifier, dataProvider, dataField)) {
        missingIdentifiers.add(identifier);
      }
    }
    final Map<ExternalId, UniqueId> resultMap = addToMaster(missingIdentifiers, dataProvider, dataField, startDate, endDate);
  ...
  private Map<ExternalId, UniqueId> addToMaster(
    ...
    for (final ExternalId identifier : missingIdentifiers) {
      ...
      // add the series
      final LocalDateDoubleTimeSeries timeSeries = generateRandomHistoricalTimeSeries(startDate, endDate);
      final UniqueId uid = _htsMaster.updateTimeSeriesDataPoints(addedDoc.getInfo().getTimeSeriesObjectId(), timeSeries);
      result.put(identifier, uid);
    }
    return result;
  }

  private void checkValidDateRange(final LocalDate startDate, final LocalDate endDate) {
    if (endDate.isBefore(startDate)) {
      throw new IllegalArgumentException(endDate + " is before " + startDate);
    }
  }

  private LocalDateDoubleTimeSeries generateRandomHistoricalTimeSeries(final LocalDate startDate, final LocalDate endDate) {
    final int daysBetween = DateUtils.getDaysBetween(startDate, endDate);
  ...
  private boolean haveTimeSeries(final ExternalId identifier, final String dataProvider, final String dataField) {
    final HistoricalTimeSeriesInfoSearchRequest searchRequest = new HistoricalTimeSeriesInfoSearchRequest(identifier);
    ...
    final HistoricalTimeSeriesInfoSearchResult searchTimeSeries = _htsMaster.search(searchRequest);
    final List<HistoricalTimeSeriesInfoDocument> documents = searchTimeSeries.getDocuments();
