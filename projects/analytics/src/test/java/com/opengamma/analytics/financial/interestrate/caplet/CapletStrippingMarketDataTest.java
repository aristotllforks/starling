...
public class CapletStrippingMarketDataTest {
  ...
  private static final double[] INDEX_CURVE_NODES = new double[] { 0.0438356164383561, 0.0876712328767123, 0.172602739726027,
      0.254794520547945, 0.506849315068493, 0.758904109589041, 1.00547945205479,
      2.01369863013698, 3.01020285949547, 4.00547945205479, 5.00547945205479, 6.00547945205479, 7.01839958080694, 8.01095890410959,
      9.00821917808219, 10.0082191780821, 15.0074706190583,
      20.0082191780821, 25.0109589041095, 30.0136986301369 };
  private static final double[] INDEX_CURVE_VALUES = new double[] { 0.00184088091044285, 0.00201024117395892, 0.00241264832694067,
  ...
  private static final double[] DIS_CURVE_VALUES = new double[] { 0.00212916045658802, 0.00144265912946933, 0.00144567477491987,
      ...
      0.00127592397233014, 0.00132302501180961, 0.00138688847322639, 0.00172748279241698, 0.00254381216780551, 0.00410024606039574,
  ...
  private static final double[][] CAP_IMPLIED_VOL = new double[][] { { 0.7311 }, // ,
      // 0.9965,
      // 0.9769,
      // 1.02785,
      // 0.9133,
      ...
      // 1.04179,
      // 1.05794},
      { 0.7941 }, // , 0.8154, 0.78869, 0.8298, 0.80356, 0.8469, 0.81564,
                  // 0.8545, 0.82533, 0.8414, 0.8571, 0.8575},
      { 0.8528, 0.71025, 0.63236, 0.612, 0.58151, 0.5658, 0.56404, 0.5388, 0.55844, 0.528, 0.5259, 0.5091, 0.5159, 0.5021, 0.5091, 0.5113,
          0.4939, 0.4918 },
      { 0.7356, 0.554, 0.47497, 0.4489, 0.39571, 0.3883, 0.36043, 0.3589, 0.34799, 0.3403, 0.3317, 0.3212, 0.317, 0.3148, 0.3189, 0.3346,
          0.3332, 0.3322 } };
  ...
  private static final int N_CAPS;
  private static final List<CapFloor> CAPS;
  private static final double[] MARKET_VOLS;
  private static final double[] SIGMA;

  private static final String[] NAMES = new String[] { "alpha", "beta", "rho", "nu" };
  ...
  private static final LinkedHashMap<String, double[]> CURVE_NODES;
  private static final LinkedHashMap<String, Interpolator1D> INTERPOLATORS;
  private static final LinkedHashMap<String, ParameterLimitsTransform> TRANSFORMS;
  ...
  static {
    ...
    // single curve for discount and projection (this is consistent with
    // Bloomberg's cap quotes)
    ...
    TRANSFORMS.put(NAMES[0], new SingleRangeLimitTransform(0.0, LimitType.GREATER_THAN)); // alpha
    // >
    // 0
    ...
    // TRANSFORMS.put(NAMES[3], new SingleRangeLimitTransform(0.0,
    // LimitType.GREATER_THAN));
    ...
    for (final String name : NAMES) {
      if (name == "beta") {
        ...
        final Interpolator1D flat = NamedInterpolator1dFactory.of(LinearInterpolator1dAdapter.NAME, FlatExtrapolator1dAdapter.NAME);
  ...
  public void testFwd() {
    ...
    // VolatilityModel1D volModel = new
    // SABRTermStructureParameters(ConstantDoublesCurve.from(0.3),
    // ConstantDoublesCurve.from(1.0), ConstantDoublesCurve.from(-0.5),
    // ConstantDoublesCurve.from(0.5));
  ...
  public void testStripping() {
    ...
    // start from some realistic values, and transform these into the fitting
    // parameters
