...
public class CouponIborCompoundingDiscountingMethodTest {
  ...
  private static final CouponIborCompoundingDefinition CPN_DEFINITION = CouponIborCompoundingDefinition.from(NOTIONAL, START_DATE, M6,
      CADCDOR3M, CalendarAdapter.of(CALENDAR));
  ...
  private static final DoubleTimeSeries<ZonedDateTime> FIXING_TS = ImmutableZonedDateTimeDoubleTimeSeries
      .ofUTC(new ZonedDateTime[] { DateUtils.getUTCDate(2012, 8, 23), DateUtils.getUTCDate(2012, 8, 24),
  ...
  public void presentValueBeforeFirstFixing() {
    ...
    for (int loopsub = 0; loopsub < nbSub; loopsub++) {
      notionalAccrued *= 1.0 + CPN_BEFORE.getPaymentAccrualFactors()[loopsub]
          * MULTICURVES.getSimplyCompoundForwardRate(CPN_BEFORE.getIndex(), CPN_BEFORE.getFixingPeriodStartTimes()[loopsub],
              CPN_BEFORE.getFixingPeriodEndTimes()[loopsub],
  ...
  public void presentValueAfter1Fixing() {
    ...
    accruedNotional *= 1.0 + CPN_1.getPaymentAccrualFactors()[0]
        * MULTICURVES.getSimplyCompoundForwardRate(CPN_1.getIndex(), CPN_1.getFixingPeriodStartTimes()[0],
            CPN_1.getFixingPeriodEndTimes()[0], CPN_1.getFixingPeriodAccrualFactors()[0]);
