...
public class CapFloorInflationZeroCouponInterpolationBlackSmileMethodTest {
  ...
  private static final WorkingDayCalendar CALENDAR_USD = MulticurveProviderDiscountDataSets.getUSDCalendar();
  ...
  private static final CouponInflationZeroCouponInterpolationDefinition ZERO_COUPON_DEFINITION = CouponInflationZeroCouponInterpolationDefinition
      .from(START_DATE, PAYMENT_DATE, NOTIONAL,
          PRICE_INDEX_EUR, MONTH_LAG, MONTH_LAG, false);

  private static final ZonedDateTime PRICING_DATE = DateUtils.getUTCDate(2011, 8, 3);
  private static final CapFloorInflationZeroCouponInterpolationDefinition ZERO_COUPON_DEFINITION_CAP = CapFloorInflationZeroCouponInterpolationDefinition
  ...
  private static final ZonedDateTimeDoubleTimeSeries TS_PRICE_INDEX_USD = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(
      ...
      new double[] { 108.23, 108.64, 200, 200 });
  private static final CapFloorInflationZeroCouponInterpolation ZERO_COUPON_CAP = (CapFloorInflationZeroCouponInterpolation) ZERO_COUPON_DEFINITION_CAP
  ...
  public void presentValue() {
    ...
    final double volatility = BLACK_INFLATION.getBlackParameters().getVolatility(ZERO_COUPON_CAP.getReferenceEndTime()[1],
  ...
  public void presentValueSeasonality() {
    ...
    final BlackSmileCapInflationZeroCouponProviderDiscount blackInflation = new BlackSmileCapInflationZeroCouponProviderDiscount(
        marketSeason.getInflationProvider(), BLACK_PARAM);
    ...
    final ZonedDateTime paymentDate = ScheduleCalculator.getAdjustedDate(settleDate, Period.ofYears(tenorYear), BUSINESS_DAY, CALENDAR_USD,
    ...
    final CouponInflationZeroCouponInterpolationDefinition zeroCouponUsdDefinition = CouponInflationZeroCouponInterpolationDefinition.from(
        settleDate, paymentDate, notional, PRICE_INDEX_US,
        MONTH_LAG, MONTH_LAG, false);
    final CapFloorInflationZeroCouponInterpolationDefinition capZeroCouponUsdDefinition = CapFloorInflationZeroCouponInterpolationDefinition
        .from(zeroCouponUsdDefinition,
            LAST_KNOWN_FIXING_DATE, MATURITY, STRIKE, IS_CAP);

    final ZonedDateTimeDoubleTimeSeries ts = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(
        new ZonedDateTime[] { DateUtils.getUTCDate(2008, 4, 30), DateUtils.getUTCDate(2008, 5, 31), DateUtils.getUTCDate(2011, 5, 31),
            DateUtils.getUTCDate(2011, 6, 30),
            DateUtils.getUTCDate(2011, 9, 27),
            DateUtils.getUTCDate(2011, 9, 28) },
        new double[] { 108.23, 108.64, 225.964, 225.722, 200, 200 });

    final CapFloorInflationZeroCouponInterpolation capZeroCouponUsd = (CapFloorInflationZeroCouponInterpolation) capZeroCouponUsdDefinition
        .toDerivative(PRICING_DATE, ts);
    final CouponInflationZeroCouponInterpolation zeroCouponUsd = (CouponInflationZeroCouponInterpolation) zeroCouponUsdDefinition
        .toDerivative(PRICING_DATE, ts);
    ...
    final EuropeanVanillaOption option = new EuropeanVanillaOption(
        Math.pow(1 + capZeroCouponUsd.getStrike(), capZeroCouponUsd.getMaturity()), timeToMaturity, capZeroCouponUsd.isCap());
    final double volatility = blackInflation.getBlackParameters().getVolatility(capZeroCouponUsd.getReferenceEndTime()[1],
        capZeroCouponUsd.getStrike());
    final BlackFunctionData dataBlack = new BlackFunctionData(forward, 1.0, volatility);
