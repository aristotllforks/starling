...
public class STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethodTest {
  ...
  private static final InterestRateFutureOptionMarginSecurityDefinition CALL_ERZ4_099_DEFINITION = new InterestRateFutureOptionMarginSecurityDefinition(
      ERZ4_DEFINITION, EXPIRY_DATE, STRIKE_099, IS_CALL);
  private static final InterestRateFutureOptionMarginSecurityDefinition PUT_ERZ4_099_DEFINITION = new InterestRateFutureOptionMarginSecurityDefinition(
      ERZ4_DEFINITION, EXPIRY_DATE, STRIKE_099, !IS_CALL);
  ...
  final private static BlackSTIRFuturesExpLogMoneynessProvider MULTICURVE_BLACK = new BlackSTIRFuturesExpLogMoneynessProvider(MULTICURVE,
      BLACK_SURFACE_LOGMONEY, EURIBOR3M);
  ...
  private static final InterestRateFutureSecurityDiscountingMethod METHOD_FUTURE = InterestRateFutureSecurityDiscountingMethod
      .getInstance();
  private static final InterestRateFutureOptionMarginSecurityBlackSTIRFuturesMethod METHOD_OPT = InterestRateFutureOptionMarginSecurityBlackSTIRFuturesMethod
      .getInstance();
  ...
  public void futurePrice() {
    ...
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: underlying futures price", priceExpected, priceComputed,
        TOLERANCE_RATE);
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: underlying futures price", priceExpected, priceComputed2,
        TOLERANCE_RATE);
  }
  ...
  public void priceBlackSensitivity() {
    ...
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: Black parameters sensitivity", vega,
        vegaComputed.getSensitivity().toSingleValue(), TOLERANCE_DELTA);
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: Black parameters sensitivity", 1,
    ...
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: Black parameters sensitivity",
        CALL_ERZ4_099.getExpirationTime(), point.getKey().getFirst(), TOLERANCE_RATE);
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: Black parameters sensitivity",
        CALL_ERZ4_099.getUnderlyingFuture().getTradingLastTime() - CALL_ERZ4_099.getExpirationTime(), point.getKey().getSecond(),
        TOLERANCE_RATE);
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: Black parameters sensitivity", CALL_ERZ4_099.getStrike(),
  ...
  public void theoreticalDelta() {
    ...
    assertEquals("STIRFuturesOptionMarginSecurityBlackExpLogMoneynessMethod: delta", deltaCallExpected - 1.0d, deltaPutComputed,
        TOLERANCE_DELTA);
  }
  ...
  public void theoreticalTheta() {
    ...
    final double expiry = CALL_ERZ4_099.getExpirationTime();
    final double volatility = BLACK_SURFACE_LOGMONEY.getZValue(expiry, logmoney);
    final double rate = -Math
        .log(MULTICURVE.getMulticurveProvider().getDiscountFactor(CALL_ERZ4_099.getCurrency(), CALL_ERZ4_099.getExpirationTime()))
        / CALL_ERZ4_099.getExpirationTime();
    final double thetaCallExpected = BlackFormulaRepository.theta(rateFutures, rateStrike, CALL_ERZ4_099.getExpirationTime(), volatility,
        !CALL_ERZ4_099.isCall(), rate);
