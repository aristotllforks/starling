...
/**
 * Tests related to the calibration engine for LMM DD calibration at best to European swaptions. The calibration is obtained by changing
 * volatility parameters with a common multiplicative factor and displacement with a common additive term.
 */
...
public class SuccessiveLeastSquareSwaptionPhysicalLMMDDCalibrationObjectiveTest {
  ...
  private static final ZonedDateTime EXPIRY_DATE = ScheduleCalculator.getAdjustedDate(REFERENCE_DATE, Period.ofYears(EXPIRY_TENOR),
      EURIBOR6M, TARGET);
  ...
  // {-0.01, 0.0, 0.01}, {-0.0100, -0.0025, 0.0025, 0.0100}, {-0.0100, -0.0025, 0.00, 0.0025, 0.0100}, {-0.0150, -0.0075, -0.0025, 0.0025,
  // 0.0075, 0.0150}
  ...
  static {
    ...
    SWAP_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, Period.ofYears(SWAP_TENOR), EUR1YEURIBOR6M, NOTIONAL, RATE,
        FIXED_IS_PAYER);
    ...
    for (int loopexp = 0; loopexp < SWAP_TENOR; loopexp++) {
      ...
      cpnIbor[2 * loopexp + 1] = ((CouponIborDefinition) legIbor.getNthPayment(2 * loopexp + 1))
          .withNotional(((CouponIborDefinition) legIbor.getNthPayment(2 * loopexp + 1)).getNotional()
  ...
  private static final LiborMarketModelDisplacedDiffusionParameters LMM_PARAM_INIT = TestsDataSetLiborMarketModelDisplacedDiffusion
      .createLMMParameters(REFERENCE_DATE, SWAP_DEFINITION.getIborLeg());
  private static final SwaptionPhysicalFixedIborSABRLMMLeastSquareMethod METHOD_CALIBRATION = new SwaptionPhysicalFixedIborSABRLMMLeastSquareMethod(
      MONEYNESS, LMM_PARAM_INIT);
  ...
  public void calibration() {
    ...
    final CalibrationEngineWithCalculators<SABRSwaptionProviderInterface> calibrationEngine = new SuccessiveLeastSquareLMMDDCalibrationEngine<>(
        objective, NB_STRIKE);
    ...
    for (int loopexp = 0; loopexp < SWAP_TENOR_YEAR.length; loopexp++) {
      for (int loopstrike = 0; loopstrike < NB_STRIKE; loopstrike++) {
        ...
        pvLmm[loopexp][loopstrike] = METHOD_LMM.presentValue(swaptionCalibration[loopexp * NB_STRIKE + loopstrike],
            objective.getLMMProvider());
