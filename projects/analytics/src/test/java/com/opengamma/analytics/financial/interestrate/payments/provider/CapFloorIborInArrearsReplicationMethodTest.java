...
public class CapFloorIborInArrearsReplicationMethodTest {
  ...
  private static final CapFloorIborDefinition CAP_IA_LONG_DEFINITION = new CapFloorIborDefinition(EUR, END_ACCRUAL_DATE, START_ACCRUAL_DATE,
      END_ACCRUAL_DATE, ACCRUAL_FACTOR, NOTIONAL, FIXING_DATE,
      EURIBOR6M, STRIKE, IS_CAP, CalendarAdapter.of(CALENDAR));
  private static final CouponIborDefinition COUPON_IBOR_IA_DEFINITION = new CouponIborDefinition(EUR, END_ACCRUAL_DATE, START_ACCRUAL_DATE,
      END_ACCRUAL_DATE, ACCRUAL_FACTOR, NOTIONAL, FIXING_DATE,
  ...
  private static final CapFloorIborDefinition CAP_IA_SHORT_DEFINITION = new CapFloorIborDefinition(EUR, END_ACCRUAL_DATE,
      START_ACCRUAL_DATE, END_ACCRUAL_DATE, ACCRUAL_FACTOR, -NOTIONAL, FIXING_DATE,
      EURIBOR6M, STRIKE, IS_CAP, CalendarAdapter.of(CALENDAR));
  private static final CapFloorIborDefinition FLOOR_IA_SHORT_DEFINITION = new CapFloorIborDefinition(EUR, END_ACCRUAL_DATE,
      START_ACCRUAL_DATE, END_ACCRUAL_DATE, ACCRUAL_FACTOR, -NOTIONAL,
      FIXING_DATE, EURIBOR6M, STRIKE, !IS_CAP, CalendarAdapter.of(CALENDAR));
  // To derivative
  ...
  private static final CapFloorIborSABRCapExtrapolationRightMethod METHOD_SABREXTRA_STD = new CapFloorIborSABRCapExtrapolationRightMethod(
      CUT_OFF_STRIKE, MU);
  private static final CouponIborInArrearsReplicationMethod METHOD_SABREXTRA_COUPON_IA = new CouponIborInArrearsReplicationMethod(
      METHOD_SABREXTRA_STD);
  private static final CapFloorIborInArrearsSABRCapGenericReplicationMethod METHOD_SABREXTRA_CAP_IA = new CapFloorIborInArrearsSABRCapGenericReplicationMethod(
      METHOD_SABREXTRA_STD);
  ...
  /**
   * Tests the present value using the SABR with extrapolation to the right method to price standard cap/floor. It is suggested not to use
   * the standard SABR method as it can lead to exploding prices for long term contracts.
   */
  @Test
  public void persentValueSABRExtrapolation() {
    final CapFloorIbor capStandard = new CapFloorIbor(EUR, CAP_LONG.getFixingPeriodEndTime(), CAP_LONG.getPaymentYearFraction(), NOTIONAL,
        CAP_LONG.getFixingTime(), EURIBOR6M,
    ...
    final double forward = MULTICURVES.getSimplyCompoundForwardRate(CAP_LONG.getIndex(), CAP_LONG.getFixingPeriodStartTime(),
        CAP_LONG.getFixingPeriodEndTime(), CAP_LONG.getFixingAccrualFactor());
    final double beta = (1.0 + CAP_LONG.getFixingAccrualFactor() * forward)
        * MULTICURVES.getDiscountFactor(EUR, CAP_LONG.getFixingPeriodEndTime())
  ...
  public void presentValueSABRNoAdjustment() {
    final double forward = MULTICURVES.getSimplyCompoundForwardRate(EURIBOR6M, CAP_LONG.getFixingPeriodStartTime(),
        CAP_LONG.getFixingPeriodEndTime(), CAP_LONG.getFixingAccrualFactor());
    ...
    assertTrue("Coupon IA - SABR pricing: coupon = cap with strike eps",
        priceIbor.getAmount(EUR) > forward * NOTIONAL * CAP_LONG.getPaymentYearFraction()
            * MULTICURVES.getDiscountFactor(EUR, CAP_LONG.getPaymentTime()));
  }
  ...
  public void persentValueSABRExtrapolationParity() {
    ...
    assertEquals("Cap/floor IA - SABR pricing: long-short parity", priceCapLong.getAmount(EUR), -priceCapShort.getAmount(EUR),
        TOLERANCE_PV);
    final MultipleCurrencyAmount priceIbor = METHOD_SABREXTRA_COUPON_IA.presentValue(COUPON_IBOR, SABR_MULTICURVES);
    final CapFloorIborDefinition cap0Definition = new CapFloorIborDefinition(EUR, END_ACCRUAL_DATE, START_ACCRUAL_DATE, END_ACCRUAL_DATE,
        ACCRUAL_FACTOR, NOTIONAL, FIXING_DATE, EURIBOR6M, 0.0, IS_CAP, CalendarAdapter.of(CALENDAR));
    ...
    assertEquals("Cap/floor IA - SABR pricing: cap/floor parity", priceIbor.getAmount(EUR) - priceStrike.getAmount(EUR),
        priceCapLong.getAmount(EUR) + priceFloorShort.getAmount(EUR), 5.0E+4);
  ...
  public void sabrExtrapolationPerformance() {
    ...
    for (int looptest = 0; looptest < nbTest; looptest++) {
      final CapFloorIborSABRCapExtrapolationRightMethod methodSABRExtraStd = new CapFloorIborSABRCapExtrapolationRightMethod(CUT_OFF_STRIKE,
          MU); // To start with a "clean" method
      final CapFloorIborInArrearsSABRCapGenericReplicationMethod methodSABRExtraIA = new CapFloorIborInArrearsSABRCapGenericReplicationMethod(
          methodSABRExtraStd);
  ...
  private static class InArrearsIntegrant extends Function1D<Double, Double> {
     ...
     *          The SABR data bundle used in the standard cap/floor pricing.
     */
    public InArrearsIntegrant(final CapFloorIborSABRCapMethodInterface baseMethod, final CapFloorIbor capStandard,
        final SABRCapProviderInterface sabr) {
