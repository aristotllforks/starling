...
public class SwaptionPhysicalFixedIborSABRMethodE2ETest {
  ...
  private static final WorkingDayCalendar NYC = StandardDataSetsMulticurveUSD.calendarArray()[0];
  ...
  private static final SABRSwaptionProviderDiscount MULTICURVE_SABR = new SABRSwaptionProviderDiscount(MULTICURVE, SABR_PARAMETER,
      USD6MLIBOR3M);
  ...
  private static final SwapFixedIborDefinition SWAP_PAYER_DEFINITION = USD6MLIBOR3M.generateInstrument(EXPIRY_DATE, FIXED_RATE_3M, NOTIONAL,
      ATTRIBUTE_3M);
  private static final SwaptionPhysicalFixedIborDefinition SWAPTION_P_2YX7Y_DEFINITION = SwaptionPhysicalFixedIborDefinition
      .from(EXPIRY_DATE, SWAP_PAYER_DEFINITION, true, true);
  ...
  private static final PresentValueSABRSensitivitySABRSwaptionCalculator PVSSSSC = PresentValueSABRSensitivitySABRSwaptionCalculator
      .getInstance();
  private static final ParameterSensitivityParameterCalculator<SABRSwaptionProviderInterface> PSC = new ParameterSensitivityParameterCalculator<>(
      PVCSSSC);
  private static final MarketQuoteSensitivityBlockCalculator<SABRSwaptionProviderInterface> MQSBC = new MarketQuoteSensitivityBlockCalculator<>(
      PSC);
  ...
  public void presentValue() {
    ...
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: present value from standard curves", pvExpected.getAmount(USD),
  ...
  /**
   * Test Bucketed PV01 with a standard set of data against hard-coded standard values for a swap fixed vs LIBOR3M. Can be used for platform
  ...
  public void pv01() {
    ...
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: pv01 from standard curves", pv01dsc,
        pv01Computed.getMap().get(Pairs.of(MULTICURVE.getName(USD), USD)), TOLERANCE_RATE);
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: pv01 from standard curves", pv01fwd,
        pv01Computed.getMap().get(Pairs.of(MULTICURVE.getName(USDLIBOR3M), USD)), TOLERANCE_RATE);
  }

  /**
   * Test Bucketed PV01 with a standard set of data against hard-coded standard values for a swaption physical fixed vs LIBOR3M. Can be used
   * for platform testing or regression testing.
   */
  public void BucketedPV01() {
    final double[] deltaDsc = { -0.8970521909327039, -0.8970528138871251, 2.0679726864123788E-5, -2.800077859468568E-4,
        0.020545355340195248, -28.660344224880443, 1.0311659235333974,
    ...
    final double[] deltaFwd3 = { 0.6768377111533482, -0.013861472263779616, -0.00815248053117034, 28.045784074714817, -10296.86232676286,
        -9.445439010985615, -12.048126446934697,
    ...
    final MultipleCurrencyParameterSensitivity pvpsComputed = MQSBC.fromInstrument(SWAPTION_P_2YX7Y, MULTICURVE_SABR, BLOCK)
        .multipliedBy(BP1);
    AssertSensitivityObjects.assertEquals("SwaptionPhysicalFixedIborSABRMethod: bucketed deltas from standard curves", pvpsExpected,
  ...
  public void bucketedSABRRisk() {
    ...
    final PresentValueSABRSensitivityDataBundle pvssNodeComputed = SABRSensitivityNodeCalculator.calculateNodeSensitivities(pvssComputed,
        SABR_PARAMETER);
    final Map<DoublesPair, Double> alphaRiskExpected = new HashMap<>();
    ...
    final Map<DoublesPair, Double> alphaRiskComputed = pvssNodeComputed.getAlpha().getMap();
    for (final Entry<DoublesPair, Double> entry : alphaRiskExpected.entrySet()) {
    ...
    final Map<DoublesPair, Double> betaRiskExpected = new HashMap<>();
    ...
    final Map<DoublesPair, Double> betaRiskComputed = pvssNodeComputed.getBeta().getMap();
    for (final Entry<DoublesPair, Double> entry : betaRiskExpected.entrySet()) {
      assertEquals("SwaptionPhysicalFixedIborSABRMethod: BucketedSABRRisk from standard curves " + entry.getKey(),
          betaRiskComputed.get(entry.getKey()), entry.getValue(), TOLERANCE_PV_DELTA);
    }
    final Map<DoublesPair, Double> rhoRiskExpected = new HashMap<>();
    ...
    final Map<DoublesPair, Double> rhoRiskComputed = pvssNodeComputed.getRho().getMap();
    for (final Entry<DoublesPair, Double> entry : rhoRiskExpected.entrySet()) {
      assertEquals("SwaptionPhysicalFixedIborSABRMethod: BucketedSABRRisk from standard curves " + entry.getKey(),
          rhoRiskComputed.get(entry.getKey()), entry.getValue(), TOLERANCE_PV_DELTA);
    }
    final Map<DoublesPair, Double> nuRiskExpected = new HashMap<>();
    ...
    final Map<DoublesPair, Double> nuRiskComputed = pvssNodeComputed.getNu().getMap();
    for (final Entry<DoublesPair, Double> entry : nuRiskExpected.entrySet()) {
      assertEquals("SwaptionPhysicalFixedIborSABRMethod: BucketedSABRRisk from standard curves " + entry.getKey(),
