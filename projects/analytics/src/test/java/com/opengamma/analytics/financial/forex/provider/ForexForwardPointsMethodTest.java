...
public class ForexForwardPointsMethodTest {
  ...
  private static final ZonedDateTime SPOT_DATE = ScheduleCalculator.getAdjustedDate(REFERENCE_DATE, 2, CalendarAdapter.of(CALENDAR));
  ...
  private static final ForexDefinition FX_INDIRECT_DEFINITION = new ForexDefinition(CUR_2, CUR_1, PAYMENT_DATE, -NOMINAL_1 * FX_RATE,
      1.0d / FX_RATE);
  ...
  static {
    for (int i = 0; i < NB_MARKET_QUOTES; i++) {
      MARKET_QUOTES_FWD[i] = MARKET_QUOTES_PTS[i] + FX_RATE;
    }
  }
  private static final Period[] MARKET_QUOTES_TENOR = new Period[] { Period.ofDays(-2), Period.ofDays(-1), Period.ofMonths(1),
  ...
  static {
    for (int i = 0; i < NB_MARKET_QUOTES; i++) {
      MARKET_QUOTES_DATE[i] = ScheduleCalculator.getAdjustedDate(SPOT_DATE, MARKET_QUOTES_TENOR[i], USDLIBOR3M,
          CalendarAdapter.of(CALENDAR));
      MARKET_QUOTES_TIME[i] = TimeCalculator.getTimeBetween(REFERENCE_DATE, MARKET_QUOTES_DATE[i]);
    }
  }
  ...
  private static final InterpolatedDoublesCurve FWD_RATES = new InterpolatedDoublesCurve(MARKET_QUOTES_TIME, MARKET_QUOTES_FWD, LINEAR_FLAT,
      true);
  private static final MulticurveForwardPointsProviderDiscount MULTICURVES_FWD = new MulticurveForwardPointsProviderDiscount(MULTICURVES,
      FWD_RATES, PAIR);
  ...
  private static final PresentValueCurveSensitivityForexForwardPointsCalculator PVCSFFPC = PresentValueCurveSensitivityForexForwardPointsCalculator
      .getInstance();
  ...
  private static final ParameterSensitivityParameterCalculator<MulticurveForwardPointsProviderInterface> PS_FFP_C = new ParameterSensitivityParameterCalculator<>(
      PVCSFFPC);
  private static final ParameterSensitivityForexForwardPointsDiscountInterpolatedFDCalculator PS_FFP_FDC = new ParameterSensitivityForexForwardPointsDiscountInterpolatedFDCalculator(
      PVFFPC, SHIFT);
  ...
  public void presentValueOppositeOrder() {
    ...
    assertEquals("ForexForwardPointsMethod: presentValue", pvComputedIndirect.getAmount(CUR_2), pvComputedDirect.getAmount(CUR_2),
        TOLERANCE_PV);
  ...
  public void currencyExposure() {
    ...
    final MultipleCurrencyAmount pvComputed = METHOD_FX_PTS.presentValue(FX, MULTICURVES, FWD_RATES, PAIR);
    assertEquals("ForexForwardPointsMethod: currencyExposure", MULTICURVES.getFxRates().convert(ceComputed, CUR_2).getAmount(),
        pvComputed.getAmount(CUR_2), TOLERANCE_PV);
  }
  ...
  public void presentValueForwardPointsSensitivity() {
    ...
    for (int i = 0; i < NB_MARKET_QUOTES; i++) {
      ...
      fpsExpected[i] = (pvShift.getAmount(CUR_2) - pv.getAmount(CUR_2)) / shift;
      assertEquals("ForexForwardPointsMethod: presentValueForwardPointsSensitivity - node " + i, fpsExpected[i], fpsComputed[i],
          TOLERANCE_PV_DELTA);
    }
  }

  /**
   * Tests the sensitivity of the pv to the curve.
   */
  @Test
  ...
  /**
   * Compares the method and the calculator.
   */
  @Test
  public void presentValueCurveSensitivityMethodVsCalculator() {
    ...
    AssertSensitivityObjects.assertEquals("ForexForwardPointsMethod: presentValueCurveSensitivity", pvcsMethod1, pvcsMethod2,
        TOLERANCE_PV_DELTA);
  }

  /**
   * Compares the method and calculator.
   */
  @Test
  public void presentValueCurveSensitivityIndirectOrderMethodVsCalculator() {
    final MultipleCurrencyMulticurveSensitivity pvcsMethod1 = METHOD_FX_PTS.presentValueCurveSensitivity(FX_INDIRECT, MULTICURVES,
        FWD_RATES, PAIR);
