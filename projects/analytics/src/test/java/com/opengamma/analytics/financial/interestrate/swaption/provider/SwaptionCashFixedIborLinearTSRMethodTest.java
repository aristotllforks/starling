...
public class SwaptionCashFixedIborLinearTSRMethodTest {
  ...
  private static final ZonedDateTime SETTLEMENT_DATE = ScheduleCalculator.getAdjustedDate(EXPIRY_DATE, EURIBOR6M.getSpotLag(),
      CalendarAdapter.of(CALENDAR));
  ...
  public void presentValueLongShortParity() {
    ...
    assertEquals("Cash-settled swaption: linear TSR: present value - long/short parity", -pvLongTSR.getAmount(EUR),
        pvShortTSR.getAmount(EUR), 1E-2);
  }
  ...
  public void presentValueMultiStrike() {
    ...
    for (int i = 0; i < nbStrike + 1; i++) {
      strike[i] = strikeMin + i * (strikeMax - strikeMin) / nbStrike;
      swapDefinition[i] = SwapFixedIborDefinition.from(SETTLEMENT_DATE, CMS_INDEX, NOTIONAL, strike[i], FIXED_IS_PAYER,
          CalendarAdapter.of(CALENDAR));
      swaptionCashDefinition[i] = SwaptionCashFixedIborDefinition.from(EXPIRY_DATE, swapDefinition[i], true, IS_LONG);
      swaptionCash[i] = swaptionCashDefinition[i].toDerivative(REFERENCE_DATE);
      swaptionPhysDefinition[i] = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE, swapDefinition[i], true, IS_LONG);
      swaptionPhys[i] = swaptionPhysDefinition[i].toDerivative(REFERENCE_DATE);
    }
    ...
    for (int i = 0; i < nbStrike + 1; i++) {
      pvCashStandard[i] = METHOD_CASH_SABR.presentValue(swaptionCash[i], SABR_MULTICURVES).getAmount(EUR);
      pvCashTSR[i] = METHOD_CASH_TSR.presentValue(swaptionCash[i], SABR_MULTICURVES).getAmount(EUR);
      pvPhysical[i] = METHOD_PHYS_SABR.presentValue(swaptionPhys[i], SABR_MULTICURVES).getAmount(EUR);
    }
  }
