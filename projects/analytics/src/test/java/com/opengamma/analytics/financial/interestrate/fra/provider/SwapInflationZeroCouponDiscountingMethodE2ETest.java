...
public class SwapInflationZeroCouponDiscountingMethodE2ETest {
  ...
  private static final SwapFixedInflationZeroCouponDefinition SWAP_DEFINITION = GENERATOR_INFLATION_SWAP
      .generateInstrument(STD_ACCRUAL_START_DATE, RATE_FIXED_LEG, STD_NOTIONAL, RPI_GBP_ATTR);
  private static final ZonedDateTimeDoubleTimeSeries TS_PRICE_INDEX_USD_WITH_TODAY = ImmutableZonedDateTimeDoubleTimeSeries
      .ofUTC(new ZonedDateTime[] { DateUtils.getUTCDate(2013, 12, 31),
  ...
  private static InstrumentDerivative convert(final InstrumentDefinition<?> instrument) {
    ...
    if (instrument instanceof SwapFixedInflationZeroCouponDefinition) {
      final Annuity<? extends Payment> ird1 = ((SwapFixedInflationZeroCouponDefinition) instrument).getFirstLeg()
          .toDerivative(STD_REFERENCE_DATE);
      final Annuity<? extends Payment> ird2 = ((SwapFixedInflationZeroCouponDefinition) instrument).getSecondLeg()
          .toDerivative(STD_REFERENCE_DATE, TS_PRICE_INDEX_USD_WITH_TODAY);
