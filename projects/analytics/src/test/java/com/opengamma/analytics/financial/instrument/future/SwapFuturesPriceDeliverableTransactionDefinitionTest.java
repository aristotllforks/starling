...
public class SwapFuturesPriceDeliverableTransactionDefinitionTest {
  ...
  @Test
  public void getter() {
    assertEquals("DeliverableSwapFuturesTransactionDefinition: getter", SWAP_FUTURES_SECURITY_DEFINITION,
        SWAP_FUTURES_TRANSACTION_DEFINITION.getUnderlyingSecurity());
  ...
  public void toDerivativeOnTradeDate() {
    ...
    final SwapFuturesPriceDeliverableTransaction derivativeExpected = new SwapFuturesPriceDeliverableTransaction(underlying, TRAN_PRICE,
        QUANTITY);
    final SwapFuturesPriceDeliverableTransaction derivativeConverted = SWAP_FUTURES_TRANSACTION_DEFINITION.toDerivative(referenceDate,
        lastMargin);
  ...
  public void toDerivativeAfterTradeDate() {
    ...
    final SwapFuturesPriceDeliverableSecurity underlying = SWAP_FUTURES_SECURITY_DEFINITION.toDerivative(referenceDate);
    final SwapFuturesPriceDeliverableTransaction derivativeExpected = new SwapFuturesPriceDeliverableTransaction(underlying, lastMargin,
        QUANTITY);
    final SwapFuturesPriceDeliverableTransaction derivativeConverted = SWAP_FUTURES_TRANSACTION_DEFINITION.toDerivative(referenceDate,
        lastMargin);
    assertEquals("DeliverableSwapFuturesTransactionDefinition: toDerivative", derivativeExpected, derivativeConverted);
