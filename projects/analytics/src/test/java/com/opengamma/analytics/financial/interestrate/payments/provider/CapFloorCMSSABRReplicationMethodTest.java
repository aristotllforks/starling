...
public class CapFloorCMSSABRReplicationMethodTest {
  ...
  private static final SwapFixedIborDefinition SWAP_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR,
      EUR1YEURIBOR6M, 1.0, 0.0, true);
  ...
  private static final CouponCMSDefinition CMS_COUPON_DEFINITION = CouponCMSDefinition.from(PAYMENT_DATE, ACCRUAL_START_DATE,
      ACCRUAL_END_DATE, ACCRUAL_FACTOR, NOTIONAL, FIXING_DATE, SWAP_DEFINITION,
      INDEX_SWAP_5Y);
  ...
  public void testPriceReplication() {
    ...
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", priceCMSCoupon.getAmount(EUR), priceCMSCap0.getAmount(EUR),
        TOLERANCE_PV);
    final MultipleCurrencyAmount priceCMSCap = PVSSC.visit(CMS_CAP, SABR_MULTICURVES);
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", 5224.559, priceCMSCap.getAmount(EUR), TOLERANCE_PV); // From previous
                                                                                                                         // run
    final MultipleCurrencyAmount priceCMSFloor = PVSSC.visit(CMS_FLOOR, SABR_MULTICURVES);
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", 20149.939, priceCMSFloor.getAmount(EUR), TOLERANCE_PV); // From previous
                                                                                                                            // run
    ...
    assertEquals("CapFloorCMSSABRReplicationMethod: present value", priceCMSCap.getAmount(EUR) - priceCMSFloor.getAmount(EUR),
        priceCMSCoupon.getAmount(EUR) - priceStrike.getAmount(EUR), 2.0E+3);
  }
  ...
  public void presentValueCurveSensitivityCap() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SS_C.calculateSensitivity(CMS_CAP, SABR_MULTICURVES,
  ...
  public void presentValueCurveSensitivityFloor() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SS_C.calculateSensitivity(CMS_FLOOR, SABR_MULTICURVES,
        SABR_MULTICURVES.getMulticurveProvider().getAllNames());
  ...
  public void presentValueSABRSensitivity() {
    ...
    final double maturity = CMS_CAP.getUnderlyingSwap().getFixedLeg()
        .getNthPayment(CMS_CAP.getUnderlyingSwap().getFixedLeg().getNumberOfPayments() - 1).getPaymentTime() - CMS_CAP.getSettlementTime();
  ...
  public void presentValueStrikeSensitivityCap() {
    ...
    for (int i = 0; i < nbStrikes; i++) {
      final CapFloorCMSDefinition cmsCapDefinition = CapFloorCMSDefinition.from(CMS_COUPON_DEFINITION, strikes[i], IS_CAP);
      final CapFloorCMSDefinition cmsCapShiftUpDefinition = CapFloorCMSDefinition.from(CMS_COUPON_DEFINITION, strikes[i] + shift,
          IS_CAP);
      final CapFloorCMSDefinition cmsCapShiftDoDefinition = CapFloorCMSDefinition.from(CMS_COUPON_DEFINITION, strikes[i] - shift,
          IS_CAP);
      ...
      errorRelative[i] = (sensiExpected - sensiComputed) / sensiExpected;
      assertEquals("CMS cap/floor SABR: Present value strike sensitivity " + i, 0, errorRelative[i], 5.0E-4); // Numerical
    }
  }
  ...
  public void presentValueStrikeSensitivityFloor() {
    ...
    for (int loopstrike = 0; loopstrike < nbStrikes; loopstrike++) {
      ...
      final CapFloorCMSDefinition cmsFloorShiftUpDefinition = CapFloorCMSDefinition.from(CMS_COUPON_DEFINITION, strikes[loopstrike] + shift,
          !IS_CAP);
      final CapFloorCMSDefinition cmsFloorShiftDoDefinition = CapFloorCMSDefinition.from(CMS_COUPON_DEFINITION, strikes[loopstrike] - shift,
          !IS_CAP);
  ...
  public void presentValueAnnuity() {
    final Period startCmsCap = Period.ofYears(5);
    final Period lengthCmsCap = Period.ofYears(10);
    final ZonedDateTime startDate = ScheduleCalculator.getAdjustedDate(SPOT_DATE, startCmsCap,
        EUR1YEURIBOR6M.getIborIndex().getBusinessDayConvention(), CALENDAR, EUR1YEURIBOR6M.getIborIndex()
            .isEndOfMonth());
    final ZonedDateTime endDate = startDate.plus(lengthCmsCap);
    final Period capPeriod = Period.ofMonths(6);
    final DayCount capDayCount = DayCounts.ACT_360;
    final AnnuityCapFloorCMSDefinition capDefinition = AnnuityCapFloorCMSDefinition.from(startDate, endDate, NOTIONAL, INDEX_SWAP_5Y,
        capPeriod, capDayCount, false, STRIKE, IS_CAP, CalendarAdapter.of(CALENDAR));
