...
/**
 * Tests related to the pricing of Yield average bond futures transaction (in particular for AUD-SFE futures) with discounting method, i.e.
...
public class BondFuturesYieldAverageTransactionDiscountingMethodTest {
  ...
  private static final ZonedDateTime[] UNDERLYING_MATURITY_DATE = new ZonedDateTime[] { DateUtils.getUTCDate(2022, 7, 15),
      DateUtils.getUTCDate(2023, 4, 15),
  ...
  static {
    for (int i = 0; i < NB_BOND; i++) {
      START_ACCRUAL_DATE[i] = UNDERLYING_MATURITY_DATE[i].minusYears(12);
      BASKET_SECURITY_DEFINITION[i] = BondFixedSecurityDefinition.from(AUD, START_ACCRUAL_DATE[i], UNDERLYING_MATURITY_DATE[i],
          PAYMENT_TENOR,
          UNDERLYING_COUPON[i], SETTLEMENT_DAYS, NOTIONAL_BOND, EX_DIVIDEND_DAYS, CalendarAdapter.of(CALENDAR), DAY_COUNT, BUSINESS_DAY,
          YIELD_CONVENTION, IS_EOM, ISSUER_LEGAL_ENTITY, "Repo");
    }
  }
  private static final double SYNTHETIC_COUPON = 0.06;
  private static final int TENOR = 10;

  private static final BondFuturesYieldAverageSecurityDefinition FUT_SEC_DEFINITION = new BondFuturesYieldAverageSecurityDefinition(
      LAST_TRADING_DATE,
  ...
  private static final BondFuturesYieldAverageTransactionDefinition FUT_TRA_DEFINITION = new BondFuturesYieldAverageTransactionDefinition(
      FUT_SEC_DEFINITION,
  ...
  public void presentValueFromPrice() {
    ...
    assertEquals("YieldAverageBondFuturesTransactionDiscountingMethod: presentValueFromPrice", pvExpected, pvComputed.getAmount(AUD),
        TOLERANCE_PV);
  }
  ...
  public void presentValue() {
    ...
    assertEquals("YieldAverageBondFuturesTransactionDiscountingMethod: presentValue", pvExpected.getAmount(AUD), pvComputed.getAmount(AUD),
        TOLERANCE_PV);
  }
