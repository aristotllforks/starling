...
public class UsdDiscountingGovernment3Test extends CurveBuildingTests {
  ...
  private static final ZonedDateTime[] BOND_START_ACCRUAL_DATE = new ZonedDateTime[] { DateUtils.getUTCDate(2012, 07, 31),
      DateUtils.getUTCDate(2012, 8, 15),
      DateUtils.getUTCDate(2012, 07, 31), DateUtils.getUTCDate(2012, 07, 31), DateUtils.getUTCDate(2012, 8, 15),
      DateUtils.getUTCDate(2012, 8, 15) };
  /** Bond maturities */
  private static final ZonedDateTime[] BOND_MATURITY = new ZonedDateTime[] { DateUtils.getUTCDate(2014, 07, 31),
      DateUtils.getUTCDate(2015, 05, 15),
      DateUtils.getUTCDate(2017, 07, 31), DateUtils.getUTCDate(2019, 07, 31), DateUtils.getUTCDate(2022, 8, 15),
      DateUtils.getUTCDate(2042, 8, 15) };
  /** Coupons */
  ...
  static {
    for (int i = 0; i < BOND_MATURITY.length; i++) {
      BOND_SECURITY[i] = BondFixedSecurityDefinition.from(Currency.USD, BOND_MATURITY[i], BOND_START_ACCRUAL_DATE[i], Period.ofMonths(6),
          RATE_FIXED[i], 3, 1,
          CalendarAdapter.of(NYC), DayCounts.ACT_360, BusinessDayConventions.FOLLOWING, YIELD_BOND_USGOVT, false, NAME_COUNTERPART,
          "General collateral");
  ...
  private static final double[] GOVTUS_USD_MARKET_QUOTES = new double[] { 0.0010, 0.0015, 0.0020, 0.0015, 0.99642, 0.9981, 0.99587, 0.99466,
      0.99496, 0.98489 };
  /** Vanilla instrument generators for the government curve */
  private static final GeneratorInstrument[] GOVTUS_USD_GENERATORS = new GeneratorInstrument[] {
      GENERATOR_DEPOSIT_ON_USGOVT, GENERATOR_BILL[0], GENERATOR_BILL[1], GENERATOR_BILL[2], GENERATOR_BOND[0], GENERATOR_BOND[1],
      GENERATOR_BOND[2], GENERATOR_BOND[3], GENERATOR_BOND[4], GENERATOR_BOND[5] };
  /** Attribute generators for the government curve */
  private static final GeneratorAttributeIR[] GOVTUS_USD_ATTR;
  static {
    final Period[] tenors = new Period[] { Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0), Period.ofDays(0),
        Period.ofDays(0),
