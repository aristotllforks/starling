...
public class BillSecurityDiscountingMethodTest {
  ...
  // ISIN: BE0312677462
  private static final BillSecurityDefinition BILL_BEL_IAM_SEC_DEFINITION = new BillSecurityDefinition(EUR, END_DATE, NOTIONAL,
      SETTLEMENT_DAYS, WEEKEND,
      YIELD_IAM, ACT360, ISSUER_NAMES[1]);
  private static final BillSecurityDefinition BILL_US_DSC_SEC_DEFINITION = new BillSecurityDefinition(USD, END_DATE, NOTIONAL,
      SETTLEMENT_DAYS, WEEKEND,
  ...
  public void presentValue() {
    ...
    final double pvExpected = NOTIONAL
        * ISSUER_MULTICURVE.getDiscountFactor(BILL_BEL_IAM_SEC.getIssuerEntity(), BILL_BEL_IAM_SEC.getEndTime());
  ...
  public void yieldFromPriceDerivative() {
    ...
    assertEquals("Bill Security: discounting method - yield", yieldDerivativeExpectedIAM, yieldDerivativeComputedIAM,
    ...
    assertEquals("Bill Security: discounting method - yield", yieldDerivativeExpectedDSC, yieldDerivativeComputedDSC,
        TOLERANCE_YIELD_DERIVATIVE);
  }
  ...
  public void presentValueFromPrice() {
    ...
    final double pvExpected = NOTIONAL * PRICE
        * ISSUER_MULTICURVE.getMulticurveProvider().getDiscountFactor(EUR, BILL_BEL_IAM_SEC.getSettlementTime());
    assertEquals("Bill Security: discounting method - present value", pvExpected, pvComputed.getAmount(BILL_BEL_IAM_SEC.getCurrency()),
        TOLERANCE_PV);
  }
  ...
  public void presentValueCurveSensitivityMethodVsCalculator() {
    final MultipleCurrencyMulticurveSensitivity pvcsMethod = METHOD_SECURITY.presentValueCurveSensitivity(BILL_BEL_IAM_SEC,
        ISSUER_MULTICURVE);
