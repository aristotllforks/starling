...
public class CapFloorIborLMMDDMethodTest {
  ...
  private static final GeneratorSwapFixedIbor EUR3MEURIBOR3M = new GeneratorSwapFixedIbor("Ibor", EURIBOR3M.getTenor(),
      EURIBOR3M.getDayCount(), EURIBOR3M, CALENDAR);
  ...
  private static final SwapFixedIborDefinition SWAP_PAYER_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, SWAP_INDEX, NOTIONAL,
      STRIKE, FIXED_IS_PAYER, CalendarAdapter.of(CALENDAR));
  // to derivatives
  ...
  private static final CapFloorIbor CAP_LAST = new CapFloorIbor(EUR, COUPON_IBOR_LAST.getPaymentTime(),
      COUPON_IBOR_LAST.getPaymentYearFraction(), NOTIONAL,
      COUPON_IBOR_LAST.getFixingTime(), EURIBOR3M, COUPON_IBOR_LAST.getFixingPeriodStartTime(), COUPON_IBOR_LAST.getFixingPeriodEndTime(),
      COUPON_IBOR_LAST.getFixingAccrualFactor(),
      STRIKE, IS_CAP);
  private static final CapFloorIbor FLOOR_LAST = new CapFloorIbor(EUR, COUPON_IBOR_LAST.getPaymentTime(),
      COUPON_IBOR_LAST.getPaymentYearFraction(), NOTIONAL,
      COUPON_IBOR_LAST.getFixingTime(), EURIBOR3M, COUPON_IBOR_LAST.getFixingPeriodStartTime(), COUPON_IBOR_LAST.getFixingPeriodEndTime(),
      COUPON_IBOR_LAST.getFixingAccrualFactor(),
      STRIKE, !IS_CAP);
  private static final CapFloorIbor CAP_LAST_SHORT = new CapFloorIbor(EUR, COUPON_IBOR_LAST.getPaymentTime(),
      COUPON_IBOR_LAST.getPaymentYearFraction(), -NOTIONAL,
      COUPON_IBOR_LAST.getFixingTime(), EURIBOR3M, COUPON_IBOR_LAST.getFixingPeriodStartTime(), COUPON_IBOR_LAST.getFixingPeriodEndTime(),
      COUPON_IBOR_LAST.getFixingAccrualFactor(),
      STRIKE, IS_CAP);
  ...
  private static final CapFloorIbor CAP_6 = new CapFloorIbor(EUR, COUPON_IBOR_6.getPaymentTime(), COUPON_IBOR_6.getPaymentYearFraction(),
      NOTIONAL,
      COUPON_IBOR_6.getFixingTime(), EURIBOR3M, COUPON_IBOR_6.getFixingPeriodStartTime(), COUPON_IBOR_6.getFixingPeriodEndTime(),
      COUPON_IBOR_6.getFixingAccrualFactor(),
      STRIKE, IS_CAP);
  ...
  public void presentValueExplicit() {
    ...
    final double timeDependentFactor = Math.sqrt(
        (Math.exp(2 * PARAMETERS_LMM.getMeanReversion() * CAP_LAST.getFixingTime()) - 1.0) / (2.0 * PARAMETERS_LMM.getMeanReversion()));
    volatility *= timeDependentFactor;
    final double displacement = PARAMETERS_LMM.getDisplacement()[index];
    final double forward = MULTICURVES.getSimplyCompoundForwardRate(CAP_LAST.getIndex(), CAP_LAST.getFixingPeriodStartTime(),
        CAP_LAST.getFixingPeriodEndTime(), CAP_LAST.getFixingAccrualFactor());
    final double beta = (1.0 + CAP_LAST.getFixingAccrualFactor() * forward)
        * MULTICURVES.getDiscountFactor(EUR, CAP_LAST.getFixingPeriodEndTime())
    ...
    final double forwardDsc = (MULTICURVES.getDiscountFactor(EUR, CAP_LAST.getFixingPeriodStartTime())
        / MULTICURVES.getDiscountFactor(EUR, CAP_LAST.getFixingPeriodEndTime()) - 1.0)
  ...
  public void presentValuePutCallParity() {
    ...
    assertEquals("Cap/floor: LMM pricing by explicit formula", -pvIbor.getAmount(EUR) - pvFixed.getAmount(EUR),
        pvCapShort.getAmount(EUR) + pvFloorLong.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void longShortParity() {
    ...
    assertEquals("Cap/floor - LMM - present value - long/short parity", pvLongExplicit.getAmount(EUR), -pvShortExplicit.getAmount(EUR),
        TOLERANCE_PV);
    LiborMarketModelMonteCarloMethod methodLmmMc;
    methodLmmMc = new LiborMarketModelMonteCarloMethod(new NormalRandomNumberGenerator(0.0, 1.0, new MersenneTwister()), NB_PATH);
  ...
  public void capFloorParity() {
    ...
    assertEquals("Cap/floor - LMM - present value Explcit- cap/floor/strike/Ibor parity",
        pvCapExplicit.getAmount(EUR) - pvFloorExplicit.getAmount(EUR) - pvFixedExplicit.getAmount(EUR),
    ...
    assertEquals("Cap/floor - LMM - present value - cap/floor/strike/Ibor parity",
        pvCapMC.getAmount(EUR) - pvFloorMC.getAmount(EUR) - pvFixedExplicit.getAmount(EUR), pvIborExplicit.getAmount(EUR),
        1.0E+3);
  }
