...
public class DayPeriodPreCalculatedDiscountCurveTest {
  ...
  private static final CouponIborDefinition CPN_IBOR_6_DEFINITION = CouponIborDefinition.from(END_DATE_3, END_DATE_6, NOTIONAL, EURIBOR6M,
      CalendarAdapter.of(CALENDAR));
  private static final AnnuityDefinition<CouponDefinition> ANNUITY_IBOR_DEFINITION = new AnnuityDefinition<>(
      new CouponIborDefinition[] { CPN_IBOR_3_DEFINITION, CPN_IBOR_6_DEFINITION }, CalendarAdapter.of(CALENDAR));
  private static final GeneratorSwapFixedIbor EUR1YEURIBOR6M = GeneratorSwapFixedIborMaster.getInstance().getGenerator("EUR1YEURIBOR6M",
      CALENDAR);
  private static final AnnuityCouponFixedDefinition ANNUITY_FIXED_DEFINITION = AnnuityCouponFixedDefinition.from(START_DATE, TOTAL_TENOR,
   ...
   */
  private static final YieldAndDiscountCurve EUR_DSC = DiscountCurve
      .from(new InterpolatedDoublesCurve(EUR_DSC_TIME, EUR_DSC_DISCOUNT_FACTOR, LINEAR_FLAT, true, EUR_DSC_NAME));
  ...
  /**
   * Tests the present value for a swap with the Ibor leg having different Ibor indexes (EURIBOR3M and EURIBOR6M). this test passed onbly if
   * the MODEL_DAYCOUNT in TimeCalculator.properties is set at Actul / 365.25
   */
  @Test(enabled = false)
  public void testWithSwap() {

    MULTICURVES.replaceCurve(EUR, EUR_DSC);
    final DayPeriodPreCalculatedDiscountCurve curve = new DayPeriodPreCalculatedDiscountCurve("test",
        new InterpolatedDoublesCurve(EUR_DSC_TIME, EUR_DSC_DISCOUNT_FACTOR, LINEAR_FLAT, true,
            EUR_DSC_NAME),
        365.25);
    curve.preCalculateDiscountFactors(50);
    final MulticurveProviderDiscount curvesWithPrecalibratedDiscount = MULTICURVES.withDiscountFactor(EUR, curve);
    ...
    final MultipleCurrencyAmount pvCalcCpn3_WITH_PRECALCULATED_DISCOUNT = CPN_IBOR_3.accept(PVDC, curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvMethCpn3_WITH_PRECALCULATED_DISCOUNT = METHOD_CPN_IBOR.presentValue(CPN_IBOR_3,
        curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvCalcCpn6_WITH_PRECALCULATED_DISCOUNT = CPN_IBOR_6.accept(PVDC, curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvMethCpn6_WITH_PRECALCULATED_DISCOUNT = METHOD_CPN_IBOR.presentValue(CPN_IBOR_6,
        curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvCalcAnnIbor_WITH_PRECALCULATED_DISCOUNT = ANNUITY_IBOR.accept(PVDC,
        curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvCalcAnnFixed_WITH_PRECALCULATED_DISCOUNT = ANNUITY_FIXED.accept(PVDC,
        curvesWithPrecalibratedDiscount);
    final MultipleCurrencyAmount pvCalcSwap_WITH_PRECALCULATED_DISCOUNT = SWAP.accept(PVDC, curvesWithPrecalibratedDiscount);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcCpn3_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR),
        pvCalcCpn3.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcCpn6_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR),
        pvCalcCpn6.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcAnnIbor_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR),
        pvCalcAnnIbor.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcAnnFixed.getAmount(EUR),
        pvCalcAnnFixed_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvMethCpn3.getAmount(EUR),
        pvMethCpn3_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvMethCpn6.getAmount(EUR),
        pvMethCpn6_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR), TOLERANCE_PV);
    assertEquals("PresentValueDiscountingProviderCalculator: multiple Ibor index", pvCalcSwap_WITH_PRECALCULATED_DISCOUNT.getAmount(EUR),
        pvCalcSwap.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void testPreCalculatedVsOutput() {
    ...
    final double[] dscCurveTime = new double[] { 0.0, 1 / 365.25, 2 / 365.25, 3 / 365.25, 91 / 365.25, 183 / 365.25, 274 / 365.25,
        365 / 365.25, 457 / 365.25, 548 / 365.25, 639 / 365.25, 731 / 365.25,
        1096 / 365.25, 1461 / 365.25, 1826 / 365.25, 2192 / 365.25, 2557 / 365.25, 2922 / 365.25, 3287 / 365.25, 3653 / 365.25,
        4383 / 365.25, 5479 / 365.25, 7305 / 365.25, 9131 / 365.25,
        10958 / 365.25, 14610 / 365.25, 18263 / 365.25 };

    final double[] dscCurveDiscountFactor = new double[] { 1.0, 0.999995414, 0.999990827, 0.999986241, 0.999583194, 0.999167209,
        0.998770249, 0.998379852, 0.997966181, 0.997459857, 0.996773166,
        0.995843348, 0.989078497, 0.976557279, 0.959377269, 0.938363477, 0.91453858, 0.888747447, 0.861693639, 0.83391463, 0.778517911,
        0.700537471, 0.596392269, 0.516411342, 0.451196242, 0.3436152,
        0.259911234 };

    final DayPeriodPreCalculatedDiscountCurve periodPreCalculatedCurve = new DayPeriodPreCalculatedDiscountCurve("test",
        new InterpolatedDoublesCurve(dscCurveTime, dscCurveDiscountFactor,
            logNaturalinterpolator, true, preiInterpolatedCurveName),
        365.25);
    periodPreCalculatedCurve.preCalculateDiscountFactors(50);
    final double[] outputTime = new double[] { 0.0, 1 / 365.25, 2 / 365.25, 3 / 365.25, 4 / 365.25, 7 / 365.25, 8 / 365.25, 9 / 365.25,
        17 / 365.25, 77 / 365.25, 91 / 365.25, 140 / 365.25,
        183 / 365.25, 219 / 365.25, 274 / 365.25, 301 / 365.25, 365 / 365.25, 672 / 365.25, 1001 / 365.25, 2034 / 365.25, 9879 / 365.25,
        13374 / 365.25, 16180 / 365.25 };

    final double[] outputDiscountFactor = new double[] { 1.0, 0.999995414, 0.999990827, 0.999986241, 0.999981654, 0.999967895, 0.999963309,
        0.999958723, 0.999922035, 0.999647188, 0.999583194,
        0.999360197, 0.999167209, 0.999008575, 0.998770249, 0.998654304, 0.998379852, 0.996469188, 0.991376737, 0.947845026, 0.488392208,
        0.377151958, 0.304915694 };
    for (int i = 0; i < outputDiscountFactor.length; i++) {
      assertEquals("DayPeriodPreCalculatedDiscountCurve: discount factor value" + outputTime[i] * 365.25, outputDiscountFactor[i],
          periodPreCalculatedCurve.getDiscountFactor(outputTime[i]),
          1.0E-6);
    }
  }
  ...
  public void testPreCalculatedVsOriginalCurve() {
    ...
    final YieldAndDiscountCurve discountCurve = new DiscountCurve(curveName,
        new InterpolatedDoublesCurve(dscCurveTime, dscCurveDiscountFactor,
            logNaturalinterpolator, true, preiInterpolatedCurveName));
    final DayPeriodPreCalculatedDiscountCurve periodPreCalculatedCurve = new DayPeriodPreCalculatedDiscountCurve("test",
        new InterpolatedDoublesCurve(dscCurveTime, dscCurveDiscountFactor,
    ...
    final double[] outputDiscountFactor = new double[] { 1.0, 0.999995414, 0.999990827, 0.999986241, 0.999981654, 0.999967895, 0.999963309,
        ...
        0.377151958, 0.304915694 };
    for (int i = 0; i < outputDiscountFactor.length; i++) {
      assertEquals("DayPeriodPreCalculatedDiscountCurve: discount factor value" + outputTime[i] * 365.25,
          discountCurve.getDiscountFactor(outputTime[i]),
