...
public class BondCapitalIndexedSecurityDiscountingMethodGBPTest {
  ...
  // private static final CouponInflationZeroCouponInterpolationGearingDiscountingMethod METHOD_INFLATION_ZC_INTERPOLATION = new
  // CouponInflationZeroCouponInterpolationGearingDiscountingMethod();
  ...
  private static final ParameterSensitivityInflationMulticurveDiscountInterpolatedFDCalculator PS_PV_FDC = new ParameterSensitivityInflationMulticurveDiscountInterpolatedFDCalculator(
      PVDIC, SHIFT_FD);
  // private static final ParameterSensitivityIssuerInflationMulticurveDiscountInterpolatedFDCalculator PS_PV_FDIC = new
  // ParameterSensitivityIssuerInflationMulticurveDiscountInterpolatedFDCalculator(
  // PVDIIC, SHIFT_FD);
  ...
  private static final BondCapitalIndexedSecurity<Coupon> BOND_SECURITY_GILT_2 = BOND_SECURITY_GILT_1_DEFINITION
      .toDerivative(PRICING_DATE_2, UK_RPI);
  ...
  public void accruedInterest() {
    final double aiComputed = BOND_SECURITY_GILT_2.getAccruedInterest(); // Accrued interest real
    final double indexRatio = 252.1d / 173.6d;
    final double aiExpected = 0.01083124; // Accrued interest nominal.
  ...
  public void priceYield() {
    final double yieldExpected = 0.00132482;
    final double priceComputed = METHOD_BOND_INFLATION.cleanPriceFromYield(BOND_SECURITY_GILT_2, yieldExpected);
    assertEquals("Inflation Capital Indexed bond: yield - price", priceComputed, PRICE_GILT, TOLERANCE_PRICE);
    final double yieldComputed = METHOD_BOND_INFLATION.yieldRealFromCleanPrice(BOND_SECURITY_GILT_2, PRICE_GILT);
    assertEquals("Inflation Capital Indexed bond: present value", yieldComputed, yieldExpected, TOLERANCE_YIELD);
    final double yieldComputed2 = METHOD_BOND_INFLATION.yieldRealFromCleanPrice(BOND_SECURITY_GILT_2, priceComputed);
  ...
  private static final BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> BOND_SECURITY_CORP_1_DEFINITION = BondCapitalIndexedSecurityDefinition
      .fromMonthly(
          PRICE_INDEX_UKRPI, MONTH_LAG_CORP_1, START_DATE_CORP_1, INDEX_START_CORP_1, FIRST_COUPON_DATE_CORP_1, MATURITY_DATE_CORP_1,
          COUPON_PERIOD_CORP_1, NOTIONAL_CORP_1, REAL_RATE_CORP_1,
          BUSINESS_DAY_GBP, SETTLEMENT_DAYS_CORP_1, CalendarAdapter.of(CALENDAR_GBP), DAY_COUNT_GILT_1, YIELD_CONVENTION_CORP_1,
          IS_EOM_GILT_1, ISSUER_TOYOTA);
  private static final BondCapitalIndexedSecurity<Coupon> BOND_SECURITY_CORP_1 = BOND_SECURITY_CORP_1_DEFINITION
      .toDerivative(PRICING_DATE_3, UK_RPI);
  ...
  public void priceYieldUKILBond() {
    final double priceComputed = METHOD_BOND_INFLATION.cleanPriceFromYield(BOND_SECURITY_CORP_1, YIELD_CORP);
    final double yieldComputed = METHOD_BOND_INFLATION.yieldRealFromCleanPrice(BOND_SECURITY_CORP_1, PRICE_CLEAN_CORP);
    final double yieldComputed3 = METHOD_BOND_INFLATION.yieldRealFromCleanPrice(BOND_SECURITY_CORP_1, priceComputed);
