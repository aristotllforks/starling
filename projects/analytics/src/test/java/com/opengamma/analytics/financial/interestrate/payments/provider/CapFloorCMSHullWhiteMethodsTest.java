...
public class CapFloorCMSHullWhiteMethodsTest {
  ...
  static {
    for (int i = 0; i < NB_STRIKE; i++) {
      CAP_CMS_DEFINITION[i] = CapFloorCMSDefinition.from(PAYMENT_DATE, START_DATE, PAYMENT_DATE, ACCRUAL_FACTOR, NOTIONAL, SWAP_EUR10Y,
          STRIKE[i], true, CalendarAdapter.of(TARGET));
  ...
  public void presentValueApproximation() {
    for (int i = 0; i < NB_STRIKE; i++) {
      ...
      assertEquals(pvNumericalIntegration.getAmount(EUR), pvApproximation.getAmount(EUR), NOTIONAL
          * ACCRUAL_FACTOR * BP1);
