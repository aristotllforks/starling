...
/**
 * Class to test the present value and present value rate sensitivity of the physical delivery swaption in the SABR with extrapolation
 * method. The SABR smile is extrapolated above a certain cut-off strike.
 */
...
public class SwaptionPhysicalFixedIborSABRExtrapolationRightMethodTest {
  ...
  private static final SwapFixedIborDefinition SWAP_PAYER_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR,
      EUR1YEURIBOR6M, NOTIONAL, RATE, FIXED_IS_PAYER);
  private static final SwapFixedIborDefinition SWAP_RECEIVER_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR,
      EUR1YEURIBOR6M, NOTIONAL, RATE, !FIXED_IS_PAYER);
  ...
  private static final SwapFixedIborDefinition SWAP_PAYER_HIGH_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR,
      EUR1YEURIBOR6M, NOTIONAL, HIGH_STRIKE, FIXED_IS_PAYER);
  private static final SwapFixedIborDefinition SWAP_RECEIVER_HIGH_DEFINITION = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR,
      EUR1YEURIBOR6M, NOTIONAL, HIGH_STRIKE, !FIXED_IS_PAYER);
  private static final SwapFixedCoupon<Coupon> SWAP_PAYER_HIGH = SWAP_PAYER_HIGH_DEFINITION.toDerivative(REFERENCE_DATE);
  private static final SwaptionPhysicalFixedIborDefinition SWAPTION_LONG_PAYER_HIGH_DEFINITION = SwaptionPhysicalFixedIborDefinition
      .from(EXPIRY_DATE, SWAP_PAYER_HIGH_DEFINITION, true, IS_LONG);
  private static final SwaptionPhysicalFixedIborDefinition SWAPTION_SHORT_PAYER_HIGH_DEFINITION = SwaptionPhysicalFixedIborDefinition
      .from(EXPIRY_DATE, SWAP_PAYER_HIGH_DEFINITION, true, !IS_LONG);
  ...
  private static final SwaptionPhysicalFixedIbor SWAPTION_LONG_PAYER_HIGH = SWAPTION_LONG_PAYER_HIGH_DEFINITION
      .toDerivative(REFERENCE_DATE);
  private static final SwaptionPhysicalFixedIbor SWAPTION_SHORT_PAYER_HIGH = SWAPTION_SHORT_PAYER_HIGH_DEFINITION
      .toDerivative(REFERENCE_DATE);
  private static final SwaptionPhysicalFixedIbor SWAPTION_LONG_RECEIVER_HIGH = SWAPTION_LONG_RECEIVER_HIGH_DEFINITION
      .toDerivative(REFERENCE_DATE);
  // Extrapolation
  ...
  private static final SwaptionPhysicalFixedIborSABRExtrapolationRightMethod METHOD_SABR_EXTRAPOLATION = new SwaptionPhysicalFixedIborSABRExtrapolationRightMethod(
      CUT_OFF_STRIKE, MU);
  // Calculators
  ...
  public void presentValueNoExtra() {
    ...
    assertEquals("Swaption SABR extrapolation: below cut-off strike", priceLongPayerNoExtra.getAmount(EUR), priceLongPayer.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: below cut-off strike", priceShortPayerNoExtra.getAmount(EUR), priceShortPayer.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: below cut-off strike", priceLongReceiverNoExtra.getAmount(EUR),
        priceLongReceiver.getAmount(EUR), TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: below cut-off strike", priceShortReceiverNoExtra.getAmount(EUR),
        priceShortReceiver.getAmount(EUR), TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: below cut-off strike long/short parity", priceLongPayer.getAmount(EUR),
        -priceShortPayer.getAmount(EUR), TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: below cut-off strike long/short parity", priceLongReceiver.getAmount(EUR),
        -priceShortReceiver.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void presentValueLimit() {
    ...
    final SwapFixedIborDefinition swapPayerHighStrike = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR, EUR1YEURIBOR6M,
        NOTIONAL, highStrike, FIXED_IS_PAYER);
    final SwapFixedIborDefinition swapReceiverHighStrike = SwapFixedIborDefinition.from(SETTLEMENT_DATE, ANNUITY_TENOR, EUR1YEURIBOR6M,
        NOTIONAL, highStrike, !FIXED_IS_PAYER);
    final SwaptionPhysicalFixedIborDefinition swaptionDefinitionLongPayerHighStrike = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE,
        swapPayerHighStrike, true, IS_LONG);
    final SwaptionPhysicalFixedIborDefinition swaptionDefinitionShortPayerHighStrike = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE,
        swapPayerHighStrike, true, !IS_LONG);
    final SwaptionPhysicalFixedIborDefinition swaptionDefinitionLongReceiverHighStrike = SwaptionPhysicalFixedIborDefinition
        .from(EXPIRY_DATE, swapReceiverHighStrike, false, IS_LONG);
    ...
    final MultipleCurrencyAmount priceLongReceiver = METHOD_SABR_EXTRAPOLATION.presentValue(swaptionLongReceiverHighStrike,
        SABR_MULTICURVES);
    ...
    assertEquals("Swaption SABR extrapolation: extrapolation limit", priceLongPayerSABR.getAmount(EUR), priceLongPayer.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: extrapolation limit", priceLongReceiverSABR.getAmount(EUR), priceLongReceiver.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: long/short parity", priceLongPayer.getAmount(EUR), -priceShortPayer.getAmount(EUR),
        TOLERANCE_PV);
  }

  /**
   * Tests present value in the region where there is extrapolation. Test a hard-coded value. Tests long/short parity. Test
   * payer/receiver/swap parity.
   */
  @Test
  public void presentValueExtra() {
    ...
    assertEquals("Swaption SABR extrapolation: long/short parity", priceLongPayer.getAmount(EUR), -priceShortPayer.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("Swaption SABR extrapolation: payer/receiver/swap parity", pricePayer.getAmount(EUR),
        priceLongPayer.getAmount(EUR) - priceLongReceiver.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void presentValueCurveSensitivityLongPayerExtra() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SSX_C.calculateSensitivity(SWAPTION_LONG_PAYER_HIGH, SABR_MULTICURVES,
  ...
  public void presentValueCurveSensitivityLongReceiverExtra() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SSX_C.calculateSensitivity(SWAPTION_LONG_RECEIVER_HIGH, SABR_MULTICURVES,
        SABR_MULTICURVES.getMulticurveProvider().getAllNames());
  ...
  public void presentValueCurveSensitivityShortReceiverExtra() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SSX_C.calculateSensitivity(SWAPTION_SHORT_PAYER_HIGH, SABR_MULTICURVES,
        SABR_MULTICURVES.getMulticurveProvider().getAllNames());
  ...
  public void presentValueSABRSensitivity() {
    ...
    final PresentValueSABRSensitivityDataBundle pvsLongPayer = METHOD_SABR_EXTRAPOLATION
        .presentValueSABRSensitivity(SWAPTION_LONG_PAYER_HIGH, SABR_MULTICURVES);
    PresentValueSABRSensitivityDataBundle pvsShortPayer = METHOD_SABR_EXTRAPOLATION.presentValueSABRSensitivity(SWAPTION_SHORT_PAYER_HIGH,
        SABR_MULTICURVES);
    // Long/short parity
    ...
    final SABRSwaptionProviderDiscount sabrBundleAlphaBumped = new SABRSwaptionProviderDiscount(MULTICURVES, sabrParameterAlphaBumped,
        EUR1YEURIBOR6M);
    final MultipleCurrencyAmount pvLongPayerAlphaBumped = METHOD_SABR_EXTRAPOLATION.presentValue(SWAPTION_LONG_PAYER_HIGH,
        sabrBundleAlphaBumped);
    ...
    final SABRSwaptionProviderDiscount sabrBundleBetaBumped = new SABRSwaptionProviderDiscount(MULTICURVES, sabrParameterBetaBumped,
        EUR1YEURIBOR6M);
    final MultipleCurrencyAmount pvLongPayerBetaBumped = METHOD_SABR_EXTRAPOLATION.presentValue(SWAPTION_LONG_PAYER_HIGH,
        sabrBundleBetaBumped);
    ...
    final SABRSwaptionProviderDiscount sabrBundleRhoBumped = new SABRSwaptionProviderDiscount(MULTICURVES, sabrParameterRhoBumped,
        EUR1YEURIBOR6M);
    final MultipleCurrencyAmount pvLongPayerRhoBumped = METHOD_SABR_EXTRAPOLATION.presentValue(SWAPTION_LONG_PAYER_HIGH,
        sabrBundleRhoBumped);
  ...
  public void performance() {
    ...
    // Performance note: price+delta+vega payer extrapolation: 12-Dec-2012: On Mac Pro 3.2 GHz Quad-Core Intel Xeon: 155 ms for 1000
    // swaptions.
