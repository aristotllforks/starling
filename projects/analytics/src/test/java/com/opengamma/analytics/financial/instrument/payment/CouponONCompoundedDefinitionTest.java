...
public class CouponONCompoundedDefinitionTest {
  ...
  private static final CouponONCompoundedDefinition ON_COMPOUNDED_COUPON_DEFINITION = new CouponONCompoundedDefinition(EUR_CUR,
      EUR_PAYMENT_DATE, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE,
  ...
  public void nullIndex() {
    new CouponONCompoundedDefinition(EUR_CUR, EUR_PAYMENT_DATE, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE, EUR_PAYMENT_YEAR_FRACTION,
        NOTIONAL, null, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE,
        EUR_CALENDAR);
  }
  ...
  public void nullStartFixing() {
    new CouponONCompoundedDefinition(EUR_CUR, EUR_PAYMENT_DATE, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE, EUR_PAYMENT_YEAR_FRACTION,
        NOTIONAL, EUR_OIS, null, EUR_END_ACCRUAL_DATE, EUR_CALENDAR);
  }
  ...
  public void nullEndFixing() {
    new CouponONCompoundedDefinition(EUR_CUR, EUR_PAYMENT_DATE, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE, EUR_PAYMENT_YEAR_FRACTION,
        NOTIONAL, EUR_OIS, START_ACCRUAL_DATE, null, EUR_CALENDAR);
  }
  ...
  public void inmcompatibleCurrency() {
    new CouponONCompoundedDefinition(Currency.USD, EUR_PAYMENT_DATE, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE, EUR_PAYMENT_YEAR_FRACTION,
        NOTIONAL, EUR_OIS, START_ACCRUAL_DATE, EUR_END_ACCRUAL_DATE,
        EUR_CALENDAR);
  }
  ...
  public void getter() {
    ...
    assertEquals("CouponONCompounded definition: getter", EUR_LAST_FIXING_DATE,
        LocalDateTime.of(2011, 9, 15, 0, 0, 0, 0).atZone(ZoneOffset.UTC));
    assertEquals("CouponONCompounded definition: getter", lastFixingPeriodEndDate,
  ...
  @Test
  public void from() {
    final CouponONCompoundedDefinition cpnFrom = CouponONCompoundedDefinition.from(EUR_OIS, SPOT_DATE, CPN_TENOR, NOTIONAL,
        EUR_SETTLEMENT_DAYS, EUR_BUSINESS_DAY, EUR_IS_EOM, EUR_CALENDAR);
  ...
  public void toDerivativeNoFixingDeprecated() {
    ...
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 1; i++) {
      FIXING_PERIOD_START_TIMES[i] = EUR_DAY_COUNT.getDayCountFraction(TRADE_DATE,
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i], EUR_CALENDAR);
      FIXING_PERIOD_END_TIMES[i] = EUR_DAY_COUNT.getDayCountFraction(TRADE_DATE,
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1], EUR_CALENDAR);
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i],
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1],
          EUR_CALENDAR);
    }
    final CouponONCompounded cpnExpected = new CouponONCompounded(EUR_CUR, paymentTime, curveNames[0], EUR_PAYMENT_YEAR_FRACTION, NOTIONAL,
        EUR_OIS, FIXING_PERIOD_START_TIMES,
  ...
  public void toDerivativeFixingBeforeStartDeprecated() {
    ...
    final double[] FIXING_PERIOD_ACCRUAL_FACTOR = new double[ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 1];
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 1; i++) {
      FIXING_PERIOD_START_TIMES[i] = EUR_DAY_COUNT.getDayCountFraction(referenceDate,
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i], EUR_CALENDAR);
      FIXING_PERIOD_END_TIMES[i] = EUR_DAY_COUNT.getDayCountFraction(referenceDate,
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1], EUR_CALENDAR);
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i],
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1],
          EUR_CALENDAR);
  ...
  public void toDerivativeFixingOnStartNotYetFixedDeprecated() {
    ...
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 1; i++) {
      ...
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i],
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1],
          EUR_CALENDAR);
      FIXING_PERIOD_ACCRUAL_FACTOR_ACT_ACT[i] = TimeCalculator.getTimeBetween(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i],
          ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1]);
    }
    final CouponONCompounded cpnExpected = new CouponONCompounded(EUR_CUR, paymentTime, curveNames[0], EUR_PAYMENT_YEAR_FRACTION, NOTIONAL,
        EUR_OIS, FIXING_PERIOD_START_TIMES,
  ...
  public void toDerivativeFixingOnStartFixedDeprecated() {
    ...
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 2; i++) {
      ...
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 1],
  ...
  public void toDerivativeFixingMiddleNotYetFixedDeprecated() {
    ...
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 3; i++) {
      ...
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 2],
  ...
  public void toDerivativeFixingMiddleFixedDeprecated() {
    ...
    for (int i = 0; i < ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates().length - 4; i++) {
      ...
      FIXING_PERIOD_ACCRUAL_FACTOR[i] = EUR_DAY_COUNT.getDayCountFraction(ON_COMPOUNDED_COUPON_DEFINITION.getFixingPeriodDates()[i + 3],
