...
public class BlackImpliedVolatilityWithGreeksTest {
  ...
  private static final int N_STRIKES = STRIKE.length;
  private static final int N_VOLS = VOL.length;
  private static final int N_RATES = INTEREST_RATE.length;

  private static final double[] SPOT = new double[N_RATES];
  static {
      ...
      for (int i = 0; i < N_STRIKES; ++i) {
        for (int j = 0; j < N_VOLS; ++j) {
          for (int k = 0; k < N_RATES; ++k) {
            final double spotOptionPrice =
                BlackScholesFormulaRepository.price(SPOT[k], STRIKE[i], TIME_TO_EXPIRY, VOL[j], INTEREST_RATE[k], INTEREST_RATE[k], call);
            final double forwardOptionPrice =
                BlackFormulaRepository.price(FORWARD, STRIKE[i], TIME_TO_EXPIRY, VOL[j], call);
            final double[] volAndGreeks =
                CALCULATOR.getImpliedVolatilityAndGreeksForward(spotOptionPrice, FORWARD, SPOT[k], STRIKE[i], TIME_TO_EXPIRY, call);
            final double[] volAndGreeksOption =
                CALCULATOR.getImpliedVolatilityAndGreeksForwardOption(spotOptionPrice, forwardOptionPrice, SPOT[k], STRIKE[i], TIME_TO_EXPIRY, call);
            final double[] volAndGreeksDiscount =
                CALCULATOR.getImpliedVolatilityAndGreeksDiscountFactor(spotOptionPrice, Math.exp(-INTEREST_RATE[k] * TIME_TO_EXPIRY), SPOT[k], STRIKE[i],
                TIME_TO_EXPIRY, call);
            ...
            final double refDelta =
                BlackScholesFormulaRepository.delta(SPOT[k], STRIKE[i], TIME_TO_EXPIRY, volAndGreeks[0], INTEREST_RATE[k], INTEREST_RATE[k], call);
            final double refGamma =
                BlackScholesFormulaRepository.gamma(SPOT[k], STRIKE[i], TIME_TO_EXPIRY, volAndGreeks[0], INTEREST_RATE[k], INTEREST_RATE[k]);
            final double refVega =
                BlackScholesFormulaRepository.vega(SPOT[k], STRIKE[i], TIME_TO_EXPIRY, volAndGreeks[0], INTEREST_RATE[k], INTEREST_RATE[k]);
   ...
   * which are inputs of getImpliedVolatilityAndGreeksDiscountFactor (or getPriceAndGreeksDiscountFactor)
   * and getImpliedVolatilityAndGreeksForward (or getPriceAndGreeksForward), respectively.
   */
  @Test
  public void sampleTest() {
    ...
    final double[] discountFactor =
        new double[] {
            0.9991236718712008, 0.9991236718712008, 0.9991236718712008, 0.9991236718712008, 0.9991236718712008, 0.9991236718712008, 0.9972093804899117,
            0.9972093804899117, 0.9972093804899117, 0.9972093804899117, 0.9972093804899117 };
    for (int i = 0; i < nOptions; ++i) {
      final double[] volAndGreeks =
          CALCULATOR.getImpliedVolatilityAndGreeksDiscountFactor(optionPrice[i], discountFactor[i], spot, strike[i], timeToExpiry[i], isCall);
      if (print) {
    ...
    final double[] forward =
        new double[] {
            1687.4387500422877, 1687.4387500422877, 1687.4387500422877, 1687.4387500422877, 1687.4387500422877, 1687.4387500422877, 1690.6780391212496,
