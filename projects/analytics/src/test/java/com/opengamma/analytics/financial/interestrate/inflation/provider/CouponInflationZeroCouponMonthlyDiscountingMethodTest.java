...
public class CouponInflationZeroCouponMonthlyDiscountingMethodTest {
  ...
  private static final CouponInflationZeroCouponMonthlyDefinition ZERO_COUPON_NO_DEFINITION = CouponInflationZeroCouponMonthlyDefinition
      .from(START_DATE, PAYMENT_DATE, NOTIONAL, PRICE_INDEX_EUR,
  ...
  private static final DoubleTimeSeries<ZonedDateTime> PRICE_INDEX_TS = ImmutableZonedDateTimeDoubleTimeSeries.ofUTC(
      new ZonedDateTime[] { DateUtils.getUTCDate(2008, 5, 31), DateUtils.getUTCDate(2011, 5, 31), DateUtils.getUTCDate(2018, 5, 31),
          DateUtils.getUTCDate(2018, 6, 30) },
      new double[] { 108.23, 115.0,
          128.23, 128.43 });

  private static final CouponInflationZeroCouponMonthly ZERO_COUPON_NO = (CouponInflationZeroCouponMonthly) ZERO_COUPON_NO_DEFINITION
      .toDerivative(PRICING_DATE, PRICE_INDEX_TS);
  private static final CouponInflationZeroCouponMonthlyDefinition ZERO_COUPON_WITH_DEFINITION = CouponInflationZeroCouponMonthlyDefinition
      .from(START_DATE, PAYMENT_DATE, NOTIONAL, PRICE_INDEX_EUR,
          MONTH_LAG, MONTH_LAG, true);
  private static final CouponInflationZeroCouponMonthly ZERO_COUPON_WITH = (CouponInflationZeroCouponMonthly) ZERO_COUPON_WITH_DEFINITION
      .toDerivative(PRICING_DATE, PRICE_INDEX_TS);
  ...
  private static final ParSpreadInflationMarketQuoteDiscountingCalculator PSIMQDC = ParSpreadInflationMarketQuoteDiscountingCalculator
      .getInstance();
  private static final ParSpreadInflationMarketQuoteCurveSensitivityDiscountingCalculator PSIMQSDC = ParSpreadInflationMarketQuoteCurveSensitivityDiscountingCalculator
      .getInstance();
  private static final GeneratorSwapFixedInflationZeroCoupon GENERATOR_INFLATION_SWAP = GeneratorSwapFixedInflationMaster.getInstance()
  ...
  private static final SwapFixedInflationZeroCouponDefinition SWAP_DEFINITION = GENERATOR_INFLATION_SWAP.generateInstrument(PRICING_DATE,
      MARKET_QUOTE, NOTIONAL,
      GENERATOR);
  private static final InstrumentDerivative SWAP_DERIVATIVE = SWAP_DEFINITION.toDerivative(PRICING_DATE,
      new ZonedDateTimeDoubleTimeSeries[] { (ZonedDateTimeDoubleTimeSeries) PRICE_INDEX_TS,
  ...
  public void parSpreadSensitivityOnASwap() {
    ...
    final CouponInflationZeroCouponMonthly secondLeg = (CouponInflationZeroCouponMonthly) ((Swap<?, ?>) SWAP_DERIVATIVE).getSecondLeg()
        .getNthPayment(0);
    ...
    final DoublesPair[] sensi = { DoublesPair.of(secondLeg.getReferenceEndTime(),
        1 / indexStartvalue * 1.0 / 10.0 * Math.pow(estimatedPriceIndex / indexStartvalue, 1.0 / 10.0 - 1.0)) };
    ...
    AssertSensitivityObjects.assertEquals("Zero-coupon inflation DiscountingMethod: presentValueCurveSensitivity ", parSpreadSensitivity,
        parSpreadSensitivityCalculated, 10e-8);
  }
  ...
  public void presentValueNoNotional() {
    ...
    assertEquals("Zero-coupon inflation DiscountingMethod: Present value", pvExpected, pv.getAmount(ZERO_COUPON_NO.getCurrency()),
        TOLERANCE_PV);
  }
  ...
  public void netAmountNoNotional() {
    ...
    assertEquals("Zero-coupon inflation DiscountingMethod: net amount", naExpected, pv.getAmount(ZERO_COUPON_NO.getCurrency()),
        TOLERANCE_PV);
  }
  ...
  public void presentValueWithNotional() {
    ...
    assertEquals("Zero-coupon inflation DiscountingMethod: Present value", pvExpected, pv.getAmount(ZERO_COUPON_WITH.getCurrency()),
        TOLERANCE_PV);
  }
  ...
  public void netAmountWithNotional() {
    ...
    assertEquals("Zero-coupon inflation DiscountingMethod: net amount", pvExpected, pv.getAmount(ZERO_COUPON_WITH.getCurrency()),
        TOLERANCE_PV);
  }
  ...
  public void presentValueMarketSensitivityMethodVsCalculatorNoNotional() {
    final MultipleCurrencyInflationSensitivity pvcisMethod = METHOD.presentValueCurveSensitivity(ZERO_COUPON_NO,
  ...
  public void presentValueMarketSensitivityMethodVsCalculatorWithNotional() {
    final MultipleCurrencyInflationSensitivity pvcisMethod = METHOD.presentValueCurveSensitivity(ZERO_COUPON_WITH,
        MARKET.getInflationProvider());
