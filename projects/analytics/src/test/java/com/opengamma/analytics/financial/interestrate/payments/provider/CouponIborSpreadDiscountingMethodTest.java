...
public class CouponIborSpreadDiscountingMethodTest {
  ...
  private static final ZonedDateTime END_DATE = ScheduleCalculator.getAdjustedDate(START_DATE, EURIBOR3M, CalendarAdapter.of(CALENDAR));
  ...
  private static final CouponIborSpreadDefinition CPN_IBOR_SPREAD_DEFINITION = CouponIborSpreadDefinition.from(START_DATE, END_DATE,
      ACCURAL, NOTIONAL, EURIBOR3M, SPREAD, CalendarAdapter.of(CALENDAR));
  private static final CouponIborDefinition CPN_IBOR_DEFINITION = CouponIborDefinition.from(START_DATE, END_DATE, ACCURAL, NOTIONAL,
  ...
  public void presentValue() {
    ...
    final double forward = PROVIDER.getSimplyCompoundForwardRate(EURIBOR3M, CPN_IBOR_SPREAD.getFixingPeriodStartTime(),
        CPN_IBOR_SPREAD.getFixingPeriodEndTime(),
        CPN_IBOR_SPREAD.getFixingAccrualFactor());
    final double pv = NOTIONAL * CPN_IBOR_SPREAD.getPaymentYearFraction() * (forward + SPREAD)
        * PROVIDER.getDiscountFactor(CPN_IBOR_SPREAD.getCurrency(), CPN_IBOR_SPREAD.getPaymentTime());
    final CurrencyAmount pvExpected = CurrencyAmount.of(EURIBOR3M.getCurrency(), pv);
    assertEquals(pvExpected.getAmount(), pvComputed.getAmount(EUR), TOLERANCE_PV);
    ...
    assertEquals(pvIbor.plus(pvFixed).getAmount(EUR), pvComputed.getAmount(EUR),
        TOLERANCE_PV);
  }
   ...
   */
  @Test
  public void presentValueNoSpreadPositiveNotional() {
    ...
    final double forward = PROVIDER.getSimplyCompoundForwardRate(EURIBOR3M, CPN_IBOR_SPREAD.getFixingPeriodStartTime(),
        CPN_IBOR_SPREAD.getFixingPeriodEndTime(),
        CPN_IBOR_SPREAD.getFixingAccrualFactor());
    final double pvExpected = -NOTIONAL * CPN_IBOR_SPREAD.getPaymentYearFraction() * forward
        * PROVIDER.getDiscountFactor(CPN_IBOR_SPREAD.getCurrency(), CPN_IBOR_SPREAD.getPaymentTime());
    assertEquals(pvExpected, pvComputed.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void presentValueMethodVsCalculator() {
    ...
    assertEquals(pvMethod.getAmount(EUR), pvCalculator.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void presentValueCurveSensitivity() {
    ...
    AssertSensitivityObjects.assertEquals("", pvcsExpected, pvcsComputed,
        TOLERANCE_PV);
  }
  ...
  public void presentValueMarketQuoteSensitivity() {
    final CouponIborSpreadDefinition cpnSpreadPDefinition = CouponIborSpreadDefinition.from(START_DATE, END_DATE, ACCURAL, NOTIONAL,
        EURIBOR3M, SPREAD + SHIFT, CalendarAdapter.of(CALENDAR));
    final CouponIborSpreadDefinition cpnSpreadMDefinition = CouponIborSpreadDefinition.from(START_DATE, END_DATE, ACCURAL, NOTIONAL,
        EURIBOR3M, SPREAD - SHIFT, CalendarAdapter.of(CALENDAR));
    ...
    assertEquals(mqsExpected, mqsComputed,
        TOLERANCE_PV_DELTA);
  }
