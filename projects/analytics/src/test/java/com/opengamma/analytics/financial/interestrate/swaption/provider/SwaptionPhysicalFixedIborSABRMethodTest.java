...
public class SwaptionPhysicalFixedIborSABRMethodTest {
  ...
  /**
   * Tests present value with respect to a hard-coded value. Tests against the explicit formula. Tests long/short parity and
  ...
  public void testPresentValue() {
    ...
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: implied volatility", volatility,
        METHOD_SWPT_SABR.impliedVolatility(SWAPTION_LONG_PAYER, SABR_MULTICURVES), TOLERANCE_PV);
    ...
    // Long/Short parity
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: presentValue", priceLongPayer.getAmount(EUR), -priceShortPayer.getAmount(EUR),
        TOLERANCE_PV);
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: presentValue", priceLongReceiver.getAmount(EUR), -priceShortReceiver.getAmount(EUR),
        TOLERANCE_PV);
    ...
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: presentValue", priceSwapPayer.getAmount(EUR),
        priceLongPayer.getAmount(EUR) + priceShortReceiver.getAmount(EUR), TOLERANCE_PV);
    assertEquals("SwaptionPhysicalFixedIborSABRMethod: presentValue", priceSwapReceiver.getAmount(EUR),
        priceLongReceiver.getAmount(EUR) + priceShortPayer.getAmount(EUR), TOLERANCE_PV);
  }
  ...
  public void testPresentValueConventionArbitrage() {
    ...
    final IndexSwap index360 = new IndexSwap(EUR1YEURIBOR6M.getFixedLegPeriod(), DayCounts.ACT_360, EURIBOR6M, ANNUITY_TENOR,
    ...
    final SwapFixedIborDefinition swap360 = SwapFixedIborDefinition.from(SETTLEMENT_DATE, index360, NOTIONAL, rate360, true,
        CalendarAdapter.of(CALENDAR));
    final SwaptionPhysicalFixedIborDefinition swaption360Definition = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE, swap360, true,
        IS_LONG);
    final SwaptionPhysicalFixedIbor swaption360 = swaption360Definition.toDerivative(REFERENCE_DATE);
    final double rate365 = 0.0365;
    final IndexSwap index365 = new IndexSwap(EUR1YEURIBOR6M.getFixedLegPeriod(), DayCounts.ACT_365, EURIBOR6M, ANNUITY_TENOR,
        CalendarAdapter.of(CALENDAR));
    final SwapFixedIborDefinition swap365 = SwapFixedIborDefinition.from(SETTLEMENT_DATE, index365, NOTIONAL, rate365, true,
        CalendarAdapter.of(CALENDAR));
    final SwaptionPhysicalFixedIborDefinition swaption365Definition = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE, swap365, true,
        IS_LONG);
  ...
  public void presentValueCurveSensitivity() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_SS_C.calculateSensitivity(SWAPTION_SHORT_RECEIVER, SABR_MULTICURVES,
  ...
  public void presentValueCurveSensitivityMethodVsCalculator() {
    final MultipleCurrencyMulticurveSensitivity pvcsMethod = METHOD_SWPT_SABR.presentValueCurveSensitivity(SWAPTION_LONG_PAYER,
        SABR_MULTICURVES);
  ...
  public void presentValueSABRSensitivity() {
    ...
    final PresentValueSABRSensitivityDataBundle pvsLongPayer = METHOD_SWPT_SABR.presentValueSABRSensitivity(SWAPTION_LONG_PAYER,
        SABR_MULTICURVES);
    PresentValueSABRSensitivityDataBundle pvsShortPayer = METHOD_SWPT_SABR.presentValueSABRSensitivity(SWAPTION_SHORT_PAYER,
        SABR_MULTICURVES);
    // Long/short parity
  ...
  public void presentValueSABRSensitivityMethodVsCalculator() {
    final PresentValueSABRSensitivityDataBundle pvssMethod = METHOD_SWPT_SABR.presentValueSABRSensitivity(SWAPTION_LONG_PAYER,
        SABR_MULTICURVES);
  ...
  public void presentValueSABRHullWhiteMonteCarlo() {
    ...
    assertEquals("Swaption Physical SABR: Present value using Hull-White by Monte Carlo", pvSABR.getAmount(EUR), pvMC.getAmount(EUR),
        2.5E+4);
  }
  ...
  // SwapFixedIborDefinition swapDefinition = SwapFixedIborDefinition.from(settleDate, underlyingTenor, USD6MLIBOR3M, notional,
  // strikes[loopstrike], true);
  ...
  public void performance() {
    ...
    System.out.println("SwaptionPhysicalFixedIborSABRMethodTest: " + nbTest + " physical swaptions SABR (price+delta+vega separately): "
    ...
    endTime = System.currentTimeMillis();
    System.out.println("SwaptionPhysicalFixedIborSABRMethodTest: " + nbTest + " physical swaptions SABR (price+delta+vega together): "
        + (endTime - startTime) + " ms");
    ...
    endTime = System.currentTimeMillis();
    System.out.println(
        "SwaptionPhysicalFixedIborSABRMethodTest: " + nbTest + " physical swaptions SABR (price): " + (endTime - startTime) + " ms");
    ...
    endTime = System.currentTimeMillis();
    System.out.println("SwaptionPhysicalFixedIborSABRMethodTest: " + nbTest2 + " physical swaptions SABR + Hull-White Monte Carlo: "
        + (endTime - startTime) + " ms");
