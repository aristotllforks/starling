...
public class SwaptionCashFixedIborHullWhiteMethodTest {
  ...
  // Calculator
  private static final SwaptionCashFixedIborHullWhiteNumericalIntegrationMethod METHOD_HW_INTEGRATION = SwaptionCashFixedIborHullWhiteNumericalIntegrationMethod
      .getInstance();
  private static final SwaptionCashFixedIborHullWhiteApproximationMethod METHOD_HW_APPROXIMATION = SwaptionCashFixedIborHullWhiteApproximationMethod
      .getInstance();
  ...
  private static final double TOLERANCE_PV_DELTA = 1.0E+4; // Testing note: Sensitivity is for a movement of 1. 1E+2 = 1 cent for a 1 bp
                                                           // move.
  ...
  public void longShortParity() {
    ...
    assertEquals("Swaption cash - Hull-White - present value - long/short parity", pvLong.getAmount(EUR), -pvShort.getAmount(EUR),
        TOLERANCE_PV);
  }
  ...
  public void scaling() {
    ...
    final SwapFixedIborDefinition scaledSwapDefinition = SwapFixedIborDefinition.from(SETTLEMENT_DATE, CMS_INDEX, scale * NOTIONAL, RATE,
        FIXED_IS_PAYER, CalendarAdapter.of(CALENDAR));
    final SwaptionCashFixedIborDefinition scaledSwaptionDefinition = SwaptionCashFixedIborDefinition.from(EXPIRY_DATE, scaledSwapDefinition,
        true, IS_LONG);
    ...
    assertEquals("Swaption cash - Hull-White - present value - scaling", scale * pvOriginal.getAmount(EUR), pvScaled.getAmount(EUR),
        TOLERANCE_PV);
  }
  ...
  public void comparison() {
    ...
    assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration",
        pvPayerLongExplicit.getAmount(EUR) / NOTIONAL * bp1, pvPayerLongIntegration.getAmount(EUR) / NOTIONAL
            * bp1,
        4.0E-1);
    ...
    assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration",
        pvPayerShortExplicit.getAmount(EUR) / NOTIONAL * bp1, pvPayerShortIntegration.getAmount(EUR) / NOTIONAL
            * bp1,
        4.0E-1);
    ...
    assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration",
        pvReceiverLongExplicit.getAmount(EUR) / NOTIONAL * bp1, pvReceiverLongIntegration.getAmount(EUR)
            / NOTIONAL * bp1,
        5.0E-1);
    ...
    assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration",
        pvReceiverShortExplicit.getAmount(EUR) / NOTIONAL * bp1, pvReceiverShortIntegration.getAmount(EUR)
            / NOTIONAL * bp1,
        5.0E-1);
  }
  ...
  public void errorAnalysis() {
    ...
    for (int loopstrike = 0; loopstrike < strikeRel.length; loopstrike++) {
      final SwapFixedIborDefinition swapStrikePayerDefinition = SwapFixedIborDefinition.from(SETTLEMENT_DATE, CMS_INDEX, bp1,
          forward + strikeRel[loopstrike], FIXED_IS_PAYER, CalendarAdapter.of(CALENDAR));
      final SwaptionCashFixedIborDefinition swaptionStrikePayerDefinition = SwaptionCashFixedIborDefinition.from(EXPIRY_DATE,
          swapStrikePayerDefinition, true, IS_LONG);
      ...
      assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration", pvPayerApproximation[loopstrike],
          pvPayerIntegration[loopstrike], errorLimit);
      final SwapFixedIborDefinition swapStrikeReceiverDefinition = SwapFixedIborDefinition.from(SETTLEMENT_DATE, CMS_INDEX, bp1,
          forward + strikeRel[loopstrike], !FIXED_IS_PAYER, CalendarAdapter.of(CALENDAR));
      final SwaptionCashFixedIborDefinition swaptionStrikeReceiverDefinition = SwaptionCashFixedIborDefinition.from(EXPIRY_DATE,
          swapStrikeReceiverDefinition, false, IS_LONG);
      ...
      assertEquals("Swaption cash - Hull-White - present value - explicit/numerical integration", pvReceiverApproximation[loopstrike],
          pvReceiverIntegration[loopstrike], errorLimit);
    }
  }
  ...
  public void presentValueCurveSensitivity() {
    final MultipleCurrencyParameterSensitivity pvpsExact = PS_HW_C.calculateSensitivity(SWAPTION_RECEIVER_SHORT, HW_MULTICURVES,
