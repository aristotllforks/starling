...
public class BondFuturesOptionMarginSecurityBlackExpLogMoneynessMethodTest {
  ...
  private static final BondFuturesOptionMarginSecurityDefinition CALL_BOBL_116_DEFINITION = new BondFuturesOptionMarginSecurityDefinition(
      BOBLM4_DEFINITION,
      LAST_TRADING_DATE_OPT, EXPIRY_DATE_OPT, STRIKE_116, IS_CALL);
  private static final BondFuturesOptionMarginSecurityDefinition PUT_BOBL_116_DEFINITION = new BondFuturesOptionMarginSecurityDefinition(
      BOBLM4_DEFINITION,
      LAST_TRADING_DATE_OPT, EXPIRY_DATE_OPT, STRIKE_116, !IS_CALL);
  ...
  private static final BlackBondFuturesExpLogMoneynessProviderDiscount BLACK_FLAT_BNDFUT = new BlackBondFuturesExpLogMoneynessProviderDiscount(
      ISSUER_SPECIFIC_MULTICURVES, BLACK_SURFACE, LEGAL_ENTITY_GERMANY);
  /** Methods and calculators */
  private static final BondFuturesOptionMarginSecurityBlackBondFuturesMethod METHOD_OPT = BondFuturesOptionMarginSecurityBlackBondFuturesMethod
      .getInstance();
  ...
  public void putCallParity() {
    ...
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: put call parity price", priceCallComputed - pricePutComputed,
        priceFutures - STRIKE_116, TOLERANCE_RATE);
  }
  ...
  public void priceBlackSensitivity() {
    ...
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: Black parameters sensitivity", vega,
        vegaComputed.getSensitivity().toSingleValue(), TOLERANCE_DELTA);
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: Black parameters sensitivity", 1,
        vegaComputed.getSensitivity().getMap().size());
    final Entry<Triple<Double, Double, Double>, Double> point = vegaComputed.getSensitivity().getMap().entrySet().iterator().next();
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: Black parameters sensitivity", CALL_BOBL_116.getExpirationTime(),
        point.getKey().getFirst(), TOLERANCE_RATE);
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: Black parameters sensitivity",
        CALL_BOBL_116.getUnderlyingFuture().getTradingLastTime() - CALL_BOBL_116.getExpirationTime(), point.getKey().getSecond(),
        TOLERANCE_RATE);
    assertEquals("BondFuturesOptionMarginSecurityBlackFlatMethod: Black parameters sensitivity", CALL_BOBL_116.getStrike(),
        point.getKey().getThird(), TOLERANCE_RATE);
  }
  ...
  public void theoreticalTheta() {
    ...
    final double expiry = CALL_BOBL_116.getExpirationTime();
    final double volatility = BLACK_SURFACE.getZValue(expiry, logmoney);
    final double rate = -Math.log(ISSUER_SPECIFIC_MULTICURVES.getMulticurveProvider().getDiscountFactor(CALL_BOBL_116.getCurrency(),
        CALL_BOBL_116.getExpirationTime())) / CALL_BOBL_116.getExpirationTime();
    final double thetaCallExpected = BlackFormulaRepository.theta(priceFutures, STRIKE_116, CALL_BOBL_116.getExpirationTime(), volatility,
        CALL_BOBL_116.isCall(), rate);
  ...
  private static final double[] VALUE_EXP_MON = new double[] { 0.50, 0.50, 0.45, 0.49, 0.49, 0.44, 0.47, 0.47, 0.42, 0.48, 0.48, 0.43, 0.51,
      0.51, 0.46 };
  ...
  private static final InterpolatedDoublesSurface BLACK_SURFACE_EXP_LOGMONEY = InterpolatedDoublesSurface.fromGrid(EXP, LOGMONEY,
      VALUE_EXP_MON,
      INTERPOLATOR_TIMESQUARE_LINEAR_2D); // Replace LINEAR by "SQUARE_LINEAR" (TBC)
  private static final InterpolatedDoublesSurface BLACK_SURFACE_LOGMONEY_EXP = InterpolatedDoublesSurface.fromGrid(LOGMONEY, EXP,
      VALUE_MON_EXP,
      INTERPOLATOR_LINEAR_TIMESQUARE_2D);
  private static final InterpolatedDoublesSurface BLACK_SURFACE_EXP_LOGMONEY_VAR = InterpolatedDoublesSurface.fromGrid(EXP, LOGMONEY,
      VALUE_EXP_MON_VAR,
  ...
  public void interpolation() {
    ...
    for (int loopmon = 0; loopmon < nbMonTest; loopmon++) {
      for (int loopexp = 0; loopexp < nbExpTest; loopexp++) {
        ...
        assertEquals("Time square interpolation: change of order " + loopmon + " - " + loopexp, intExpMon, Math.sqrt(intVar / exp),
            TOLERANCE_VOL);
      }
    }
  }
