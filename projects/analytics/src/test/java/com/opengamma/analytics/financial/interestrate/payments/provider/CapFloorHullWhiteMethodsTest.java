...
public class CapFloorHullWhiteMethodsTest {
  ...
  private static final ZonedDateTime FIXING_DATE = ScheduleCalculator.getAdjustedDate(REFERENCE_DATE, TENOR_FIXING,
      GENERATOR_EUR1YEURIBOR6M.getBusinessDayConvention(), TARGET, GENERATOR_EUR1YEURIBOR6M.isEndOfMonth());
  private static final ZonedDateTime START_DATE = ScheduleCalculator.getAdjustedDate(FIXING_DATE, GENERATOR_EUR1YEURIBOR6M.getSpotLag(),
      TARGET);
  private static final ZonedDateTime PAYMENT_DATE = ScheduleCalculator.getAdjustedDate(START_DATE, TENOR_COUPON,
      GENERATOR_EUR1YEURIBOR6M.getBusinessDayConvention(), TARGET, GENERATOR_EUR1YEURIBOR6M.isEndOfMonth());
  ...
  static {
    for (int i = 0; i < NB_STRIKE; i++) {
      CAP_CMS_DEFINITION[i] = CapFloorCMSDefinition.from(PAYMENT_DATE, START_DATE, PAYMENT_DATE, ACCRUAL_FACTOR, NOTIONAL,
          SWAP_EUR10Y, STRIKE[i], true, CalendarAdapter.of(TARGET));
      CAP_CMS[i] = (CapFloorCMS) CAP_CMS_DEFINITION[i].toDerivative(REFERENCE_DATE);
    }
  }

  private static final CapFloorCMSHullWhiteNumericalIntegrationMethod METHOD_NI = CapFloorCMSHullWhiteNumericalIntegrationMethod
      .getInstance();
  ...
  public void presentValueApproximation() {
    for (int i = 0; i < NB_STRIKE; i++) {
      final MultipleCurrencyAmount pvNumericalIntegration = METHOD_NI.presentValue(CAP_CMS[i], HW_MULTICURVES);
      final MultipleCurrencyAmount pvApproximation = METHOD_APP.presentValue(CAP_CMS[i], HW_MULTICURVES);
      assertEquals(pvNumericalIntegration.getAmount(Currency.EUR), pvApproximation.getAmount(Currency.EUR), NOTIONAL
          * ACCRUAL_FACTOR * BP1);
