...
public class SimpleCapFloorMaker {
  ...
  /**
   * A simplistic (i.e. for testing) method for making a cap/floor. The simplified assumptions are:
   * <ul>
   ...
   * @param start
   *          Integer for the start of the cap. A spot starting cap with have start = 0, while one starting in one year, based on 3M-Libor
   *          will have start = 4.
   * @param end
   *          Integer for the end of the cap. A 10 year (spot staring) cap based on 6M Libor will have end = 20.
   * @param strike
   *          The strike.
   * @param capFloor
   ...
   */
  public static CapFloor makeCap(final Currency ccy, final IborIndex index, final int start, final int end, final double strike,
      final boolean capFloor) {
    return new CapFloor(makeCapletStrip(ccy, index, start, end, strike, capFloor));
  }

  private static CapFloorIbor[] makeCapletStrip(final Currency ccy, final IborIndex index, final int start, final int end,
      final double strike, final boolean capFloor) {
