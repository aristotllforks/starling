...
/**
 * Tests related to the pricing methods for Ibor coupon in the discounting method with data.
 */
@Test(groups = TestGroup.UNIT)
public class CouponIborDiscountingMethodTest {
  ...
  private static final CouponIborDefinition CPN_IBOR_DEFINITION = CouponIborDefinition.from(ACCRUAL_START_DATE, ACCRUAL_END_DATE,
      ACCRUAL_FACTOR, NOTIONAL, EURIBOR3M, CalendarAdapter.of(CALENDAR));
  ...
  public void presentValueMarketDiscount() {
    ...
    final double forward = MULTICURVES.getSimplyCompoundForwardRate(EURIBOR3M, CPN_IBOR.getFixingPeriodStartTime(),
        CPN_IBOR.getFixingPeriodEndTime(), CPN_IBOR.getFixingAccrualFactor());
    ...
    assertEquals("CouponIborDiscountingMarketMethod: present value", pvExpected, pvComputed.getAmount(EURIBOR3M.getCurrency()),
        TOLERANCE_PV);
  }
