...
public class BondConstantSpreadHorizonCalculatorTest {
  ...
  /** Holiday calendar */
  private static final WorkingDayCalendar CALENDAR = EmptyWorkingDayCalendar.INSTANCE;
  /** Issuer name */
  ...
  private static final InstrumentDerivativeVisitor<ParameterIssuerProviderInterface, MultipleCurrencyAmount> PV_CALCULATOR = PresentValueIssuerCalculator
      .getInstance();
  private static final HorizonCalculator<BondTransactionDefinition<?, ?>, IssuerProviderInterface, Double> CALCULATOR = BondConstantSpreadHorizonCalculator.INSTANCE;
  ...
  static {
    SECURITY = BondFixedSecurityDefinition.from(USD, REFERENCE_DATE.plusYears(10), REFERENCE_DATE, PAYMENT_TENOR, COUPON, SETTLEMENT_DAYS,
        CalendarAdapter.of(CALENDAR), DAY_COUNT, BUSINESS_DAY, YIELD_CONVENTION, IS_EOM,
        new LegalEntity(US_GOVT, US_GOVT, null, null, null));
    ...
    final Map<Currency, YieldAndDiscountCurve> discounting = Collections.<Currency, YieldAndDiscountCurve> singletonMap(USD,
        flatDiscounting);
    ...
    final Pair<Object, LegalEntityFilter<LegalEntity>> issuerKey = Pairs.<Object, LegalEntityFilter<LegalEntity>> of(US_GOVT,
        new LegalEntityShortName());
    final Map<Pair<Object, LegalEntityFilter<LegalEntity>>, YieldAndDiscountCurve> issuer = Collections
        .<Pair<Object, LegalEntityFilter<LegalEntity>>, YieldAndDiscountCurve> singletonMap(issuerKey, flatIssuer);
  ...
  public void testNullDefinition() {
    CALCULATOR.getTheta(null, HORIZON_DATE, FLAT_ISSUER_MULTICURVES, 1, CalendarAdapter.of(CALENDAR));
  ...
  public void testNullDate() {
    CALCULATOR.getTheta(TRANSACTION, null, FLAT_ISSUER_MULTICURVES, 1, CalendarAdapter.of(CALENDAR));
  }
  ...
  public void testNullData() {
    CALCULATOR.getTheta(TRANSACTION, HORIZON_DATE, null, 1, CalendarAdapter.of(CALENDAR));
  }
  ...
  public void testDaysForward1() {
    CALCULATOR.getTheta(TRANSACTION, HORIZON_DATE, FLAT_ISSUER_MULTICURVES, 2, CalendarAdapter.of(CALENDAR));
  }
  ...
  public void testConstantCurves() {
    ...
    MultipleCurrencyAmount actual = CALCULATOR.getTheta(TRANSACTION, HORIZON_DATE, FLAT_ISSUER_MULTICURVES, 1, CalendarAdapter.of(CALENDAR),
        REFERENCE_PRICE);
    ...
    actual = CALCULATOR.getTheta(TRANSACTION, HORIZON_DATE, FLAT_ISSUER_MULTICURVES, -1, CalendarAdapter.of(CALENDAR), REFERENCE_PRICE);
    assertMcaEquals(expected, actual);
    // bond prices should be approximately linear
    assertMcaEquals(HorizonCalculator.subtract(pvYesterday, pvToday), HorizonCalculator.subtract(pvToday, pvTomorrow), 1e-5);
  }
  ...
  private void assertMcaEquals(final MultipleCurrencyAmount expected, final MultipleCurrencyAmount actual) {
    assertMcaEquals(expected, actual, EPS);
  }
   ...
   *          The actual object
   */
  private void assertMcaEquals(final MultipleCurrencyAmount expected, final MultipleCurrencyAmount actual, final double eps) {
