...
public class CapFloorHullWhiteCalibrationObjectiveTest {
  ...
  private static final AnnuityCapFloorIborDefinition CAP_DEFINITION = AnnuityCapFloorIborDefinition.fromWithNoInitialCaplet(SETTLEMENT_DATE,
      MATURITY_DATE, NOTIONAL, EURIBOR3M, IS_PAYER, STRIKE,
  ...
  public void calibration() {
    ...
    final SuccessiveRootFinderHullWhiteCalibrationObjective objective = new SuccessiveRootFinderHullWhiteCalibrationObjective(hwParameters,
        EUR);
    final SuccessiveRootFinderHullWhiteCalibrationEngine<SABRCapProviderInterface> calibrationEngine = new SuccessiveRootFinderHullWhiteCalibrationEngine<>(
        objective);
    ...
    for (int i = 0; i < CAP.getNumberOfPayments(); i++) {
      pvSabr[i] = CAP.getNthPayment(i).accept(PVSCC, SABR_MULTICURVES);
      pvHw[i] = METHOD_CAP_HW.presentValue((CapFloorIbor) CAP.getNthPayment(i), objective.getHwProvider());
      assertEquals("Hull-White calibration: cap/floor " + i, pvSabr[i].getAmount(EUR), pvHw[i].getAmount(EUR),
          TOLERANCE_PV);
    }
  }
