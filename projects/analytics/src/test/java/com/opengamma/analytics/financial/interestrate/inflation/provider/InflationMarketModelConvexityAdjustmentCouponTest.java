...
public class InflationMarketModelConvexityAdjustmentCouponTest {
  ...
  private static final ZonedDateTime PAYMENT_DATE_YOY = ScheduleCalculator.getAdjustedDate(PRICING_DATE, Period.ofYears(1), BUSINESS_DAY,
      CALENDAR_EUR);
  private static final ZonedDateTime PAYMENT_DATE_YOY_MINUS1 = ScheduleCalculator.getAdjustedDate(PRICING_DATE, Period.ofYears(0),
      BUSINESS_DAY, CALENDAR_EUR);
  private static final ZonedDateTime PAYMENT_DATE = ScheduleCalculator.getAdjustedDate(PRICING_DATE, COUPON_TENOR, BUSINESS_DAY,
      CALENDAR_EUR);
  private static final ZonedDateTime PAYMENT_DATE_MINUS1 = ScheduleCalculator.getAdjustedDate(PRICING_DATE, Period.ofYears(9), BUSINESS_DAY,
      CALENDAR_EUR);

  private final static double DELTA = 10e-8;

  private static final CouponInflationYearOnYearMonthlyDefinition YEAR_ON_YEAR_DEFINITION = CouponInflationYearOnYearMonthlyDefinition.from(
      PAYMENT_DATE_MINUS1, PAYMENT_DATE, NOTIONAL,
  ...
  private static final InterpolatedDoublesCurve BLACK_SURF = InterpolatedDoublesCurve.from(new double[] { 1.0, 5.0, 10.0 },
      new double[] { .2, .2, .2 }, INTERPOLATOR_LINEAR);
  private static final BlackFlatCapFloorParameters BLACK_PARAM = new BlackFlatCapFloorParameters(BLACK_SURF, EURIBOR6M);
  private static final double[] INFLATION_TIME = new double[] { YEAR_ON_YEAR.getNaturalPaymentStartTime(),
      YEAR_ON_YEAR.getNaturalPaymentEndTime() };
  ...
  private static final Curve<Double, Double> PRICE_INDEX_RATE_CORRELATION = InterpolatedDoublesCurve.from(new double[] { 1.0, 10.0, 30.0 },
      new double[] { -0.15, -0.15, -0.15 }, INTERPOLATOR_LINEAR);

  private static final InflationConvexityAdjustmentParameters INFLATION_PARAM = new InflationConvexityAdjustmentParameters(INFLATION_TIME,
      ATM_VOLATILITY, LIBOR_CORRELATION, PRICE_INDEX_CORRELATION,
      PRICE_INDEX_RATE_CORRELATION, PRICE_INDEX_EUR);
  private static final InflationConvexityAdjustmentProviderDiscount PARAMETER_INTERFACE = new InflationConvexityAdjustmentProviderDiscount(
      INFLATION_PROVIDER, INFLATION_PARAM, BLACK_PARAM);
  ...
  public void valueForZeroCouponMonthly() {
    ...
    for (int i = 0; i < 20; i++) {
      zeroCouponDefinitions[i] = CouponInflationZeroCouponMonthlyDefinition.from(PRICING_DATE, PAYMENT_DATE, NOTIONAL, PRICE_INDEX_EUR,
          MONTH_LAG, 3 + i, true);
      ...
      pricesWithConvexity[i] = METHOD_ZC_MONTHLY_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE)
          .getAmount(zeroCoupons[i].getCurrency());
      pricesWithoutConvexity[i] = METHOD_ZC_MONTHLY.presentValue(zeroCoupons[i], INFLATION_PROVIDER)
          .getAmount(zeroCoupons[i].getCurrency());
      assertEquals("convexity adjustment for inflation zero coupon monthly", pricesWithConvexity[i],
          pricesWithoutConvexity[i] * convexitysAdjustments[i], DELTA);
    }
  }
  ...
  public void valueForZeroCouponMonthlyYearly() {
    ...
    final double[] pricesWithoutConvexity = new double[20];
    for (int i = 0; i < 20; i++) {
      zeroCouponDefinitions[i] = CouponInflationZeroCouponMonthlyDefinition.from(PRICING_DATE, PAYMENT_DATE.plusYears(i), NOTIONAL,
          PRICE_INDEX_EUR, MONTH_LAG, 8, true);
      zeroCoupons[i] = (CouponInflationZeroCouponMonthly) zeroCouponDefinitions[i].toDerivative(PRICING_DATE, cpiTimeSerie);
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getZeroCouponConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_ZC_MONTHLY_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE)
          .getAmount(zeroCoupons[i].getCurrency());
  ...
  public void valueForZeroCouponInterpolation() {
    ...
    final double[] pricesWithoutConvexity = new double[20];
    for (int i = 0; i < 20; i++) {
      zeroCouponDefinitions[i] = CouponInflationZeroCouponInterpolationDefinition.from(PAYMENT_DATE_MINUS1, PAYMENT_DATE, NOTIONAL,
          PRICE_INDEX_EUR, MONTH_LAG, 3 + i, true);
      zeroCoupons[i] = zeroCouponDefinitions[i].toDerivative(PRICING_DATE);
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getZeroCouponConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_ZC_INTERPOLATION_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE)
          .getAmount(zeroCoupons[i].getCurrency());
      pricesWithoutConvexity[i] = METHOD_ZC_INTERPOLATION.presentValue(zeroCoupons[i], INFLATION_PROVIDER)
          .getAmount(zeroCoupons[i].getCurrency());
      assertEquals("convexity adjustment for inflation zero coupon interpolation", pricesWithConvexity[i],
          pricesWithoutConvexity[i] * convexitysAdjustments[i], DELTA);
    }
  }
  ...
  public void valueForYearOnYearInterpolation() {
    ...
    final double[] pricesWithoutConvexity = new double[20];
    for (int i = 0; i < 20; i++) {
      zeroCouponDefinitions[i] = CouponInflationYearOnYearInterpolationDefinition.from(PAYMENT_DATE_YOY_MINUS1.plusYears(i),
          PAYMENT_DATE_YOY.plusYears(i), NOTIONAL, PRICE_INDEX_EUR, MONTH_LAG,
          MONTH_LAG,
          true);
      zeroCoupons[i] = zeroCouponDefinitions[i].toDerivative(PRICING_DATE);
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getYearOnYearConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_YOY_INTERPOLATION_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE)
          .getAmount(zeroCoupons[i].getCurrency());
      pricesWithoutConvexity[i] = METHOD_YOY_INTERPOLATION.presentValue(zeroCoupons[i], INFLATION_PROVIDER)
          .getAmount(zeroCoupons[i].getCurrency());
      assertEquals("convexity adjustment for inflation year on year interpolation", pricesWithConvexity[i],
          pricesWithoutConvexity[i] * convexitysAdjustments[i], DELTA);
    }
  }
  ...
  public void valueForYearOnYearMonthly() {
    ...
    final double[] pricesWithoutConvexity = new double[20];
    for (int i = 0; i < 20; i++) {
      zeroCouponDefinitions[i] = CouponInflationYearOnYearMonthlyDefinition.from(PAYMENT_DATE_YOY_MINUS1.plusYears(i),
          PAYMENT_DATE_YOY.plusYears(i), NOTIONAL, PRICE_INDEX_EUR, MONTH_LAG, MONTH_LAG,
          true);
      zeroCoupons[i] = zeroCouponDefinitions[i].toDerivative(PRICING_DATE);
      convexitysAdjustments[i] = CONVEXITY_ADJUSTMENT_FUNCTION.getYearOnYearConvexityAdjustment(zeroCoupons[i], PARAMETER_INTERFACE);
      pricesWithConvexity[i] = METHOD_YOY_MONTHLY_CONVEXITY_ADJUSTMENT.presentValue(zeroCoupons[i], PARAMETER_INTERFACE)
          .getAmount(zeroCoupons[i].getCurrency());
      pricesWithoutConvexity[i] = METHOD_YOY_MONTHLY.presentValue(zeroCoupons[i], INFLATION_PROVIDER)
          .getAmount(zeroCoupons[i].getCurrency());
      assertEquals("convexity adjustment for inflation year on year monthly", pricesWithConvexity[i],
          pricesWithoutConvexity[i] * convexitysAdjustments[i], DELTA);
    }
  }
}
