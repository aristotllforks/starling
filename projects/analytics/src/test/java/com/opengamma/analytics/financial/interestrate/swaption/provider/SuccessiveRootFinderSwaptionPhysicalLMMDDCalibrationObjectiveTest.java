...
/**
 * Tests related to the calibration engine for LMM DD calibration to European swaptions (by calibrating volatility parameters with a common
...
public class SuccessiveRootFinderSwaptionPhysicalLMMDDCalibrationObjectiveTest {
  ...
  static {
    for (int i = 0; i < SWAP_TENOR_YEAR.length; i++) {
      SWAP_INDEX[i] = new IndexSwap(EUR1YEURIBOR3M, Period.ofYears(SWAP_TENOR_YEAR[i]));
      SWAP_PAYER_DEFINITION[i] = SwapFixedIborDefinition.from(SETTLEMENT_DATE, SWAP_INDEX[i], NOTIONAL, RATE, FIXED_IS_PAYER,
          CalendarAdapter.of(CALENDAR));
      SWAPTION_LONG_PAYER_DEFINITION[i] = SwaptionPhysicalFixedIborDefinition.from(EXPIRY_DATE, SWAP_PAYER_DEFINITION[i],
  ...
  public void calibration() {
    final LiborMarketModelDisplacedDiffusionParameters lmmParameters = TestsDataSetLiborMarketModelDisplacedDiffusion.createLMMParameters(
        REFERENCE_DATE,
    ...
    for (int i = 0; i < SWAP_TENOR_YEAR.length; i++) {
      ...
      pvLmm[i] = METHOD_LMM.presentValue(SWAPTION_LONG_PAYER[i], objective.getLMMProvider());
      assertEquals("Hull-White calibration: swaption " + i, pvSabr[i].getAmount(EUR), pvLmm[i].getAmount(EUR),
          TOLERANCE_PV);
    }
  }
  ...
  public void performance() {
    ...
    for (int i = 0; i < nbTest; i++) {
      ...
      final CalibrationEngineWithCalculators<SABRSwaptionProviderInterface> calibrationEngine = new SuccessiveRootFinderLMMDDCalibrationEngine<>(
          objective);
      for (int j = 0; j < SWAP_TENOR_YEAR.length; j++) {
