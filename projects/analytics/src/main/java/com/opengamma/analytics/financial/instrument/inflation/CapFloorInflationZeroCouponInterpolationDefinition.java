...
/**
 * Class describing an zero-coupon inflation caplet/floorlet were the inflation figure are interpolated between monthly inflation figures. The notional is
 * positive for long the option and negative for short the option. The start index value is known when the coupon is traded/issued. The index for a given month
 * is given in the yield curve and in the time series on the first of the month. The pay-off is [(Index_End / Index_Start - 1)-((1+strike)^T-1)]^{+}
 **/
public class CapFloorInflationZeroCouponInterpolationDefinition extends CouponInflationDefinition implements CapFloor {
  ...
  public CapFloorInflationZeroCouponInterpolationDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      ...
      final int conventionalMonthLag, final int monthLag, final int maturity, final ZonedDateTime[] referenceStartDates,
      final ZonedDateTime[] referenceEndDates, final double weight,
   ...
   */
  public static CapFloorInflationZeroCouponInterpolationDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime paymentDate,
      final double notional,
      final IndexPrice priceIndex, final int conventionalMonthLag, final int monthlag, final int maturity, final ZonedDateTime lastKnownFixingDate,
   ...
   * @return The cap/floor
   */
  public static CapFloorInflationZeroCouponInterpolationDefinition from(final CouponInflationZeroCouponInterpolationDefinition couponInflation,
      final ZonedDateTime lastKnownFixingDate,
      final int maturity, final double strike, final boolean isCap) {
    Validate.notNull(couponInflation, "coupon Ibor");
    return new CapFloorInflationZeroCouponInterpolationDefinition(couponInflation.getCurrency(), couponInflation.getPaymentDate(),
        couponInflation.getAccrualStartDate(),
  ...
  public Coupon toDerivative(final ZonedDateTime date, final DoubleTimeSeries<ZonedDateTime> priceIndexTimeSeries) {
    ...
    return new CapFloorInflationZeroCouponInterpolation(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), getPriceIndex(),
        lastKnownFixingTime, fixedStartIndex, referenceEndTime,
