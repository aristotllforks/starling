...
public class ISDAApproxCDSPricingMethod {
  ...
  private static class Timeline {
    ...
    Timeline(final double[] timePoints, final double[] discountFactors) {
  ...
  /**
   * Calculate the up-front charge of for a CDS according to the ISDA model, given ISDA representations of the discount curve and a flat par
   * spread from the market.
   *
   * The pricing, step-in and settlement dates need to be passed in as they are used to build the bootstrap instrument.
   *
   * @param cds
   *          The CDS to be valued
   * @param discountCurve
   *          The discount curve
   * @param flatSpread
   *          The flat par spread from the market
   * @param cleanPrice
   *          Whether the price is clean (true) or dirty (false)
   * @param pricingDate
   *          The pricing date
   * @param stepinDate
   *          The step-in date
   ...
   */
  public double calculateUpfrontCharge(final ISDACDSDerivative cds, final ISDACurve discountCurve, final double flatSpread,
      final boolean cleanPrice, final ZonedDateTime pricingDate,
    ...
    final double stepinDiscountFactor = offsetStepinTime > 0.0 ? discountCurve.getDiscountFactor(offsetStepinTime)
        : discountCurve.getDiscountFactor(0.0);
    ...
    final ISDACDSDerivative bootstrapCDS = bootstrapCDSDefinition.toDerivative(pricingDate, stepinDate, settlementDate,
        cds.getDiscountCurveName(), cds.getSpreadCurveName());
    ...
    return valueCDS(cds, hazardRateCurve, paymentTimeline, accrualTimeline, contingentTimeline, offsetStepinTime, stepinDiscountFactor,
        settlementDiscountFactor, cleanPrice);
  }

  /**
   * Calculate the up-front charge of for a CDS according to the ISDA model, given ISDA representations of the discount curve and hazard
   * rate function.
   *
   * @param cds
   *          The CDS to be valued
   ...
   * @return The clean or dirty price of the CDS contract, depending on the cleanPrice flag
   */
  public double calculateUpfrontCharge(final ISDACDSDerivative cds, final ISDACurve discountCurve, final ISDACurve hazardRateCurve,
   ...
   *
   * In this case most parameters of the bootstrap instrument, particularly the maturity date, are the same as the instrument being priced.
   *
   * @param cds
   *          The CDS being priced
   * @param parSpread
   *          Flat par spread from the market
  ...
  private ISDACDSDefinition makeBootstrapCDSDefinition(final ISDACDSDerivative cds, final double parSpread, final Calendar calendar) {
    ...
    final ISDACDSPremiumDefinition premiumDefinition = ISDACDSPremiumDefinition.from(startDate, maturity, cds.getCouponFrequency(),
    ...
    return new ISDACDSDefinition(startDate, maturity, premiumDefinition, notional, parSpread, cds.getRecoveryRate(),
        cds.isAccrualOnDefault(), payOnDefault, protectStart, cds.getCouponFrequency(),
   ...
   *
   * Note the clean price can be found by subtracting accrued premium at the pricing point from the dirty price. This value is available in
   * the ISDACDSDerivative object ({@link ISDACDSDerivative#getAccruedInterest()}).
   *
   * @param cds
   *          The CDS being valued
   * @param hazardRateCurve
   *          A representation of the hazard rate function
   * @param paymentTimeline
   *          The timeline for fee payments
   * @param accrualTimeline
   *          The timeline used to compute accrual on default for the fee leg
   * @param contingentTimeline
   *          The timeline used to price the contingent leg
   * @param stepinTime
   *          The step-in time
   * @param stepinDiscountFactor
   *          The discount factor at the step-in time
   * @param settlementDiscountFactor
   *          The discount factor at the settlement time
   * @param cleanPrice
   *          Whether to produce a clean price (true) or a dirty price (false)
   * @return The value of the CDS contract, according to the ISDA model.
   */
  private double valueCDS(final ISDACDSDerivative cds, final ISDACurve hazardRateCurve, final Timeline paymentTimeline,
      final Timeline accrualTimeline, final Timeline contingentTimeline,
    ...
    final double feeLeg = valueFeeLeg(cds, paymentTimeline, accrualTimeline, hazardRateCurve, stepinTime, stepinDiscountFactor,
        settlementDiscountFactor);
   ...
   *
   * @param cds
   *          The CDS contract being priced
   * @param hazardRateCurve
   *          Curve describing the hazard rate function
   * @param stepinDate
   *          The step-in date
   * @return PV of the CDS premium leg
   */
  private double valueFeeLeg(final ISDACDSDerivative cds, final Timeline paymentTimeline, final Timeline accrualTimeline,
      final ISDACurve hazardRateCurve, final double stepinTime,
    ...
    for (int i = 0; i < coupons.length; i++) {
      ...
      if (cds.isAccrualOnDefault()) {
        ...
        result += valueFeeLegAccrualOnDefault(amount, accrualTimeline, hazardRateCurve, startIndex, endIndex, stepinTime,
            stepinDiscountFactor);
      }
    }
   ...
   *
   * @param amount
   *          Amount of premium that would be accrued over the entire period (in the case of no default)
   * @param timeline
   *          The accrual time line, covers the entire CDS (excluding past accrual periods), not just the accrual period being evaluated
   * @param hazardRateCurve
   *          Curve describing the hazard rate function
   * @param startIndex
   *          Index in to the time line for the start of the current accrual period
   * @param endIndex
   *          Index in to the time line for the end of the current accrual period
   * @param stepinTime
   *          Step-in time for the CDS contract
   * @param stepinDiscountFactor
   *          Associated discount factor for the step-in time
   * @return Accrual-on-default portion of PV for the accrual period
   */
  private double valueFeeLegAccrualOnDefault(final double amount, final Timeline timeline, final ISDACurve hazardRateCurve,
    ...
    for (int i = startIndex + 1; i <= endIndex; ++i) {
      ...
      valueForTimeStep = lambda * accrualRate * survival0 * discount0
          * ((t0 + 1.0 / lambdaFwdRate) / lambdaFwdRate
              - (t1 + 1.0 / lambdaFwdRate) / lambdaFwdRate * survival1 / survival0 * discount1 / discount0);
   ...
   *
   * @param cds
   *          The derivative object representing the CDS
   * @param hazardRateCurve
   *          Curve describing the hazard rate function
   * @return PV of the CDS default leg
   */
  private double valueContingentLeg(final ISDACDSDerivative cds, final Timeline contingentTimeline, final ISDACurve hazardRateCurve,
      final double settlementDiscountFactor) {
    ...
    final double value = cds.isPayOnDefault() ? valueContingentLegPayOnDefault(recoveryRate, contingentTimeline, hazardRateCurve)
        : valueContingentLegPayOnMaturity(recoveryRate, contingentTimeline,
            hazardRateCurve);
   ...
   *
   * @param recoveryRate
   *          Recovery rate of the CDS underlying asset
   * @param timeline
   *          The contingent leg time line, bounded by startTime and maturity
   * @param hazardRateCurve
   *          Curve describing the hazard rate function
   ...
   *
   * The resulting timeline object contains t values for every point on both curves, and optionally for the start and end time of every
   * accrual period. Discount factors for each time point are computed and stored. The timeline is truncated to the specified start and end
   * times (these time points are included in the timeline).
   *
   * @param cds
   *          The CDS being priced
   * @param discountCurve
   ...
   * @param endTime
   *          The last point on the timeline
   * @param includeSchedule
   *          Whether to include the CDS accrual period start/end times
   * @return The populated timeline object
   */
  private Timeline buildTimeline(final ISDACDSDerivative cds, final ISDACurve discountCurve, final ISDACurve hazardRateCurve,
  ...
  /**
   * Build a timeline based on a series of coupon payment dates. All payment dates are included, and the discount factors for those dates.
   *
   * @param cds
   *          Payment dates are extracted from the CDS premium and included in the timeline
   * @param discountCurve
   *          Discount factors for each payment date are taken from the discount curve
   * @return The populated timeline object
   */
