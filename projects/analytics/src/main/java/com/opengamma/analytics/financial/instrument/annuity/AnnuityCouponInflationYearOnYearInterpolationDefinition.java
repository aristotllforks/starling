...
public class AnnuityCouponInflationYearOnYearInterpolationDefinition extends AnnuityCouponDefinition<CouponInflationYearOnYearInterpolationDefinition> {
   ...
   * 
   * @param payments
   *          The InflationYearOnYearMonthly coupons.
   * @param calendar
   *          The calendar
   */
  ...
  /**
   * Year on year annuity (or Year on year coupon leg) constructor from standard description. The coupon are fixing in advance and payment in arrears. The month
   * lag is the conventional one.
   * 
   * @param priceIndex
   *          The price index.
   ...
   * @param paymentPeriod
   *          The payment period of the coupons.
   ...
   * @param payNotional
   *          Payer (true) / receiver (false) flag.
   * @param weightStart
   *          The weight on the first month index in the interpolation of the index at the denominator.
   * @param weightEnd
   *          The weight on the first month index in the interpolation of the index at the numerator.
  ...
  public static AnnuityCouponInflationYearOnYearInterpolationDefinition from(final IndexPrice priceIndex, final ZonedDateTime settlementDate,
    ...
    final ZonedDateTime[] paymentDates = ScheduleCalculator.getAdjustedDateSchedule(settlementDate, paymentPeriod, totalPeriod, true, false,
    ...
    coupons[0] = CouponInflationYearOnYearInterpolationDefinition.from(settlementDate, paymentDates[0], notional, priceIndex, conventionalMonthLag, payNotional,
        weightStart, weightEnd);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      coupons[loopcpn] = CouponInflationYearOnYearInterpolationDefinition.from(paymentDates[loopcpn - 1], paymentDates[loopcpn], notional, priceIndex,
          conventionalMonthLag, payNotional,
          weightStart, weightEnd);
    }
  ...
  /**
   * Year on year annuity (or Year on year coupon leg) constructor without weights (this calculation is done in the coupon). The coupon are fixing in advance
  ...
  public static AnnuityCouponInflationYearOnYearInterpolationDefinition from(final IndexPrice priceIndex, final ZonedDateTime settlementDate,
    ...
    final CouponInflationYearOnYearInterpolationDefinition[] coupons = new CouponInflationYearOnYearInterpolationDefinition[paymentDates.length];
    coupons[0] = CouponInflationYearOnYearInterpolationDefinition.from(settlementDate, paymentDates[0], notional, priceIndex, conventionalMonthLag, monthLag,
        payNotional);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      coupons[loopcpn] = CouponInflationYearOnYearInterpolationDefinition.from(paymentDates[loopcpn - 1], paymentDates[loopcpn], notional, priceIndex,
          conventionalMonthLag, monthLag, payNotional);
    }
    return new AnnuityCouponInflationYearOnYearInterpolationDefinition(coupons, calendar);
  }
