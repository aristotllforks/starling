...
/**
 * Class describing a Year on Year inflation coupon were the inflation figure are interpolated. The index for a given month is given in the yield curve and in
 * the time series on the first of the month. The pay-off is paymentYearFraction*(Index_End / Index_Start - X) + Margin with X=0 for notional payment and X=1
 * for no notional payment.
 */
public class CouponInflationYearOnYearInterpolationWithMarginDefinition extends CouponInflationDefinition implements CouponInflationWithMargin {
   ...
   *          The weight on the first month index in the interpolation of the index at the coupon end.
   */
  public CouponInflationYearOnYearInterpolationWithMarginDefinition(final double factor, final Currency currency, final ZonedDateTime paymentDate,
      ...
      final int monthLag, final ZonedDateTime[] referenceStartDate, final ZonedDateTime[] referenceEndDate, final boolean payNotional, final double weightStart,
   ...
   */
  public static CouponInflationYearOnYearInterpolationWithMarginDefinition from(final double factor, final ZonedDateTime accrualStartDate,
    ...
    return new CouponInflationYearOnYearInterpolationWithMarginDefinition(factor, priceIndex.getCurrency(), paymentDate, accrualStartDate, paymentDate, 1.0,
        notional, priceIndex,
  ...
  public static CouponInflationYearOnYearInterpolationWithMarginDefinition from(final double factor, final ZonedDateTime accrualStartDate,
      final ZonedDateTime paymentDate, final double notional,
      final IndexPrice priceIndex, final int conventionalMonthLag, final int monthLag, final boolean payNotional, final double weightStart,
      final double weightEnd) {
    final ZonedDateTime[] referenceStartDate = new ZonedDateTime[2];
    final ZonedDateTime[] referenceEndDate = new ZonedDateTime[2];
  ...
  public CouponInflationYearOnYearInterpolationWithMargin toDerivative(final ZonedDateTime date, final String... yieldCurveNames) {
    ...
    return new CouponInflationYearOnYearInterpolationWithMargin(_factor, getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), getPriceIndex(),
        referenceStartTime,
