...
/**
 * Class describing a fixed for OIS swap. Both legs are in the same currency. The payment dates on the fixed leg a slightly different from the FixedIbor swap
 * due to the lag in payment at the end of each coupon.
 */
public class SwapFixedONSimplifiedDefinition extends SwapDefinition {
  ...
  public static SwapFixedONSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate, final double notional,
    ...
    final AnnuityCouponONSimplifiedDefinition oisLeg = AnnuityCouponONSimplifiedDefinition.from(settlementDate, endFixingPeriodDate, notional, generator,
        !isPayer);
    final double sign = isPayer ? -1.0 : 1.0;
    final double notionalSigned = sign * notional;
    return from(oisLeg, notionalSigned, fixedRate, generator.getOvernightCalendar());
   ...
   * @return The swap.
   */
  public static SwapFixedONSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate, final double notionalFixed,
      final double notionalOIS,
      final GeneratorSwapFixedON generator, final double fixedRate, final boolean isPayer) {
    final AnnuityCouponONSimplifiedDefinition oisLeg = AnnuityCouponONSimplifiedDefinition.from(settlementDate, endFixingPeriodDate, notionalOIS, generator,
        !isPayer);
    final double sign = isPayer ? -1.0 : 1.0;
  ...
  /**
   * Swap builder from the financial details. On the fixed leg, the accrual dates are not the same as the payment dates. There is a difference due to the
   * settlement lag required on the OIS coupons.
   * 
   * @param settlementDate
   *          The settlement date.
   * @param tenorAnnuity
   *          The swap tenor.
   * @param tenorCoupon
   *          The coupons tenor. The tenor is the same on the fixed and OIS legs.
   * @param notional
   *          The notional.
   * @param index
   *          The OIS index.
   ...
   *          The fixed leg rate.
   * @param isPayer
   *          The flag indicating if the fixed leg is paying (true) or receiving (false).
   * @param settlementDays
   *          The number of days between last fixing of each coupon and the coupon payment (also called spot lag).
   * @param businessDayConvention
   *          The business day convention to compute the end date of the coupon.
   * @param dayCount
   *          The day count convention for the OIS
   * @param isEOM
   ...
   * @return The annuity.
   */
  public static SwapFixedONSimplifiedDefinition from(final ZonedDateTime settlementDate, final Period tenorAnnuity, final Period tenorCoupon,
      final double notional, final IndexON index,
      final double fixedRate, final boolean isPayer, final int settlementDays, final BusinessDayConvention businessDayConvention, final DayCount dayCount,
      final boolean isEOM,
   ...
   *          The settlement date.
   * @param maturityDate
   *          The maturity date. This is the last date of the fixing period, not the last payment date.
   * @param frequency
   ...
   * @return The annuity.
   */
  public static SwapFixedONSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate, final Period frequency,
      final double notional, final IndexON index,
      final double fixedRate, final boolean isPayer, final int settlementDays, final BusinessDayConvention businessDayConvention, final DayCount dayCount,
      final boolean isEOM,
      final Calendar calendar) {
