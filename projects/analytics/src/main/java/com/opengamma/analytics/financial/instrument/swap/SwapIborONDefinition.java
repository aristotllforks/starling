...
/**
 * Class describing an Ibor for overnight swap. Both legs are in the same currency. The payment dates on the ibor leg a slightly different from the FixedIbor
...
public class SwapIborONDefinition extends SwapDefinition {
   ...
   *          The settlement date.
   * @param tenorSwap
   *          The total tenor of the swap.
   * @param generator
   *          The Ibor/ON generator.
   * @param notional
   *          The swap notional.
   * @param spread
   *          The spread on the Ibor leg.
   * @param isPayer
   *          The flag indicating if first leg (Ibor) is payer (true) or receiver (false).
   * @param iborCalendar
   *          The holiday calendar for the ibor leg.
   * @return The swap.
   */
  public static SwapIborONDefinition from(final ZonedDateTime settlementDate, final Period tenorSwap, final GeneratorSwapIborON generator,
   ...
   * @return The swap.
   */
  public static SwapIborONDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate, final double notional,
      final GeneratorSwapIborON generator, final double spread,
   ...
   * @return The swap.
   */
  public static SwapIborONDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate, final double notionalFixed,
   ...
   * @return The swap.
   */
  public static SwapIborONDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate, final NotionalProvider notionalFixed,
  ...
  private static SwapIborONDefinition from(final AnnuityCouponONDefinition oisLeg, final double notionalSigned, final IborIndex indexIbor, final double spread,
    ...
    for (int loopcpn = 0; loopcpn < oisLeg.getNumberOfPayments(); loopcpn++) {
      final ZonedDateTime fixingDate = ScheduleCalculator.getAdjustedDate(oisLeg.getNthPayment(loopcpn).getAccrualStartDate(), indexIbor.getSpotLag(),
          calendar);
      cpnIbor[loopcpn] = new CouponIborSpreadDefinition(oisLeg.getCurrency(), oisLeg.getNthPayment(loopcpn).getPaymentDate(),
          oisLeg.getNthPayment(loopcpn).getAccrualStartDate(), oisLeg
              .getNthPayment(loopcpn).getAccrualEndDate(),
          oisLeg.getNthPayment(loopcpn).getPaymentYearFraction(), notionalSigned, fixingDate, indexIbor, spread, calendar);
    }
    return new SwapIborONDefinition(new AnnuityCouponIborSpreadDefinition(cpnIbor, calendar), oisLeg);
  }

  private static SwapIborONDefinition from(final AnnuityCouponONDefinition oisLeg, final NotionalProvider notional, final IborIndex indexIbor,
      final double spread,
    ...
    for (int loopcpn = 0; loopcpn < oisLeg.getNumberOfPayments(); loopcpn++) {
      ...
      cpnIbor[loopcpn] = new CouponIborSpreadDefinition(oisLeg.getCurrency(), oisLeg.getNthPayment(loopcpn).getPaymentDate(),
          ...
          fixingDate, indexIbor, spread, calendar);
    }
    return new SwapIborONDefinition(new AnnuityCouponIborSpreadDefinition(cpnIbor, calendar), oisLeg);
  }
  ...
  public Swap<Coupon, Coupon> toDerivative(final ZonedDateTime date, final String... yieldCurveNames) {
    ...
    final Annuity<? extends Coupon> oisLeg = (Annuity<? extends Coupon>) this.getOISLeg().toDerivative(date,
        new String[] { yieldCurveNames[0], yieldCurveNames[2] });
