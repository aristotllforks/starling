...
implements InstrumentDefinitionWithData<FederalFundsFutureTransaction, DoubleTimeSeries<ZonedDateTime>[]> {
   ...
   */
  public FederalFundsFutureTransactionDefinition(final FederalFundsFutureSecurityDefinition underlyingFuture, final long quantity,
  ...
  public FederalFundsFutureTransaction toDerivative(final ZonedDateTime date, final String... yieldCurveNames) {
    throw new UnsupportedOperationException(
        "The method toDerivative of FederalFundsFutureTransactionDefinition does not support the two argument method "
            + "(without ON fixing and margin price data).");
  }
   ...
   *          The reference date.
   * @param data
   *          Two time series. The first one with the ON index fixing; the second one with the future closing (margining) prices.
   * @param yieldCurveNames
   *          The yield curve names The last closing price at a date strictly before "date" is used as last closing.
   * @return The derivative form {@inheritDoc}
   * @deprecated Use the method that does not take yield curve names
   */
  @Deprecated
  @Override
   ...
   * @param data
   *          One or two time series. The first one is mandatory (not null) and contains with the ON index fixing. The second one should be present if the
   *          reference "dateTime" is not hte trading date; the it contains the the future settlement (margining) prices. The last closing price at a date
   *          strictly before "date" is used as last closing.
