...
public class BondCapitalIndexedSecurityDefinition<C extends CouponInflationDefinition> extends BondSecurityDefinition<C, C>
  ...
  /**
   * Constructor of the Capital inflation indexed bond. The repo type is set to an empty string and the legal entity only contains the issuer name.
   * 
   * @param nominal
   *          The nominal annuity.
   * @param coupon
   ...
   */
  public BondCapitalIndexedSecurityDefinition(final AnnuityDefinition<C> nominal, final AnnuityDefinition<C> coupon, final double indexStartDate,
      final int exCouponDays, final int settlementDays, final Calendar calendar, final DayCount dayCount, final YieldConvention yieldConvention,
      final boolean isEOM, final int monthLag, final String issuer) {
  ...
  public BondCapitalIndexedSecurityDefinition(final AnnuityDefinition<C> nominal, final AnnuityDefinition<C> coupon, final double indexStartDate,
      final int exCouponDays, final int settlementDays, final Calendar calendar, final DayCount dayCount, final YieldConvention yieldConvention,
      final boolean isEOM, final int monthLag, final LegalEntity issuer) {
  ...
  public BondCapitalIndexedSecurityDefinition(final AnnuityDefinition<C> nominal, final AnnuityDefinition<C> coupon, final double indexStartDate,
      final int exCouponDays, final int settlementDays, final Calendar calendar, final DayCount dayCount, final YieldConvention yieldConvention,
      final boolean isEOM, final int monthLag, final String issuer, final String repoType) {
    this(nominal, coupon, indexStartDate, exCouponDays, settlementDays, calendar, dayCount, yieldConvention, isEOM, monthLag,
  ...
  public BondCapitalIndexedSecurityDefinition(final AnnuityDefinition<C> nominal, final AnnuityDefinition<C> coupon, final double indexStartDate,
      final int exCouponDays, final int settlementDays, final Calendar calendar, final DayCount dayCount, final YieldConvention yieldConvention,
      final boolean isEOM, final int monthLag, final LegalEntity issuer, final String repoType) {
    ...
    _monthLag = monthLag;
    _indexStartValue = indexStartDate;
    _couponPerYear = (int) Math
        .round(TimeCalculator.getTimeBetween(coupon.getNthPayment(1).getAccrualStartDate(), coupon.getNthPayment(1).getAccrualEndDate()) / 365);
    _isEOM = isEOM;
    _dayCount = dayCount;
    _priceIndex = nominal.getNthPayment(0).getPriceIndex();
  }
   ...
   *          The bond notional.
   * @param realRate
   *          The bond nominal rate (annual).
   * @param businessDay
   *          The business day convention to compute the payment days.
   * @param settlementDays
   *          Standard number of days between trade date and trade settlement. Used for clean price and yield computation.
   * @param calendar
   *          The payment calendar.
   ...
   * @return The bond.
   */
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> fromMonthly(final IndexPrice priceIndex,
      final int monthLag, final ZonedDateTime startDate, final double indexStartValue, final ZonedDateTime maturityDate, final Period couponPeriod,
      final double notional, final double realRate, final BusinessDayConvention businessDay, final int settlementDays, final Calendar calendar,
      final DayCount dayCount, final YieldConvention yieldConvention, final boolean isEOM, final String issuer) {
    return fromMonthly(priceIndex, monthLag, startDate, indexStartValue, maturityDate, couponPeriod, notional, realRate, businessDay, settlementDays, calendar,
        dayCount, yieldConvention, isEOM, new LegalEntity(null, issuer, null, null, null));
  }
  ...
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> fromMonthly(final IndexPrice priceIndex,
    ...
    final CouponInflationZeroCouponMonthlyGearingDefinition nominalPayment = CouponInflationZeroCouponMonthlyGearingDefinition.from(startDate, maturityDate,
        notional, priceIndex, indexStartValue, monthLag, monthLag, true, 1.0);
    ...
    // Coupon construction
    final long couponPerYear = 12 / couponPeriod.toTotalMonths();
    ...
    coupons[0] = CouponInflationZeroCouponMonthlyGearingDefinition.from(paymentDates[0], startDate, paymentDatesUnadjusted[0], notional, priceIndex,
        indexStartValue, monthLag, monthLag, true, realRate / couponPerYear);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      coupons[loopcpn] = CouponInflationZeroCouponMonthlyGearingDefinition.from(paymentDates[loopcpn], paymentDatesUnadjusted[loopcpn - 1],
          paymentDatesUnadjusted[loopcpn], notional, priceIndex, indexStartValue, monthLag, monthLag, true, realRate / couponPerYear);
    }
    ...
    return new BondCapitalIndexedSecurityDefinition<>(nominalAnnuity, couponAnnuity, indexStartValue, DEFAULT_EX_COUPON_DAYS, settlementDays, calendar,
        dayCount, yieldConvention, isEOM, monthLag, issuer);
  }
   ...
   *          The bond first coupon date. Used for short/long first coupon.
   * @param maturityDate
   *          The bond maturity date.
   * @param couponPeriod
   *          The period between coupon payments.
   ...
   */
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> fromMonthly(final IndexPrice priceIndex,
      final int monthLag, final ZonedDateTime startDate, final double indexStartValue, final ZonedDateTime firstCouponDate, final ZonedDateTime maturityDate,
      final Period couponPeriod, final double notional, final double realRate, final BusinessDayConvention businessDay, final int settlementDays,
    ...
    return fromMonthly(priceIndex, monthLag, startDate, indexStartValue, firstCouponDate, maturityDate, couponPeriod, notional, realRate, businessDay,
        settlementDays, calendar, dayCount, yieldConvention, isEOM, new LegalEntity(null, issuer, null, null, null));
  }
  ...
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> fromMonthly(final IndexPrice priceIndex,
    ...
    coupons[0] = CouponInflationZeroCouponMonthlyGearingDefinition.from(ScheduleCalculator.getAdjustedDate(firstCouponDate, 0, calendar), startDate,
        firstCouponDate, notional, priceIndex, indexStartValue, monthLag, monthLag, true, realRate / couponPerYear);
    coupons[1] = CouponInflationZeroCouponMonthlyGearingDefinition.from(paymentDates[0], firstCouponDate, paymentDatesUnadjusted[0], notional, priceIndex,
        indexStartValue, monthLag, monthLag, true, realRate / couponPerYear);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
  ...
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponMonthlyGearingDefinition> fromMonthly(final IndexPrice priceIndex,
      final int monthLag, final ZonedDateTime startDate, final double indexStartValue, final ZonedDateTime firstCouponDate, final ZonedDateTime maturityDate,
      final Period couponPeriod, final double realRate, final BusinessDayConvention businessDay, final int settlementDays, final Calendar calendar,
      final DayCount dayCount, final YieldConvention yieldConvention, final boolean isEOM, final LegalEntity issuer) {
    return fromMonthly(priceIndex, monthLag, startDate, indexStartValue, firstCouponDate, maturityDate, couponPeriod, 1.0, realRate, businessDay,
        settlementDays, calendar, dayCount, yieldConvention, isEOM, issuer);
  }
   ...
   * @return The bond.
   */
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponInterpolationGearingDefinition> fromInterpolation(final IndexPrice priceIndex,
      final int monthLag, final ZonedDateTime startDate, final double indexStartValue, final ZonedDateTime maturityDate, final Period couponPeriod,
      final double notional, final double realRate, final BusinessDayConvention businessDay, final int settlementDays, final Calendar calendar,
      final DayCount dayCount, final YieldConvention yieldConvention, final boolean isEOM, final String issuer) {
    return fromInterpolation(priceIndex, monthLag, startDate, indexStartValue, maturityDate, couponPeriod, notional, realRate, businessDay, settlementDays,
  ...
  public static BondCapitalIndexedSecurityDefinition<CouponInflationZeroCouponInterpolationGearingDefinition> fromInterpolation(final IndexPrice priceIndex,
      ...
      final DayCount dayCount, final YieldConvention yieldConvention, final boolean isEOM, final LegalEntity issuer) {
    // Nominal construction
    final CouponInflationZeroCouponInterpolationGearingDefinition nominalPayment = CouponInflationZeroCouponInterpolationGearingDefinition.from(startDate,
        maturityDate, notional, priceIndex, indexStartValue, monthLag, monthLag, true, 1.0);
    ...
    coupons[0] = CouponInflationZeroCouponInterpolationGearingDefinition.from(paymentDates[0], startDate, paymentDatesUnadjusted[0], notional, priceIndex,
        indexStartValue, monthLag, monthLag, true, realRate / couponPerYear);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
  ...
  /**
   * @param date
   *          The date to use when converting to the derivative form, not null
  ...
  public BondCapitalIndexedSecurity<Coupon> toDerivative(final ZonedDateTime date, final ZonedDateTime settlementDate,
    ...
    if (getExCouponDays() != 0) {
      final ZonedDateTime exDividendDate = ScheduleCalculator.getAdjustedDate(couponDefinition.getNthPayment(0).getPaymentDate(), -getExCouponDays(),
          getCalendar());
    ...
    final double nbDaysPeriod = couponDefinition.getNthPayment(0).getAccrualEndDate().getDayOfYear()
        - couponDefinition.getNthPayment(0).getAccrualStartDate().getDayOfYear();
    ...
    if (settlement instanceof CouponInflationZeroCouponInterpolationGearing) {
      ...
      referenceStartDates[1] = referenceStartDates[0].plusMonths(1).with(TemporalAdjusters.lastDayOfMonth()).withHour(0).withMinute(0).withSecond(0)
          .withNano(0);
    ...
    return new BondCapitalIndexedSecurity<>(nominalStandard, couponStandard, settlementTime, accruedInterest, factorToNextCoupon, ratioPeriodToNextCoupon,
        _yieldConvention, _couponPerYear, settlement, _indexStartValue, lasKnownIndexFixing, lasKnownFixingTime, indexRatio, getIssuerEntity());
  }
  ...
  public double accruedInterest(final ZonedDateTime date) {
    ...
    final double accruedInterest = AccruedInterestCalculator.getAccruedInterest(getDayCount(), couponIndex, nbCoupon, previousAccrualDate, date,
        nextAccrualDate, currentCoupon.getFactor() * getCouponPerYear(), getCouponPerYear(), isEOM()) * getCoupons().getNthPayment(couponIndex).getNotional();
