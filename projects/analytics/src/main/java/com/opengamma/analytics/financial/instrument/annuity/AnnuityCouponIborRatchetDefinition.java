...
/**
 * A wrapper class for a AnnuityDefinition containing mainly CouponIborRatchetDefinition. The first coupon should be a CouponFixedDefinition or a
 * CouponIborGearingDefinition.
 */
public class AnnuityCouponIborRatchetDefinition extends AnnuityCouponDefinition<CouponDefinition> {
   ...
   *          The Ibor index.
   * @param isPayer
   *          The payer (true) / receiver (false) flag.
   * @param firstCouponFixedRate
   *          The rate of the first coupon.
   * @param mainCoefficients
   *          The coefficients of the main payment (before floor and cap). Array of length 3. The first coefficient is the previous coupon factor, the second is
   *          the Ibor factor and the third is the additive term.
   ...
   *          The coefficients of the cap. Array of length 3. The first coefficient is the previous coupon factor, the second is the Ibor factor and the third
   *          is the additive term.
   * @param calendar
   *          The holiday calendar for the ibor leg.
   * @return The annuity.
   */
  public static AnnuityCouponIborRatchetDefinition withFirstCouponFixed(final ZonedDateTime settlementDate, final Period annuityTenor, final double notional,
      final IborIndex index,
      final boolean isPayer, final double firstCouponFixedRate, final double[] mainCoefficients, final double[] floorCoefficients,
      final double[] capCoefficients,
      final Calendar calendar) {
    final ZonedDateTime[] paymentDates = ScheduleCalculator.getAdjustedDateSchedule(settlementDate, annuityTenor, index.getTenor(),
        index.getBusinessDayConvention(), calendar,
        index.isEndOfMonth());
  ...
  /**
   * Build a Ratchet Ibor annuity with Ibor gearing first coupon. The factor and spread of the first coupon are the one of the main part of the Ratchet. All the
   * Ratchet coupons have the same coefficients.
   * 
   * @param settlementDate
   *          The annuity settlement date.
   * @param annuityTenor
   ...
   * @return The annuity.
   */
  public static AnnuityCouponIborRatchetDefinition withFirstCouponIborGearing(final ZonedDateTime settlementDate, final Period annuityTenor,
    ...
    final CouponDefinition[] coupons = new CouponDefinition[paymentDates.length];
    final double notionalSign = notional * (isPayer ? -1.0 : 1.0);
    coupons[0] = CouponIborGearingDefinition.from(settlementDate, paymentDates[0],
        index.getDayCount().getDayCountFraction(settlementDate, paymentDates[0], calendar),
  ...
  public AnnuityCouponIborRatchet toDerivative(final ZonedDateTime date, final DoubleTimeSeries<ZonedDateTime> indexFixingTS, final String... yieldCurveNames) {
    ...
    for (int loopcpn = 1; loopcpn < getNumberOfPayments(); loopcpn++) {
      if (isFixed[loopcpn]) {
        ...
        final double cpnMain = cpnRatchet.getMainCoefficients()[0] * fixedCpn.get(loopcpn - 1) + cpnRatchet.getMainCoefficients()[1] * ibor
            + cpnRatchet.getMainCoefficients()[2];
        final double cpnFloor = cpnRatchet.getFloorCoefficients()[0] * fixedCpn.get(loopcpn - 1) + cpnRatchet.getFloorCoefficients()[1] * ibor
            + cpnRatchet.getFloorCoefficients()[2];
        final double cpnCap = cpnRatchet.getCapCoefficients()[0] * fixedCpn.get(loopcpn - 1) + cpnRatchet.getCapCoefficients()[1] * ibor
            + cpnRatchet.getCapCoefficients()[2];
    ...
    if (!date.isAfter(cpn0.getPaymentDate())) {
      if (isFixed[0]) {
        cpnList.add(new CouponFixed(getCurrency(), TimeCalculator.getTimeBetween(date, cpn0.getPaymentDate()), yieldCurveNames[0],
            cpn0.getPaymentYearFraction(), cpn0.getNotional(), fixedCpn.get(0),
    ...
    for (int loopcpn = 1; loopcpn < getNumberOfPayments(); loopcpn++) {
      ...
      if (!date.isAfter(getNthPayment(loopcpn).getPaymentDate())) {
        if (isFixed[loopcpn]) {
          cpnList.add(new CouponFixed(getCurrency(), TimeCalculator.getTimeBetween(date, cpn.getPaymentDate()), yieldCurveNames[0],
              cpn.getPaymentYearFraction(), cpn.getNotional(), fixedCpn
                  .get(loopcpn),
              cpn.getAccrualStartDate(), cpn.getAccrualEndDate()));
        } else {
  ...
  public AnnuityCouponIborRatchet toDerivative(final ZonedDateTime date, final DoubleTimeSeries<ZonedDateTime> indexFixingTS) {
    ...
    if (!date.isAfter(cpn0.getPaymentDate())) {
      if (isFixed[0]) {
        cpnList.add(new CouponFixed(getCurrency(), TimeCalculator.getTimeBetween(date, cpn0.getPaymentDate()), cpn0.getPaymentYearFraction(),
            cpn0.getNotional(), fixedCpn.get(0),
            cpn0.getAccrualStartDate(), cpn0.getAccrualEndDate()));
      } else { // CouponIborGearingDefinition
    ...
    for (int loopcpn = 1; loopcpn < getNumberOfPayments(); loopcpn++) {
      ...
      if (!date.isAfter(getNthPayment(loopcpn).getPaymentDate())) {
        if (isFixed[loopcpn]) {
          cpnList.add(new CouponFixed(getCurrency(), TimeCalculator.getTimeBetween(date, cpn.getPaymentDate()), cpn.getPaymentYearFraction(), cpn.getNotional(),
              fixedCpn
                  .get(loopcpn),
              cpn.getAccrualStartDate(), cpn.getAccrualEndDate()));
        } else {
