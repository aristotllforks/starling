...
public class CouponONDefinition extends CouponDefinition implements InstrumentDefinitionWithData<Payment, DoubleTimeSeries<ZonedDateTime>> {
   ...
   */
  public static CouponONDefinition from(final IndexON index, final ZonedDateTime settlementDate, final Period tenor, final double notional,
  ...
  public Coupon toDerivative(final ZonedDateTime valZdt, final DoubleTimeSeries<ZonedDateTime> indexFixingTimeSeries, final String... yieldCurveNames) {
    ...
    if (fixedPeriod < _fixingPeriodDate.length - 1) { // Some OIS period left
      ...
      if (fixedPeriod < _fixingPeriodDate.length - 1) { // More OIS period left
        ...
        final CouponON cpn = new CouponON(getCurrency(), paymentTime, yieldCurveNames[0], getPaymentYearFraction(), getNotional(), _index,
