...
public class CouponIborDefinition extends CouponFloatingDefinition {
  ...
  /**
   * Constructor of a Ibor-like floating coupon from the coupon details and the Ibor index. The payment currency is the index currency. The fixing dates and
   * accrual factors are inferred from the index.
   * 
   * @param currency
   *          The coupon currency.
   * @param paymentDate
   ...
   *          The holiday calendar for the ibor index.
   */
  public CouponIborDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      final ZonedDateTime accrualEndDate, final double paymentAccrualFactor,
  ...
  public CouponIborDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      final ZonedDateTime accrualEndDate, final double paymentAccrualFactor,
      final double notional, final ZonedDateTime fixingDate, final ZonedDateTime fixingPeriodStartDate, final ZonedDateTime fixingPeriodEndDate,
   ...
   */
  public static CouponIborDefinition from(final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate, final ZonedDateTime accrualEndDate,
   ...
   * @return The coupon.
   */
  public static CouponIborDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime accrualEndDate, final double accrualFactor,
  ...
  /**
   * Builder of a coupon from the accrual dates and the index. The fixing dates and accrual factor are calculated using the index. The payment date is the end
   * accrual date.
   * 
   * @param accrualStartDate
   *          Start date of the accrual period.
   ...
   * @return The coupon.
   */
  public static CouponIborDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime accrualEndDate, final double notional,
  ...
  public static CouponIborDefinition from(final double notional, final ZonedDateTime fixingDate, final IborIndex index, final Calendar calendar) {
    ...
    final ZonedDateTime fixingPeriodEndDate = ScheduleCalculator.getAdjustedDate(fixingPeriodStartDate, index.getTenor(), index.getBusinessDayConvention(),
    ...
    return new CouponIborDefinition(index.getCurrency(), fixingPeriodEndDate, fixingPeriodStartDate, fixingPeriodEndDate, fixingPeriodAccrualFactor, notional,
        fixingDate, index,
        calendar);
  }

  /**
   * Builder of Ibor-like coupon from an underlying coupon, the fixing date and the index. The fixing period dates are deduced from the index and the fixing
   * date.
   * 
   * @param coupon
   *          Underlying coupon.
  ...
  public static CouponIborDefinition from(final CouponDefinition coupon, final ZonedDateTime fixingDate, final IborIndex index, final Calendar calendar) {
    ...
    ArgumentChecker.notNull(fixingDate, "fixing date");
    ArgumentChecker.notNull(index, "index");
    return new CouponIborDefinition(index.getCurrency(), coupon.getPaymentDate(), coupon.getAccrualStartDate(), coupon.getAccrualEndDate(),
   ...
   * 
   * @param coupon
   *          The coupon with spread.
   * @return The ibor coupon.
   */
  public static CouponIborDefinition from(final CouponIborSpreadDefinition coupon) {
    ArgumentChecker.notNull(coupon, "coupon");
    return new CouponIborDefinition(coupon.getCurrency(), coupon.getPaymentDate(), coupon.getAccrualStartDate(), coupon.getAccrualEndDate(),
        coupon.getPaymentYearFraction(), coupon.getNotional(),
  ...
  public CouponIborDefinition withNotional(final double notional) {
    return new CouponIborDefinition(getCurrency(), getPaymentDate(), getAccrualStartDate(), getAccrualEndDate(), getPaymentYearFraction(), notional,
        getFixingDate(), _fixingPeriodStartDate,
  ...
  public Coupon toDerivative(final ZonedDateTime dateTime, final String... yieldCurveNames) {
    ...
    return new CouponIbor(getCurrency(), paymentTime, fundingCurveName, getPaymentYearFraction(), getNotional(), fixingTime, getIndex(), fixingPeriodStartTime,
        fixingPeriodEndTime,
   ...
   * Ibor with spread is returned. If the fixing date is strictly after the conversion date, a coupon Ibor is returned. All the comparisons are between dates
   * without time. {@inheritDoc}
   * 
   * @deprecated Use the method that does not take yield curve names
   */
  @Deprecated
  ...
  public Coupon toDerivative(final ZonedDateTime dateTime) {
    ...
    final double fixingPeriodStartTime = TimeCalculator.getTimeBetween(dateTime, getFixingPeriodStartDate());
    final double fixingPeriodEndTime = TimeCalculator.getTimeBetween(dateTime, getFixingPeriodEndDate());
    return new CouponIbor(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), fixingTime, getIndex(), fixingPeriodStartTime,
        fixingPeriodEndTime,
  ...
  public String toString() {
    return super.toString() + " *Ibor coupon* Index = " + _index + ", Fixing period = [" + _fixingPeriodStartDate + " - " + _fixingPeriodEndDate + " - "
