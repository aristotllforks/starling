...
/**
 * Class describing a caplet/floorlet on CMS spread. The notional is positive for long the option and negative for short the option. The pay-off of the
 * instrument is a cap/floor on the difference between the first CMS rate and the second CMS rate. Both swaps underlying the CMS need to have the same
 * settlement date.
 */
public class CapFloorCMSSpreadDefinition extends CouponFloatingDefinition implements CapFloor {
   ...
   * @param underlyingSwap1
   *          The swap underlying the first CMS.
   * @param cmsIndex1
   *          The index associated to the first CMS.
   * @param underlyingSwap2
   *          The swap underlying the second CMS.
   * @param cmsIndex2
   *          The index associated to the second CMS.
   * @param strike
   ...
   */
  public CapFloorCMSSpreadDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      final ZonedDateTime accrualEndDate, final double accrualFactor,
      final double notional, final ZonedDateTime fixingDate, final SwapFixedIborDefinition underlyingSwap1, final IndexSwap cmsIndex1,
      final SwapFixedIborDefinition underlyingSwap2,
    ...
    ArgumentChecker.isTrue(
        underlyingSwap1.getFixedLeg().getNthPayment(0).getAccrualStartDate() == underlyingSwap2.getFixedLeg().getNthPayment(0).getAccrualStartDate(),
        "Identic settlement date");
  ...
  /**
   * Builder of a CMS spread cap/floor. The fixing date is computed from the start accrual date with the Ibor index spot lag (fixing in advance). The underlying
   * swaps are computed from that date and the CMS indexes.
   * 
   * @param paymentDate
   *          Coupon payment date.
   * @param accrualStartDate
   ...
   *          The cap (true) /floor (false) flag.
   * @param calendar1
   *          The holiday calendar of the first leg.
   * @param calendar2
   *          The holiday calendar of the second leg.
   * @return The CMS spread cap/floor.
   */
  public static CapFloorCMSSpreadDefinition from(final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate, final ZonedDateTime accrualEndDate,
      final double accrualFactor,
      final double notional, final IndexSwap cmsIndex1, final IndexSwap cmsIndex2, final double strike, final boolean isCap, final Calendar calendar1,
    ...
    return new CapFloorCMSSpreadDefinition(cmsIndex1.getIborIndex().getCurrency(), paymentDate, accrualStartDate, accrualEndDate, accrualFactor, notional,
        fixingDate, underlyingSwap1, cmsIndex1,
  ...
  public Coupon toDerivative(final ZonedDateTime date, final String... yieldCurveNames) {
    ...
    return new CapFloorCMSSpread(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), fixingTime, swap1, _cmsIndex1, swap2, _cmsIndex2,
        settlementTime, _strike, _isCap,
        fundingCurveName);
  }
