...
/**
 * Class describing a OIS-like floating compounded coupon. The pay-off of this coupon is : $$ \begin{equation*} \left(\prod_{i=1}^n (1+ r_i)^\delta_i \right)
 * \end{equation*} $$ r_i the overnight rate for the fixing date t_i (or between t_i and t_{i+1}) \delta_i is the accrued between t_i and t_{i+1} using the
 * appropriate day-count, for example if we use business/252 as daycounter \delta_i=1/252.
 *
...
public class CouponONCompoundedDefinition extends CouponDefinition implements InstrumentDefinitionWithData<Payment, DoubleTimeSeries<ZonedDateTime>> {
   ...
   */
  public CouponONCompoundedDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      final ZonedDateTime accrualEndDate,
      final double paymentYearFraction, final double notional, final IndexON index, final ZonedDateTime fixingPeriodStartDate,
   ...
   *          The coupon settlement date.
   * @param tenor
   *          The coupon tenor.
   * @param notional
   *          The notional.
   * @param settlementDays
   *          The number of days between last fixing and the payment (also called spot lag).
   ...
   */
  public static CouponONCompoundedDefinition from(final IndexON index, final ZonedDateTime settlementDate, final Period tenor, final double notional,
   ...
   *          The notional.
   * @param settlementDays
   *          The number of days between last fixing date and the payment fate (also called payment lag).
   * @param calendar
   *          The holiday calendar for the overnight index.
   * @return The OIS coupon.
   */
  public static CouponONCompoundedDefinition from(final IndexON index, final ZonedDateTime settlementDate, final ZonedDateTime fixingPeriodEndDate,
    ...
    return new CouponONCompoundedDefinition(index.getCurrency(), paymentDate, settlementDate, fixingPeriodEndDate, paymentYearFraction, notional, index,
        settlementDate,
        fixingPeriodEndDate, calendar);
  }

  /**
   * Builder from financial details using genrator. The accrual and fixing start and end dates are the same. The day count for the payment is the same as the
   * one for the index. And with the same fixingAccrualFactor for each period. The payment date is adjusted by the publication lag and the settlement days.
   * 
   * @param generator
   *          the generator
   * @param settlementDate
   ...
   * @return The OIS coupon.
   */
  public static CouponONCompoundedDefinition from(final GeneratorSwapFixedCompoundedONCompounded generator, final ZonedDateTime settlementDate,
      final Period tenor, final double notional) {
    final ZonedDateTime fixingPeriodEndDate = ScheduleCalculator.getAdjustedDate(settlementDate, tenor, generator.getBusinessDayConvention(),
  ...
  /**
   * Builder from financial details. The accrual and fixing start and end dates are the same. The day count for the payment is the same as the one for the
   * index. And with the same fixingAccrualFactor for each period. The payment date is adjusted by the publication lag and the settlement days.
   * 
   * @param generator
   *          the generator
   * @param settlementDate
  ...
  public static CouponONCompoundedDefinition from(final GeneratorSwapFixedCompoundedONCompounded generator, final ZonedDateTime settlementDate,
    ...
    final ZonedDateTime paymentDate = ScheduleCalculator.getAdjustedDate(fixingPeriodEndDate,
        -1 + generator.getIndex().getPublicationLag() + generator.getSpotLag(), generator.getOvernightCalendar());
    final double paymentYearFraction = generator.getIndex().getDayCount().getDayCountFraction(settlementDate, fixingPeriodEndDate,
        generator.getOvernightCalendar());
    return new CouponONCompoundedDefinition(generator.getIndex().getCurrency(), paymentDate, settlementDate, fixingPeriodEndDate, paymentYearFraction, notional,
        generator.getIndex(), settlementDate,
  ...
  public CouponONCompounded toDerivative(final ZonedDateTime date, final String... yieldCurveNames) {
    ...
    final LocalDate firstPublicationDate = _fixingPeriodDates[_index.getPublicationLag()].toLocalDate(); // This is often one business day following the first
                                                                                                         // fixing date
    ...
    final CouponONCompounded cpn = new CouponONCompounded(getCurrency(), paymentTime, yieldCurveNames[0], getPaymentYearFraction(), getNotional(), _index,
  ...
  public Coupon toDerivative(final ZonedDateTime valZdt, final DoubleTimeSeries<ZonedDateTime> indexFixingTimeSeries) {
    ...
    if (fixedPeriod < _fixingPeriodDates.length - 1) { // Some OIS period left
      ...
      if (fixedPeriod < _fixingPeriodDates.length - 1) { // More OIS period left
        ...
        final CouponONCompounded cpn = new CouponONCompounded(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), _index,
            fixingPeriodStartTimes,
  ...
  public String toString() {
    return "CouponONCompoundedDefinition [_fixingPeriodDate=" + Arrays.toString(_fixingPeriodDates) + ", _fixingPeriodAccrualFactor="
        + Arrays.toString(_fixingPeriodAccrualFactors) + "]";
  }
