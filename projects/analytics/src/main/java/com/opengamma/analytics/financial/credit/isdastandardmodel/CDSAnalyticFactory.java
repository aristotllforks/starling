...
public class CDSAnalyticFactory {
  ...
  /**
   * Produce CDSs with the following default values.
   * <P>
   * Step-in: T+1<br>
  ...
  /**
   * Produce CDSs with the following default values and a supplied recovery rate.
   * <P>
   * Step-in: T+1<br>
   * Cash-Settle: T+3 working days<br>
   * Pay accrual on Default: true<br>
  ...
  /**
   * Produce CDSs with the following default values and a supplied coupon interval.
   * <P>
   * Step-in: T+1<br>
   * Cash-Settle: T+3 working days<br>
   * Pay accrual on Default: true<br>
  ...
  /**
   * Produce CDSs with the following default values and a supplied recovery rate and coupon interval.
   * <P>
   * Step-in: T+1<br>
   * Cash-Settle: T+3 working days<br>
   * Pay accrual on Default: true<br>
  ...
  /**
   * The Step-in (Protection Effective Date or sometimes just Effective Date) is usually T+1. This is when protection (and risk) starts in terms of the model.
   *
   * @param stepIn
   *          Zero or more days (after trade day)
  ...
  public CDSAnalyticFactory withStepIn(final int stepIn) {
    ...
    return new CDSAnalyticFactory(stepIn, _cashSettle, _payAccOnDefault, _couponInterval, _stubType, _protectStart, _recoveryRate,
        _businessdayAdjustmentConvention, _calendar, _accrualDayCount,
        _curveDayCount);
  }

  /**
   * Valuation or Cash-settle Date. This is the date for which the present value (PV) of the CDS is calculated. It is usually three working dates after the
   * trade date.
   *
   * @param cashSettle
   *          Zero or more days (after trade day)
  ...
  public CDSAnalyticFactory withCashSettle(final int cashSettle) {
    return new CDSAnalyticFactory(_stepIn, cashSettle, _payAccOnDefault, _couponInterval, _stubType, _protectStart, _recoveryRate,
        _businessdayAdjustmentConvention, _calendar, _accrualDayCount,
        _curveDayCount);
  }
   ...
   *
   * @param payAcc
   *          Set to true to pay accrued on default
   * @return A new factory with the payAccOnDefault set
   */
  public CDSAnalyticFactory withPayAccOnDefault(final boolean payAcc) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, payAcc, _couponInterval, _stubType, _protectStart, _recoveryRate, _businessdayAdjustmentConvention,
  ...
  public CDSAnalyticFactory with(final Period couponInterval) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, _payAccOnDefault, couponInterval, _stubType, _protectStart, _recoveryRate,
        _businessdayAdjustmentConvention, _calendar, _accrualDayCount,
        _curveDayCount);
  }

  /**
   * stubType Options are FRONTSHORT, FRONTLONG, BACKSHORT, BACKLONG or NONE (default is FRONTSHORT) - <b>Note</b> in this code NONE is not allowed.
   *
   * @param stubType
   *          The stub type
  ...
  public CDSAnalyticFactory with(final StubType stubType) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, _payAccOnDefault, _couponInterval, stubType, _protectStart, _recoveryRate,
        _businessdayAdjustmentConvention, _calendar, _accrualDayCount,
        _curveDayCount);
  }
   ...
   *          Protected from start of day?
   * @return A new factory with protectStart set
   */
  public CDSAnalyticFactory withProtectionStart(final boolean protectionStart) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, _payAccOnDefault, _couponInterval, _stubType, protectionStart, _recoveryRate,
        _businessdayAdjustmentConvention, _calendar, _accrualDayCount,
        _curveDayCount);
  }
  ...
  public CDSAnalyticFactory withRecoveryRate(final double recovery) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, _payAccOnDefault, _couponInterval, _stubType, _protectStart, recovery, _businessdayAdjustmentConvention,
        _calendar, _accrualDayCount,
        _curveDayCount);
  }
  ...
  public CDSAnalyticFactory with(final BusinessDayConvention busDay) {
    return new CDSAnalyticFactory(_stepIn, _cashSettle, _payAccOnDefault, _couponInterval, _stubType, _protectStart, _recoveryRate, busDay, _calendar,
        _accrualDayCount, _curveDayCount);
  }
  ...
  /**
   * Make a CDS with a maturity date the given period on from the next IMM date after the trade-date. The accrual start date will be the previous IMM date
   * (before the trade date), business-day adjusted. <b>Note</b> it payment interval is changed from the default of 3M, this will produce a (possibly incorrect)
   * non-standard first coupon.
   *
   * @param tradeDate
   *          The trade date
   * @param tenor
   *          The tenor (length) of the CDS
   * @return A CDS analytic description
   */
  ...
  /**
   * Make a CDS with a maturity date the given period on from the next IMM date after the trade-date. The accrual start date will be the previous IMM date
   * (before the trade date). <b>Note</b> it payment interval is changed from the default of 3M, this will produce a (possibly incorrect) non-standard first
   * coupon.
   *
   * @param tradeDate
   *          The trade date
   ...
   *          The tenor (length) of the CDS
   * @param makeEffBusDay
   *          Is the accrual start day business-day adjusted.
   * @return A CDS analytic description
   */
  ...
  /**
   * Make a set of CDSs with a common trade date and maturities dates the given periods after the next IMM date (after the trade-date). The accrual start date
   * will be the previous IMM date (before the trade date), business-day adjusted. <b>Note</b> it payment interval is changed from the default of 3M, this will
   * produce a (possibly incorrect) non-standard first coupon.
   *
   * @param tradeDate
   *          The trade date
   * @param tenors
   *          The tenors (lengths) of the CDSs
   * @return An array of CDS analytic descriptions
   */
  ...
  /**
   * Make a set of CDSs with a common trade date and maturities dates the given periods after the next IMM date (after the trade-date). The accrual start date
   * will be the previous IMM date (before the trade date). <b>Note</b> it payment interval is changed from the default of 3M, this will produce a (possibly
   * incorrect) non-standard first coupon.
   *
   * @param tradeDate
   *          The trade date
   * @param tenors
   ...
   *          The trade date
   * @param accStartDate
   *          This is when the CDS nominally starts in terms of premium payments. For a standard CDS this is the previous IMM date, and for a `legacy' CDS it is
   *          T+1
   * @param tenors
   *          The tenors (lengths) of the CDSs
   * @return An array of CDS analytic descriptions
   ...
   *          T+1
   * @param maturity
   *          The maturity. For a standard CDS this is an IMM date
   * @return A CDS analytic description
   */
   ...
   * @param maturity
   *          (aka end date) This is when the contract expires and protection ends - any default after this date does not trigger a payment. (the protection
   *          ends at end of day)
   * @return A CDS analytic description
   */
   ...
   *          T+1
   * @param maturities
   *          The maturities of the CDSs. For a standard CDS these are IMM dates
   * @return An array of CDS analytic descriptions
   */
   ...
   * @return An array of CDS analytic descriptions
   */
  public CDSAnalytic[] makeCDSs(final LocalDate tradeDate, final LocalDate stepinDate, final LocalDate valueDate, final LocalDate accStartDate,
      final LocalDate[] maturities) {
  ...
  /**
   * A forward starting CDS starts on some date after today (the trade date). The stepin date and cash settlement date are taken from the forward start date (1
   * day and 3 working days by default)
   *
   * @param tradeDate
  ...
  /**
   * A forward starting CDS starts on some date after today (the trade date). The accrual start must be specified (would normally use this for T+1 accrual
   * atart). The stepin date and cash settlement date are taken from the forward start date (1 day and 3 working days by default)
   *
   * @param tradeDate
   *          The trade date (i.e. today)
   * @param forwardStartDate
   ...
   * @return A CDS analytic description for a forward starting CDS
   */
  public CDSAnalytic makeForwardStartingCDS(final LocalDate tradeDate, final LocalDate forwardStartDate, final LocalDate accStartDate,
      final LocalDate maturity) {
    ArgumentChecker.isFalse(forwardStartDate.isBefore(tradeDate), "forwardStartDate of {} is before trade date of {}", forwardStartDate, tradeDate);
    final LocalDate stepinDate = forwardStartDate.plusDays(_stepIn);
    final LocalDate valueDate = addWorkDays(forwardStartDate, _cashSettle, _calendar);
  ...
  /**
   * A forward starting CDS starts on some date after today (the trade date). The stepin date and cash settlement date are taken from the forward start date (1
   * day and 3 working days by default). The period is from the next IMM date after the forward-start-date, so for a trade-date of 13-Feb-2014, a
   * forward-start-date of 25-Mar-2014 and a tenor of 1Y, the maturity will be 20-Jun-2015.
   *
   * @param tradeDate
   *          The trade date (i.e. today)
   * @param forwardStartDate
   *          The forward start date
   * @param tenor
   *          The tenor (length) of the CDS at the forwardStartDate
   * @return A CDS analytic description for a forward starting CDS
   */
  ...
  /**
   * Make a CDS represented as a MultiCDSAnalytic instance. Note, this is mainly for testing, since if you want only a single CDS should use a method that
   * returns a {@link CDSAnalytic}
   *
   * @param tradeDate
   *          The trade day
   ...
   * @param termMatIndex
   *          The maturities are fixed integer multiples of the payment interval, so 2Y tenor with a 3M payment interval, this would be 8
   ...
   *          The trade day
   * @param accStartDate
   *          This is when the CDS nominally starts in terms of the accrual calculation for premium payments. For a standard CDS this is the previous IMM date,
   *          and for a `legacy' CDS it is T+1
   * @param maturityReferanceDate
   *          A reference date that maturities are measured from. For standard CDSSs, this is the next IMM date after the trade date, so the actually maturities
   *          will be some fixed periods after this.
   ...
   */
  public MultiCDSAnalytic makeMultiCDS(final LocalDate tradeDate, final LocalDate accStartDate, final LocalDate maturityReferanceDate,
  ...
  /**
   * Make a set of standard CDS represented as a MultiCDSAnalytic instance. The first CDS with have a tenor of firstTenor, while the last CDS will have a tenor
   * of lastTenor; the remaining CDS will consist of all the (multiple of 3 month) tenors between the first and last tenor, e.g. if firstTenor = 6M and
   * lastTenor = 5Y, there will be a total of 22 CDS with tenors of 6M, 9M, 1Y,....4Y9M, 5Y
   *
   * @param tradeDate
   *          The trade date
   * @param firstTenor
  ...
  /**
   * Make a set of standard CDS represented as a MultiCDSAnalytic instance. The maturities of the CDS are measured from the next IMM date (after the trade
   * date), and the first and last tenors are the firstIndex and lastIndex multiplied by the coupon interval (3 months), which the remaining tenors being
  ...
  /**
   * Make a set of standard CDS represented as a MultiCDSAnalytic instance. The maturities of the CDS are measured from the next IMM date (after the trade
   * date), and the tenors are the given matIndices multiplied by the coupon interval (3 months)
   *
   * @param tradeDate
   *          the trade date
   * @param matIndices
   *          The CDS tenors are these multiplied by the coupon interval (3 months)
   * @return A set of CDS represented as a MultiCDSAnalytic
   */
