...
public class AnnuityCapFloorInflationYearOnYearInterpolationDefinition extends AnnuityCouponDefinition<CapFloorInflationYearOnYearInterpolationDefinition> {
  ...
  public static AnnuityCapFloorInflationYearOnYearInterpolationDefinition from(final IndexPrice priceIndex, final ZonedDateTime settlementDate,
    ...
    final ZonedDateTime[] paymentDates = ScheduleCalculator.getAdjustedDateSchedule(settlementDate, totalTenor, paymentPeriod, true, false,
    ...
    coupons[0] = CapFloorInflationYearOnYearInterpolationDefinition.from(settlementDate, paymentDates[0], notional, priceIndex, lastKnownFixingDate,
        conventionalMonthLag, monthLag, weightStart,
        weightEnd, strike, isCap);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      coupons[loopcpn] = CapFloorInflationYearOnYearInterpolationDefinition.from(paymentDates[loopcpn - 1], paymentDates[loopcpn], notional, priceIndex,
  ...
  public static AnnuityCapFloorInflationYearOnYearInterpolationDefinition from(final IndexPrice priceIndex, final ZonedDateTime settlementDate,
    ...
    final CapFloorInflationYearOnYearInterpolationDefinition[] coupons = new CapFloorInflationYearOnYearInterpolationDefinition[paymentDates.length];
    coupons[0] = CapFloorInflationYearOnYearInterpolationDefinition.from(settlementDate, paymentDates[0], notional, priceIndex, conventionalMonthLag, monthLag,
        lastKnownFixingDate,
        strike, isCap);
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      coupons[loopcpn] = CapFloorInflationYearOnYearInterpolationDefinition.from(paymentDates[loopcpn - 1], paymentDates[loopcpn], notional, priceIndex,
          conventionalMonthLag, monthLag,
          lastKnownFixingDate,
          strike, isCap);
    }
    return new AnnuityCapFloorInflationYearOnYearInterpolationDefinition(coupons, calendar);
