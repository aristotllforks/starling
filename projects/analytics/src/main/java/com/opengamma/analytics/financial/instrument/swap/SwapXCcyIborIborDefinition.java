...
public class SwapXCcyIborIborDefinition extends SwapDefinition {
  ...
  /**
   * Builder from the settlement date and a generator. The legs have different notionals. The notionals are paid on the settlement date and final payment date
   * of each leg.
   *
   ...
   * @param notional1
   *          The first leg notional.
   * @param notional2
   *          The second leg notional.
   * @param spread
  ...
  public static SwapXCcyIborIborDefinition from(final ZonedDateTime settlementDate, final Period tenor, final GeneratorSwapXCcyIborIbor generator,
      final double notional1, final double notional2, final double spread, final boolean isPayer, final Calendar calendar1, final Calendar calendar2) {
   ...
   *          The spread to be applied to the first leg.
   * @param spread2
   *          The spread to be applied to the second leg.
   * @param isPayer
   *          The payer flag for the first leg.
   * @return The swap.
   */
  public static SwapXCcyIborIborDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate, final GeneratorSwapXCcyIborIbor generator,
      final double notional1, final double notional2, final double spread1, final double spread2, final boolean isPayer) {
    ArgumentChecker.notNull(settlementDate, "settlement date");
    ArgumentChecker.notNull(maturityDate, "Maturity date");
    ArgumentChecker.notNull(generator, "Swap generator");
    // TODO: create a mechanism for the simultaneous payments on both legs, i.e. joint calendar
    final AnnuityDefinition<CouponDefinition> firstLegNotional = AnnuityDefinitionBuilder.couponIborSpreadWithNotional(settlementDate, maturityDate, notional1,
        spread1, generator.getIborIndex1(), isPayer, generator.getCalendar1(), StubType.SHORT_START, 0, true, true);
    final AnnuityDefinition<CouponDefinition> secondLegNotional = AnnuityDefinitionBuilder.couponIborSpreadWithNotional(settlementDate, maturityDate, notional2,
        spread2, generator.getIborIndex2(), !isPayer, generator.getCalendar2(), StubType.SHORT_START, 0, true, true);
