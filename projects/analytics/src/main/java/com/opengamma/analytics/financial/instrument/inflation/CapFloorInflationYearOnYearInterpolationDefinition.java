...
/**
 * Class describing a Year on Year inflation caplet (or floorlet) were the inflation figure are interpolated. The index for a given month is given in the yield
 * curve and in the time series on the first of the month. The pay-off is paymentYearFraction* /[(Index_End / Index_Start - 1)-strike/]^{+} t.
 */
public class CapFloorInflationYearOnYearInterpolationDefinition extends CouponInflationDefinition implements CapFloor {
   ...
   * @param weightStart
   *          he weight on the first month index in the interpolation of the index at the _referenceStartDate.
   * @param weightEnd
   *          The weight on the first month index in the interpolation of the index at the _referenceEndDate.
   * @param strike
   *          The strike
   * @param isCap
   *          The cap/floor flag.
  ...
  public CapFloorInflationYearOnYearInterpolationDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      ...
      final int conventionalMonthLag, final int monthLag, final ZonedDateTime[] referenceStartDate, final ZonedDateTime[] referenceEndDate,
   ...
   */
  public static CapFloorInflationYearOnYearInterpolationDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime paymentDate,
      final double notional,
      final IndexPrice priceIndex, final ZonedDateTime lastKnownFixingDate, final int conventionalMonthLag, final int monthLag,
  ...
  public static CapFloorInflationYearOnYearInterpolationDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime paymentDate,
      final double notional,
      final IndexPrice priceIndex, final ZonedDateTime lastKnownFixingDate, final int conventionalMonthLag, final ZonedDateTime[] referenceStartDate,
   ...
   * @return The cap/floor
   */
  public static CapFloorInflationYearOnYearInterpolationDefinition from(final CouponInflationYearOnYearInterpolationDefinition coupon,
      final ZonedDateTime lastKnownFixingDate,
      final double strike, final boolean isCap) {
   ...
   *          The lag in month between the index validity and the coupon dates for the actual product.
   * @param weightStart
   *          The weight on the first month index in the interpolation of the index at the _referenceStartDate.
   * @param weightEnd
   *          The weight on the first month index in the interpolation of the index at the _referenceEndDate.
   * @param lastKnownFixingDate
   *          The fixing date (always the first of a month) of the last known fixing.
  ...
  public static CapFloorInflationYearOnYearInterpolationDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime paymentDate,
      final double notional,
      final IndexPrice priceIndex, final ZonedDateTime lastKnownFixingDate, final int conventionalMonthLag, final int monthLag, final double weightStart,
      final double weightEnd,
      final double strike, final boolean isCap) {
    ...
    return new CapFloorInflationYearOnYearInterpolationDefinition(priceIndex.getCurrency(), paymentDate, accrualStartDate, paymentDate, 1.0, notional,
        priceIndex,
        lastKnownFixingDate, conventionalMonthLag, monthLag, referenceStartDate, referenceEndDate, weightStart, weightEnd, strike, isCap);
  }

  /**
   * Builder for inflation year on year coupon based on an inflation lag and the index publication lag. The fixing date is the publication lag after the last
   * reference month. The index start value is calculated from a time series. The index value required for the coupon should be in the time series.
   * 
   * @param accrualStartDate
   *          Start date of the accrual period.
   * @param paymentDate
  ...
  public static CapFloorInflationYearOnYearInterpolationDefinition from(final ZonedDateTime accrualStartDate, final ZonedDateTime paymentDate,
    ...
    return from(accrualStartDate, paymentDate, notional, priceIndex, lastKnownFixingDate, conventionalmonthLag, monthLag, weightStart, weightEnd, strike,
        isCap);
  }
  ...
  public CapFloorInflationYearOnYearInterpolation toDerivative(final ZonedDateTime date) {
    ...
    return new CapFloorInflationYearOnYearInterpolation(getCurrency(), paymentTime, getPaymentYearFraction(), getNotional(), getPriceIndex(),
        lastKnownFixingTime, referenceStartTime,
