...
/**
 * Class describing a overnight-indexed floating coupon with a spread. The description is simplified by not creating the full set of fixing dates. Only the
 * start and the end of the fixing period are described. The description is enough to construct curves from OIS and to price forward OIS but not to describe and
 * price OIS coupons for which some fixing already took place.
 */
public class CouponONSpreadSimplifiedDefinition extends CouponDefinition {
  ...
  public CouponONSpreadSimplifiedDefinition(final Currency currency, final ZonedDateTime paymentDate, final ZonedDateTime accrualStartDate,
      final ZonedDateTime accrualEndDate, final double paymentAccrualFactor, final double notional, final IndexON index,
  ...
  /**
   * Builder from financial details. The accrual and fixing dates (start and end) are the same. The day count for the payment is the same as the one for the
   * index.
   * 
   * @param index
   *          The OIS index.
   ...
   */
  public static CouponONSpreadSimplifiedDefinition from(final IndexON index, final ZonedDateTime settlementDate, final Period tenor, final double notional,
      final double spread,
  ...
  /**
   * Builder from the financial details. The accrual and fixing dates (start and end) are the same. The day count for the payment is the same as the one for the
   * index. The payment date is computed from the endFixingPeriodDate by moving backward by one day (overnight), then forward by the index publication lag and
   * finally by the settlementDays days.
   * 
   * @param index
   *          The OIS index.
   * @param settlementDate
   ...
   *          The end date of the fixing period (also used for the end accrual date).
   * @param notional
   *          The notional.
   * @param spread
   *          The spread.
   * @param settlementDays
   *          The number of days between last fixing date and the payment date (also called payment lag).
   * @param calendar
   *          The holiday calendar for the overnight index.
   * @return The OIS coupon.
   */
