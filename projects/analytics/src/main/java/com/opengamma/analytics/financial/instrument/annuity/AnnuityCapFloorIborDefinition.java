...
public class AnnuityCapFloorIborDefinition extends AnnuityDefinition<CapFloorIborDefinition> {
   ...
   *          The payer flag.
   * @param strike
   *          The common strike of all caplet.
   * @param isCap
   ...
   */
  public static AnnuityCapFloorIborDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate, final double notional,
      final IborIndex index, final boolean isPayer,
  ...
  /**
   * Annuity builder from the conventions and common characteristics. The payments uses the index conventions and payment period. The first caplet/floorlet
   * (which is in practice immediately fixed) is not included.
   * 
   * @param settlementDate
   *          The settlement date.
   * @param maturityDate
   ...
   * @return The Ibor annuity.
   */
  public static AnnuityCapFloorIborDefinition fromWithNoInitialCaplet(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate,
  ...
  /**
   * Annuity builder from the conventions and common characteristics. The payments index conventions and payment period are separated from the one of the
   * underlying index.
   * 
   * @param settlementDate
   *          The settlement date.
   ...
   */
  public static AnnuityCapFloorIborDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate, final double notional,
      final IborIndex index, final DayCount dayCount,
    ...
    coupons[0] = CapFloorIborDefinition.from(paymentDates[0], settlementDate, paymentDates[0],
    ...
    for (int loopcpn = 1; loopcpn < paymentDates.length; loopcpn++) {
      ...
      coupons[loopcpn] = CapFloorIborDefinition.from(paymentDates[loopcpn], paymentDates[loopcpn - 1], paymentDates[loopcpn],
          dayCount.getDayCountFraction(paymentDates[loopcpn - 1], paymentDates[loopcpn], calendar), sign * notional, fixingDate, index, strike, isCap,
          calendar);
    }
