...
/**
 * Construct a credit curve that is consistent with the ISDA c code - i.e. the credit curve is piecewise constant in the (forward) hazard
 * rates, and agrees with ISDA (up to numerical round-off) for the same inputs.
 */
public abstract class ISDACompliantCreditCurveBuilder {
  ...
  /**
   * Bootstrap the credit curve from a single market CDS quote. Obviously the resulting credit (hazard) curve will be flat.
   * 
   * @param calibrationCDS
   *          The single market CDS - this is the reference instruments used to build the credit curve
   * @param marketQuote
   *          The market quote of the CDS
   * @param yieldCurve
  ...
  /**
   * Bootstrap the credit curve from a set of reference/calibration CDSs with market quotes.
   * 
   * @param calibrationCDSs
   *          The market CDSs - these are the reference instruments used to build the credit curve
   * @param marketQuotes
   *          The market quotes of the CDSs
   * @param yieldCurve
   *          The yield (or discount) curve
   * @return The credit curve
   */
  ...
  /**
   * Bootstrap the credit curve from a single market CDS quote given as a par spread. Obviously the resulting credit (hazard) curve will be
   * flat.
   * 
   * @param cds
   *          The single market CDS - this is the reference instruments used to build the credit curve
   * @param parSpread
   *          The <b>fractional</b> par spread of the market CDS
   * @param yieldCurve
   *          The yield (or discount) curve
   * @return The credit curve
   */
  public ISDACompliantCreditCurve calibrateCreditCurve(final CDSAnalytic cds, final double parSpread,
      final ISDACompliantYieldCurve yieldCurve) {
    return calibrateCreditCurve(new CDSAnalytic[] { cds }, new double[] { parSpread }, yieldCurve, new double[1]);
  }

  /**
   * Bootstrap the credit curve from a single market CDS quote given as points up-front (PUF) and a standard premium.
   * 
   * @param cds
   *          The single market CDS - this is the reference instruments used to build the credit curve
   * @param premium
   *          The standard premium (coupon) as a fraction (these are 0.01 or 0.05 in North America)
   * @param yieldCurve
   *          The yield (or discount) curve
   * @param pointsUpfront
   *          points up-front as a fraction of notional
   * @return The credit curve
   */
  ...
  /**
   * Bootstrap the credit curve from a set of reference/calibration CDSs quoted with par spreads.
   * 
   * @param calibrationCDSs
   *          The market CDSs - these are the reference instruments used to build the credit curve
   * @param parSpreads
   ...
   * @return The credit curve
   */
  public ISDACompliantCreditCurve calibrateCreditCurve(final CDSAnalytic[] calibrationCDSs, final double[] parSpreads,
      final ISDACompliantYieldCurve yieldCurve) {
  ...
  /**
   * Bootstrap the credit curve from a set of reference/calibration CDSs quoted with points up-front and standard premiums.
   * 
   * @param calibrationCDSs
   *          The market CDSs - these are the reference instruments used to build the credit curve
   * @param premiums
   *          The standard premiums (coupons) as fractions (these are 0.01 or 0.05 in North America)
   * @param yieldCurve
   *          The yield (or discount) curve
   * @param pointsUpfront
   *          points up-front as fractions of notional
   * @return The credit curve
   */
  public abstract ISDACompliantCreditCurve calibrateCreditCurve(CDSAnalytic[] calibrationCDSs, double[] premiums,
      ISDACompliantYieldCurve yieldCurve, double[] pointsUpfront);

  /**
   * Bootstrap the credit curve from a single CDS, by making it have zero clean price. Obviously the resulting credit (hazard) curve will be
   * flat.
   * 
   * @param tradeDate
   *          The 'current' date
   ...
   * @param startDate
   *          The protection start date. If protectStart = true, then protections starts at the beginning of the day, otherwise it is at the
   *          end.
   * @param endDate
   *          The maturity (or end of protection) of the CDS
   * @param fractionalParSpread
   *          - the (fractional) coupon that makes the CDS worth par (i.e. zero clean price)
   ...
   * @return The credit curve
   */
  public ISDACompliantCreditCurve calibrateCreditCurve(final LocalDate tradeDate, final LocalDate stepinDate, final LocalDate valueDate,
      final LocalDate startDate, final LocalDate endDate, final double fractionalParSpread, final boolean payAccOnDefault,
      final Period tenor,
      final StubType stubType, final boolean protectStart, final ISDACompliantYieldCurve yieldCurve,
      final double recoveryRate) {
    return calibrateCreditCurve(tradeDate, stepinDate, valueDate, startDate, new LocalDate[] { endDate },
        new double[] { fractionalParSpread },
        payAccOnDefault, tenor, stubType, protectStart, yieldCurve, recoveryRate);
  }

  /**
   * Bootstrap the credit curve, by making each market CDS in turn have zero clean price.
   * 
   * @param tradeDate
   *          The 'current' date
   * @param stepinDate
   ...
   *          end.
   * @param endDates
   *          The maturities (or end of protection) of each of the CDSs - must be ascending
   * @param fractionalParSpreads
   *          - the (fractional) coupon that makes each CDS worth par (i.e. zero clean price)
   * @param payAccOnDefault
   *          Is the accrued premium paid in the event of a default
   * @param tenor
   *          The nominal step between premium payments (e.g. 3 months, 6 months).
   ...
   */
  public ISDACompliantCreditCurve calibrateCreditCurve(final LocalDate tradeDate, final LocalDate stepinDate, final LocalDate valueDate,
      final LocalDate startDate, final LocalDate[] endDates, final double[] fractionalParSpreads, final boolean payAccOnDefault,
      final Period tenor,
      final StubType stubType, final boolean protectStart, final ISDACompliantYieldCurve yieldCurve,
      final double recoveryRate) {
    ...
    for (int i = 0; i < n; i++) {
      cds[i] = new CDSAnalytic(tradeDate, stepinDate, valueDate, startDate, endDates[i], payAccOnDefault, tenor, stubType, protectStart,
          recoveryRate);
    }
   ...
   */
  private double[] getStandardQuoteForm(final CDSAnalytic calibrationCDS, final CDSQuoteConvention marketQuote,
      final ISDACompliantYieldCurve yieldCurve) {
    ...
    if (marketQuote instanceof ParSpread) {
      ...
      final ISDACompliantCreditCurve cc = calibrateCreditCurve(new CDSAnalytic[] { calibrationCDS }, new double[] { qSpread }, yieldCurve,
          new double[1]);
      res[0] = coupon;
  ...
  public enum ArbitrageHandling {
    /**
     * If the market data has arbitrage, the curve will still build, but the survival probability will not be monotonically decreasing
    ...
    Fail,
    /**
     * If a particular spread implies a negative forward hazard rate, the hazard rate is set to zero, and the calibration continues. The
     * resultant curve will of course not exactly reprice the input CDSs, but will find new spreads that just avoid arbitrage.
     */
    ZeroHazardRate
  }
