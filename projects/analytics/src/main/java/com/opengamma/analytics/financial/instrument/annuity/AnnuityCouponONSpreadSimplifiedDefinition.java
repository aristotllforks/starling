...
public class AnnuityCouponONSpreadSimplifiedDefinition extends AnnuityDefinition<CouponONSpreadSimplifiedDefinition> {
   ...
   * @param tenorAnnuity
   *          The annuity tenor, not null
   * @param notional
   *          The annuity notional.
   * @param spread
   *          The annuity spread. Same spread for all coupons.
   * @param generator
   ...
   * @return The annuity.
   */
  public static AnnuityCouponONSpreadSimplifiedDefinition from(final ZonedDateTime settlementDate, final Period tenorAnnuity, final double notional,
      final double spread,
    ...
    return AnnuityCouponONSpreadSimplifiedDefinition.from(settlementDate, endFixingPeriodDate, notional, spread, isPayer, generator.getIndex(),
   ...
   *          The settlement date, not null
   * @param maturityDate
   *          The maturity date. The maturity date is the end date of the last fixing period, not null
   * @param notional
   *          The notional.
   * @param spread
   *          The annuity spread. Same spread for all coupons.
   * @param generator
   *          The generator, not null.
   * @param isPayer
   *          The flag indicating if the annuity is paying (true) or receiving (false).
   * @return The annuity.
   */
  public static AnnuityCouponONSpreadSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime maturityDate, final double notional,
      final double spread,
      final GeneratorSwapFixedON generator, final boolean isPayer) {
    ArgumentChecker.notNull(settlementDate, "settlement date");
    ArgumentChecker.notNull(maturityDate, "maturity date");
    ArgumentChecker.notNull(generator, "generator");
    final ZonedDateTime[] endFixingPeriodDate = ScheduleCalculator.getAdjustedDateSchedule(settlementDate, maturityDate, generator.getLegsPeriod(),
        generator.getBusinessDayConvention(),
        generator.getOvernightCalendar(), generator.isEndOfMonth());
    return AnnuityCouponONSpreadSimplifiedDefinition.from(settlementDate, endFixingPeriodDate, notional, spread, isPayer, generator.getIndex(),
        generator.getPaymentLag(),
   ...
   * @return The annuity.
   */
  public static AnnuityCouponONSpreadSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime endFixingPeriodDate,
      final double notional, final double spread,
      final boolean isPayer, final Period paymentPeriod, final IndexON indexON, final int paymentLag, final BusinessDayConvention businessDayConvention,
      final boolean isEOM,
      final Calendar indexCalendar) {
    return from(settlementDate, endFixingPeriodDate, notional, spread, isPayer, paymentPeriod, indexON, paymentLag, businessDayConvention, isEOM, indexCalendar,
   ...
   */
  private static AnnuityCouponONSpreadSimplifiedDefinition from(final ZonedDateTime settlementDate, final ZonedDateTime[] endFixingPeriodDate,
      final double notional, final double spread,
    ...
    for (int loopcpn = 1; loopcpn < endFixingPeriodDate.length; loopcpn++) {
      coupons[loopcpn] = CouponONSpreadSimplifiedDefinition.from(indexON, endFixingPeriodDate[loopcpn - 1], endFixingPeriodDate[loopcpn], notionalSigned,
          spread, paymentLag,
          indexCalendar);
    }
