...
public class ExamplesSettlementHolidayLoader extends AbstractTool<ToolContext> {
  ...
  protected void doRun() {
    ...
    for (final RegionDocument doc : searchResult.getDocuments()) {
      ...
      calendar.setType(HolidayType.SETTLEMENT);
      calendar.setExchangeExternalId(ExternalSchemes.countryRegionId(region.getCountry()));
  ...
  private void storeHolidays(final ManageableHoliday calendar, final HolidayMaster holidayMaster) {
    ...
    switch (calendar.getType()) {
      case SETTLEMENT:
        request.addExchangeExternalIds(calendar.getExchangeExternalId());
        break;
      default:
        throw new IllegalStateException("The only supported holiday type is SETTLEMENT");
    }
