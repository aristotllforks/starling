...
public class ExampleCDSLoader extends AbstractTool<ToolContext> {
  ...
  private void portfolioWithSecurity(final Security security, final String portfolioName) {
    ...
    final PortfolioDocument portfolioDoc = new PortfolioDocument(portfolio);
  ...
  private CDSSecurity makeOneCDS() {
    ...
    final ZonedDateTime maturity = LocalDateTime.of(2020, 12, 20, 0, 0, 0, 0).atZone(ZoneOffset.UTC);
    final ZonedDateTime startDate = LocalDateTime.of(2010, 12, 20, 0, 0, 0, 0).atZone(ZoneOffset.UTC);
    final SimpleFrequency frequency = SimpleFrequency.ANNUAL;
  ...
  protected ManageablePosition makePositionAndTrade(final Security security) {
